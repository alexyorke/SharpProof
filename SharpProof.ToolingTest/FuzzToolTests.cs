...
public class FuzzToolTests
{
    ...
    private static readonly ImmutableArray<FamilyExpectation> ExpandedCoverageExpectations = ImmutableArray.Create(
        ...
        new FamilyExpectation("PureInterpolatedStringHandler", "InterpolatedStringHandlerCreation",
            "InterpolatedStringAddition", "InterpolatedStringAppendLiteral", "InterpolatedStringAppendFormatted",
    ...
    private static readonly ImmutableArray<(string Family, bool ExpectSp0002, bool ExpectSp0010)>
        ExceptionNegativeExpectations = ImmutableArray.Create(
    ...
    public async Task FuzzRunner_SmokeRun_WritesSummaryAndCoverageArtifacts()
    {
        ...
        try
        {
            ...
            Assert.That(summary.GeneratorBackedShapeCount,
                Is.EqualTo(RoslynShapeManifest.GeneratorBackedShapeIds.Length));
            Assert.That(summary.GeneratorBackedShapesWithRegistryCount,
                Is.EqualTo(RoslynShapeManifest.GeneratorBackedShapeIds.Length));
    ...
    {
        var nonFinite = Assert.Throws<ArgumentException>(static () =>
            FuzzOptions.Parse(new[] { "--iterations", "0", "--seconds", "Infinity" }));
        Assert.That(nonFinite!.Message, Does.Contain("--seconds expects a finite non-negative number."));

        var outOfRange = Assert.Throws<ArgumentException>(static () =>
            FuzzOptions.Parse(new[] { "--iterations", "0", "--hours", "1E100" }));
    ...
    public async Task FuzzRunner_RunCasesAsync_DedupesRepeatedInterestingCases_AndCapsSavedCasesPerFamily()
    {
        ...
        try
        {
            ...
            var cases = ImmutableArray.Create(
                new FuzzCase("CompileFailA", "CompileFail", repeatedSource, false, FuzzExpectation.Conservative()),
    ...
    public void FuzzCaseGenerator_DeterministicSample_IncludesExpandedFamilies()
    {
        ...
        var primaryShapes = generatedCases
            .SelectMany(fuzzCase => fuzzCase.PrimaryShapeIds.IsDefaultOrEmpty
                ? Enumerable.Empty<string>()
                : fuzzCase.PrimaryShapeIds)
            .Distinct(StringComparer.Ordinal)
    ...
    [Test]
    public async Task
        RunCasesAsync_SummaryReport_PreservesExceptionEdgesProperty_InUnexpectedSp0010Finding_WhenPresent()
    {
        var outputDirectory = CreateOutputDirectory();
        try
        {
            ...
            var unexpectedSp0010Finding =
                persistedSummary!.Findings.Single(finding => finding.Category == "unexpected_sp0010");
            var expectedSignature =
                analysis.DiagnosticSignatures.Single(signature =>
    ...
    {
        if (IsConservativeExpectation(expectation)) return "conservative";
