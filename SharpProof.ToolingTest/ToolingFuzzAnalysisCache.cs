...
internal static class ToolingFuzzAnalysisCache {
    ...
    public static Task<ImmutableDictionary<string, FuzzCaseAnalysis>> GetRegistryEntryAnalysesAsync() => RegistryEntryAnalyses.Value;
    private static async Task<ImmutableDictionary<string, FuzzCaseAnalysis>> CreateRegistryEntryAnalysesAsync() {
        ...
        var cases = FuzzCaseGenerator.RegistryEntries
            ...
            .ToImmutableArray();
        var analyses = await ToolingFuzzTestRunner.AnalyzeCasesAsync(cases, false, AnalysisParallelism);
