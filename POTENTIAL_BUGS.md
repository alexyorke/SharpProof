...
| --- | --- | --- |
| 51 | `PurityAnalysisEngine.CfgBranchAssumptions.TryCreateSuccessorState` skips the unsatisfiability check when only the symbolic branch state changed. | Add a CFG regression whose symbolic-only branch contradicts the incoming state, then prune the successor. |
| 52 | `WorklistPuritySolver` assigns `Pure` to every method without a declaring syntax reference. | Add an interface/metadata dispatch regression and replace unconditional purity with the established external-call policy if it is unsound. |
...
IDs: 1, 5, 6, 7, 11, 15, 20, 31, 35, 40, 45, 48, 59, 71, 85,
