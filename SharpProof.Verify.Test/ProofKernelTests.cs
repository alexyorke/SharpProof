...
public sealed class ProofKernelTests
{
    ...
    [TestCase(ProofDiagnosticKind.EffectContract)]
    [TestCase(ProofDiagnosticKind.Precondition)]
    public async Task UndefinedNonPostconditionGoalsUseNeutralReplayFailure(
        ProofDiagnosticKind diagnostic)
    {
        ...
        var query = new VerificationQuery(
            factory,
            [],
            new Goal(factory, predicate, diagnostic, new SourceLocationId(0)),
            [divisor]);
        var model = new BackendModel([
            KeyValuePair.Create(divisor, factory.CreateIntegerValue(0))
        ]);
