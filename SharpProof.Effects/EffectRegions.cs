...
public readonly struct EffectRegionId :
    ...
    public static EffectRegionId Parameter(int ordinal) =>
        new(EffectRegionKind.Parameter, ordinal);
    public static EffectRegionId Captured(int ordinal) =>
        new(EffectRegionKind.Captured, ordinal);
    public static EffectRegionId Static(int ordinal = 0) =>
        new(EffectRegionKind.Static, ordinal);
    public static EffectRegionId Fresh(int ordinal) =>
        new(EffectRegionKind.Fresh, ordinal);
...
public readonly struct EffectRegionSet : IEquatable<EffectRegionSet> {
    ...
    public static EffectRegionSet Unknown { get; } =
        new([EffectRegionId.Unknown]);

    public ImmutableArray<EffectRegionId> Regions =>
        _regions.IsDefault ? [] : _regions;

    public bool IsEmpty => _regions.IsDefaultOrEmpty;
    public bool IsUnknown =>
        !_regions.IsDefaultOrEmpty &&
        _regions.Length == 1 &&
        _regions[0].Kind == EffectRegionKind.Unknown;

    public static EffectRegionSet Create(params EffectRegionId[] regions) =>
        Create((IEnumerable<EffectRegionId>)regions);
