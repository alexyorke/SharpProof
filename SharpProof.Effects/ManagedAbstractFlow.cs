...
internal sealed class ManagedAbstractFlow {
    ...
    internal static ManagedAbstractFlow ForCompilation(Compilation compilation) =>
        Sessions.GetValue(compilation ?? throw new ArgumentNullException(nameof(compilation)), static value => new(value));
    ...
    internal ManagedFlowState CreateEntryState(IMethodSymbol method) {
        ...
        foreach (var parameter in method.Parameters) {
            ...
            if (parameter.RefKind != RefKind.Out) value = ApplyAttributes(value, parameter.GetAttributes());
    ...
    internal static ManagedFlowState Refine(
        ...
        Refine(state, (object)symbol, @operator, value, expected);
    ...
    private static ManagedFlowState Refine(
        ...
        if (!value.IsDefinitelyNull) return state;
        if (!current.TryGetNullness(out var nullness)) return state;
        var equals = @operator == BinaryOperatorKind.Equals ? expected
            : @operator == BinaryOperatorKind.NotEquals ? !expected : (bool?)null;
        if (!equals.HasValue) return state;
    ...
    private static ManagedAbstractValue RefineInteger(
        ...
        if (!current.TryGetInteger(out var interval)) return current;
    ...
    private ManagedAbstractValue EvaluateProperty(IPropertyReferenceOperation property, ManagedFlowState state) {
        if (property.Instance != null && property.Property.Name is "Length" or "LongLength") {
            ...
            if (receiver.TryGetCardinality(out var length)) return ManagedAbstractValue.Integer(length);
    ...
    private ManagedAbstractValue NullTest(IIsNullOperation operation, ManagedFlowState state) =>
        EvaluateCore(operation.Operand, state).TryGetNullness(out var value)
            ? value switch {
    ...
    private ManagedAbstractValue ConvertValue(IConversionOperation conversion, ManagedFlowState state) {
        ...
        if (ValuePreserving(conversion)) return operand;
    ...
    private ManagedAbstractValue EvaluateUnary(IUnaryOperation unary, ManagedFlowState state) {
        ...
        if (unary.OperatorKind == UnaryOperatorKind.Not) return ManagedAbstractValue.NegateBoolean(operand);
    ...
    private ManagedAbstractValue EvaluateCoalesce(ICoalesceOperation operation, ManagedFlowState state) {
        ...
        if (value.IsDefinitelyNonNull) return value;
        if (value.IsDefinitelyNull) return EvaluateCore(operation.WhenNull, state);
    ...
    internal bool ProvesNoOverflow(IOperation operation, ManagedFlowState state) {
        ...
        switch (operation) {
            ...
            case IConversionOperation conversion:
                if (!EvaluateCore(conversion.Operand, state).TryGetInteger(out interval)) return false;
    ...
    private static bool Matches(AttributeData attribute, INamedTypeSymbol? expected) =>
        expected != null && SymbolEqualityComparer.Default.Equals(
    ...
    private static ManagedFlowState HavocCall(
        ...
        method.MethodKind == MethodKind.LocalFunction || method.ContainingType.TypeKind == TypeKind.Delegate
            ? state.Forget()
    ...
    private static ManagedFlowState SetStorage(
        ...
        TryStorage(operation, out var storage) ? state.Set(storage, value) : state;
    ...
    private static bool ValuePreserving(IConversionOperation conversion) {
        if (conversion.OperatorMethod != null || conversion.Conversion.IsUserDefined || conversion.IsTryCast) return false;
    ...
    internal static bool IsAcyclic(ControlFlowGraph graph) {
        ...
        bool Visit(BasicBlock block) {
            if (marks[block.Ordinal] != 0) return marks[block.Ordinal] == 2;
            marks[block.Ordinal] = 1;
            foreach (var (branch, _) in Successors(block))
                if (!Visit(branch.Destination!)) return false;
    ...
    private static IEnumerable<(ControlFlowBranch Branch, bool? Expected)> Successors(BasicBlock block) {
        ...
        if (Regular(block.ConditionalSuccessor)) yield return (block.ConditionalSuccessor!, expected);
    }

    private static bool Regular(ControlFlowBranch? branch) =>
        branch is { Semantics: ControlFlowBranchSemantics.Regular, Destination: not null };
    ...
    private static bool IntegerType(ITypeSymbol? type, out CSharpIntegerSemantics semantics) =>
        CSharpScalarSemantics.TryGetInteger(type?.SpecialType ?? SpecialType.None, out semantics);
...
internal sealed class ManagedFlowAnalysis {
    private ManagedFlowAnalysis(
        ...
        if (!valid) throw new ArgumentException("Managed-flow status, reason, and result are inconsistent.");
        Status = status;
        IncompleteReason = reason;
        Result = result;
    }

    internal ManagedFlowStatus Status { get; }
    internal EffectAnalysisIncompleteReason IncompleteReason { get; }
    internal ManagedFlowResult? Result { get; }
    ...
    internal static ManagedFlowAnalysis Complete(ManagedFlowResult result) =>
        new(ManagedFlowStatus.Complete, EffectAnalysisIncompleteReason.None,
    ...
    internal static ManagedFlowAnalysis BudgetExceeded(EffectAnalysisIncompleteReason reason) =>
        new(ManagedFlowStatus.BudgetExceeded, reason, null);

    internal static ManagedFlowAnalysis Cyclic() =>
        new(ManagedFlowStatus.Cyclic, EffectAnalysisIncompleteReason.CyclicControlFlow, null);
}
...
internal sealed class ManagedFlowResult(ManagedAbstractFlow flow) {
    ...
    internal bool TryGetState(IOperation operation, out ManagedFlowState state) =>
        _states.TryGetValue(operation, out state!) || _states.TryGetValue(Key(operation), out state!);
    ...
    internal static IOperation? GetUnavoidableDirectOperation(
        ...
        while (true) {
            ...
            if (ReferenceEquals(unwrapped, operation)) return operation;
            operation = unwrapped;
        }
    }
    ...
    internal bool ProvesNonNull(IOperation origin, IOperation value) =>
        TryEvaluate(origin, value, out var result) && result.IsDefinitelyNonNull;

    internal bool ProvesNonZero(IOperation origin, IOperation value) =>
        TryEvaluate(origin, value, out var result) && result.IsDefinitelyNonZero;

    internal bool ProvesNonNegative(IOperation origin, IOperation value) =>
        TryEvaluate(origin, value, out var result) &&
    ...
    internal bool ProvesNoOverflow(IOperation operation) =>
        operation is not IBinaryOperation binary || !HasMutation(binary.RightOperand)
    ...
    private static bool HasMutation(IOperation operation) =>
        operation.DescendantsAndSelf().Any(static candidate =>
    ...
    private static bool IsControlFlow(IOperation operation) =>
        operation is IConditionalOperation or IConditionalAccessOperation or ISwitchOperation or
    ...
    private static (SyntaxTree, int, int, OperationKind) Key(IOperation operation) =>
        (operation.Syntax.SyntaxTree, operation.Syntax.SpanStart, operation.Syntax.Span.Length, operation.Kind);
...
internal sealed class ManagedFlowState {
    ...
    private ManagedFlowState(ImmutableDictionary<object, ManagedAbstractValue>? values) => _values = values;
    ...
    internal ManagedAbstractValue Get(ISymbol symbol) => Get((object)symbol);
    internal ManagedAbstractValue Get(CaptureId capture) => Get((object)capture);

    internal ManagedAbstractValue Get(object storage) {
        if (_values == null) return ManagedAbstractValue.Bottom;
        if (_values.TryGetValue(storage, out var value)) return value;
    ...
    internal ManagedFlowState Set(ISymbol symbol, ManagedAbstractValue value) => Set((object)symbol, value);
    internal ManagedFlowState Set(CaptureId capture, ManagedAbstractValue value) => Set((object)capture, value);
    internal ManagedFlowState Set(object storage, ManagedAbstractValue value) =>
        _values == null || value.IsBottom ? Bottom : new(_values.SetItem(storage, value));
    internal ManagedFlowState Forget() => IsBottom ? this : Empty;

    internal static ManagedFlowState Join(ManagedFlowState left, ManagedFlowState right) {
        if (left._values == null) return right;
        if (right._values == null) return left;
    ...
    internal static bool LessThanOrEqual(ManagedFlowState left, ManagedFlowState right) {
        if (left._values == null) return true;
        if (right._values == null) return false;
...
internal sealed class ManagedKeyComparer : IEqualityComparer<object> {
    ...
    public new bool Equals(object? left, object? right) =>
        left is IOperation || right is IOperation
...
internal readonly record struct ManagedAbstractValue(
    ...
    internal static ManagedAbstractValue BooleanUnknown { get; } =
    ...
    internal static ManagedAbstractValue Boolean(bool value) =>
        new(IntervalValue.Constant(value ? 1 : 0), default, default, false, false, true);
    internal static ManagedAbstractValue Integer(IntervalValue value, bool excludesZero = false) =>
        value.IsBottom ? Bottom : new(value, default, default, excludesZero, false, false);
    internal static ManagedAbstractValue Reference(
        NullnessValue value, IntervalValue cardinality = default) =>
        value == NullnessValue.Bottom ? Bottom : new(default, value, cardinality, false, false, false);

    internal static ManagedAbstractValue TopForType(ITypeSymbol? type) {
        if (type?.SpecialType == SpecialType.System_Boolean) return BooleanUnknown;
    ...
    internal static ManagedAbstractValue DefaultForType(ITypeSymbol? type) {
        if (type?.SpecialType == SpecialType.System_Boolean) return Boolean(false);
        if (IntegerType(type, out _)) return Integer(IntervalValue.Constant(0));
    ...
    internal static ManagedAbstractValue FromConstant(object? value, ITypeSymbol? type) {
        ...
        if (value is string text) return Reference(NullnessValue.NonNull, IntervalValue.Constant(text.Length));
        try {
    ...
    internal static ManagedAbstractValue Binary(
        ...
        if (!left.TryGetInteger(out var leftInteger) || !right.TryGetInteger(out var rightInteger)) return unknown;
    ...
    internal static ManagedAbstractValue NegateBoolean(ManagedAbstractValue value) =>
        value.TryGetBoolean(out var boolean) ? Boolean(!boolean) : Unknown;

    internal static BinaryOperatorKind Reverse(BinaryOperatorKind @operator) => @operator switch {
    ...
    internal static BinaryOperatorKind ReverseComparison(BinaryOperatorKind @operator) => Reverse(@operator);

    internal static BinaryOperatorKind Negate(BinaryOperatorKind @operator) => @operator switch {
    ...
    internal static bool TryArithmetic(
        ...
        if (minimum < long.MinValue || maximum > long.MaxValue) return false;
    ...
    internal static ManagedAbstractValue KeepWithinType(IntervalValue value, ITypeSymbol? type) =>
        FitsType(value, type) ? Integer(value) : TopForType(type);

    internal static bool FitsType(IntervalValue value, ITypeSymbol? type) =>
        IntegerType(type, out var semantics) &&
    ...
    internal static ManagedAbstractValue Join(ManagedAbstractValue left, ManagedAbstractValue right) {
        ...
        if (!left.Scalar.IsBottom && !right.Scalar.IsBottom) {
            if (left.IsBoolean != right.IsBoolean) return Unknown;
        ...
        if (left.Nullness == NullnessValue.Bottom || right.Nullness == NullnessValue.Bottom) return Unknown;
    ...
    private static ManagedAbstractValue Equality(
        ...
        if (left.TryGetBoolean(out var leftBoolean) && right.TryGetBoolean(out var rightBoolean))
            ...
            else if (Disjoint(leftInteger, rightInteger)) equal = false;
        }
        else if (left.TryGetNullness(out var leftNullness) && right.TryGetNullness(out var rightNullness)) {
            if (leftNullness == NullnessValue.Null && rightNullness == NullnessValue.Null) equal = true;
    ...
    private static bool Disjoint(IntervalValue left, IntervalValue right) =>
        left.UpperBound.HasValue && right.LowerBound.HasValue && left.UpperBound.Value < right.LowerBound.Value ||
...
internal sealed class DefiniteOperationFacts(Compilation compilation, CancellationToken cancellationToken) {
    ...
    private bool CompletesNormally(IMethodSymbol method) {
        ...
        if (normalized.DeclaringSyntaxReferences.Length != 1 || !_activeMethods.Add(normalized)) return false;
        try {
    ...
    private bool ChildrenCompleteNormally(IOperation operation) =>
        operation.ChildOperations.All(CompletesNormally);
    ...
    internal static bool IsHarmlessValue(IOperation operation) => operation switch {
        ILiteralOperation or ILocalReferenceOperation or IParameterReferenceOperation or
    ...
    internal static IOperation UnwrapHarmlessValue(IOperation operation) => operation switch {
        IConversionOperation conversion when HarmlessConversion(conversion) =>
