# SharpProof v2 release gates

`SharpProof.V2Gates` is a deterministic console gate:

```text
dotnet run --project SharpProof.V2Gates -- corpus
dotnet run --project SharpProof.V2Gates -- corpus-update
dotnet run --project SharpProof.V2Gates -- performance
dotnet run --project SharpProof.V2Gates -- all
```
...
`eng/acceptance/v2/contract.json`. It refuses to run if the release protocol
