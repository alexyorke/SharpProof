...
- `PS0002` evidence now includes low-confidence BCL fallback guess properties for otherwise unknown metadata BCL methods, constructors, properties, and fields; `PS0012` can surface the same guess when explanation diagnostics or dedicated BCL fallback reporting are enabled.
