...
public sealed class EffectAnalysisTests
{
    ...
    [Test]
    public void ImplicitCollectionCreationDoesNotHideElementEffects()
    {
        var compilation = EffectTestHost.CreateCompilation(
            """
            ...
            public static class Sample {
                ...
                public static List<int> Build() => [Touch(), 2];
            }
            """);

        var result = new EffectAnalysisSession(compilation).Analyze(
            Method(compilation, "Build"));

        using (Assert.EnterMultipleScope())
        {
            Assert.That(result.Summary.Writes.Contains(
    ...
    [Test]
    public void PrimaryConstructorParameterAssignmentWritesReceiverState()
    {
        var compilation = EffectTestHost.CreateCompilation(
            """
            public sealed class Sample(int Value) {
                public void Reset() => Value = 0;
                public static void Ordinary(int value) => value = 0;
            }
            """);

        var reset = new EffectAnalysisSession(compilation).Analyze(
            Method(compilation, "Reset"));
        var ordinary = new EffectAnalysisSession(compilation).Analyze(
            Method(compilation, "Ordinary"));

        using (Assert.EnterMultipleScope())
        {
            Assert.That(
                reset.Summary.Writes.Contains(EffectRegionId.Receiver),
                Is.True);
            Assert.That(ordinary.Summary.Writes.IsEmpty, Is.True);
        }
    }
