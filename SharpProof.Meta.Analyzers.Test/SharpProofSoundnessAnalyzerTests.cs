...
public sealed class SharpProofSoundnessAnalyzerTests
{
    ...
    public async Task SemanticCacheWritesTrackRefStorageAliases()
    {
        var diagnostics = await Analyze(
            ...
            enum Answer { Unknown, Proven }
            sealed class ProofCache : ISemanticCache {
                private Answer _latest;
                internal ref Answer Latest => ref _latest;
            }
            sealed class C {
                static void AssignUnknown(out Answer answer) =>
                    answer = Answer.Unknown;
                void M(ProofCache cache) {
                    ref var slot = ref cache.Latest;
                    slot = Answer.Unknown;
                    AssignUnknown(out slot);
                }
            }
            """);
    ...
    [Test]
    public async Task SemanticCacheTryUpdateInspectsOnlyNewValue()
    {
        var diagnostics = await Analyze(
            """
            namespace SharpProof.Verify;
            ...
            sealed class ProofCache : ISemanticCache {
                internal bool TryUpdate(
                    string key,
                    Answer newValue,
                    Answer comparisonValue) => true;
            }
            sealed class C {
                void M(ProofCache cache) {
                    cache.TryUpdate("safe", Answer.Proven, Answer.Unknown);
                    cache.TryUpdate("unsafe", Answer.Unknown, Answer.Proven);
                }
            }
            """);
    ...
    [Test]
    public async Task RejectsGetOnlyMutableStaticReferenceProperties()
    {
        var diagnostics = await Analyze(
            """
            using System.Collections.Generic;
            ...
            namespace SharpProof.Analyzer;
            static class C {
                internal static List<int> Values { get; } = new();
                internal static int[] Array { get; } = new int[1];
                internal static int Number { get; } = 1;
                internal static ImmutableArray<int> Immutable { get; } = ImmutableArray<int>.Empty;
            }
            """);
        ...
        using (Assert.EnterMultipleScope())
        {
            ...
            Assert.That(
                mutableState.Select(static diagnostic =>
                    diagnostic.GetMessage(CultureInfo.InvariantCulture)),
                Has.Some.Contains("Values"));
            Assert.That(
                mutableState.Select(static diagnostic =>
                    diagnostic.GetMessage(CultureInfo.InvariantCulture)),
                Has.Some.Contains("Array"));
        }
    }
