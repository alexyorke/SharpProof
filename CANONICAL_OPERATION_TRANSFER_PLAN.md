...
- [x] Migrate source queries, invariant/reachability analysis, exception paths,
  ...
  - [x] Consolidate containing-block entry transfer. One parent-shape router now
    ...
    conservative.
  - [x] Replace the completed-if/switch/loop/lock structural policy as one
    ...
    - [x] Add a seeded statement-region CFG traversal with operation cursors,
...
- [ ] Delete `SymbolicProgramPointFacts`, the statement/expression/assignment,
  ...
  - [x] Make CFG operation transfer the sole assignment-completion owner for
    declarations, expression statements, simple/coalesce/deconstruction
  ...
  - [x] Delete the generic `IOperation` assignment-lowering facade after exact
    search proved its only three callers were tests. Move those fixtures to the
    production assignment adapter and delete the facade's two orphaned helpers.
  - [x] Move ordered throw-guard, framework, `DoesNotReturnIf`, and source
    successful-completion sequencing into `SymbolicSourceCompletionLowerer`.
  ...
  - [x] Adjudicate completed `using` as a standalone cut. Its unique owner is
    only 24 physical / 20 nonblank lines; a safe replacement must additionally
    model resource flow captures or assignments, initializer completion facts,
  ...
  - [x] Quantify the execution-root trace deletion closure before extending CFG
    modes. All structural fallback modes total 696 physical / 650 nonblank lines,
    or 808 / 754 with completion/prior semantics. No bounded target mode removes
    more than 14 lines because the remaining modes keep the shared closure live;
    the first economical cut must replace a complete family, not another adapter.
  - [x] Route default-limit, seedless, acyclic before-current queries through one
    cached immutable execution-root trace. The trace reuses the canonical CFG
    worklist, transfer, branch, and merge owners; rebases method-entry evidence at
    each query site; isolates roots and concurrent callers; and falls back
    atomically for loops, exception/protocol regions, unsupported operations,
    cancellation, custom limits, or truncation. Isolated limit scopes prevent
    whole-root probing from leaking truncation events into point queries, and
    nondefault requests bypass the cache whose key intentionally omits limits.
  - [x] Delete the superseded 54-line invalidated-if completion summary. Canonical
    guard invalidation plus completed-path merging now produces the same normalized
    state, evidence, and symbol versions across 36 direct completion cases, the
    236-case collector fixture, source-query/limit/JSON coverage, and all lanes.
    Three broader prototypes proved the adjacent owners remain semantic: raw loop
    exit merging broke six conservative/exact source-query outcomes, removing
    abrupt-if completion broke finally and guarded-hazard diagnostics, and removing
    terminal-switch exclusions made one nested branch spuriously reachable. Those
    owners remain explicit inputs to the seeded-trace rewrite rather than presumed
    duplicates.
  - [x] Extend the trace with seeded completed-statement exit observations for
    if/switch/while/do/for and delete completed-statement scheduling from the
    generic point collector. Preserve arbitrary entry states, terminal/abrupt
    flow, evidence, versions, scope exit, typed fallback, and try/finally routing.
    Implementation disproved the prior 873-gross/250-replacement estimate: 421
    nonblank lines are distinct abrupt-if, switch-exclusion, loop-summary,
    terminal-collapse, and scope-exit semantics, while roughly 300 lines are the
    now-canonical safe statement-slice topology. Removing either group recreated
    already reproduced proof, exception/finally, and reachability defects. The
    duplicated generic scheduler is gone; the live trace inputs remain explicit.
    - [x] Route if/switch/while/do/for completion through one uncached seeded-trace
      ...
      passing cases. This characterization landed before the direct worklist in
      the following substep.
    - [x] Execute the seeded statement slice in `SymbolicCfgExecutionTrace` and
      remove completed-statement setup, operation slicing, terminal collection,
      loop finalization, and switch finalization from the generic point collector.
      The trace reuses the canonical operation, branch, loop-edge, merge, scope,
      and finally-continuation helpers. Collector tests pass 241/241, focused
    ...
    - [x] Consolidate trace topology around typed block slices, one materialized
      operation inventory, one connector closure, and topology-derived edge
      classification. Multilabel switch, nested switch, nested abrupt-if, and
      externally owned finally shapes retain their exact typed fallback details.
      The rewrite removes 22 net production lines, below its projected 105-line
  ...
  - [x] Collapse `PurityAnalysisStateMerger` to a thin ordered metadata merge
    around the canonical all-path `SymbolicStateMerger`. One common-key helper
    now owns delegate, flow-target, and flow-symbol intersection; one ordered
    pass owns impurity evidence and capture union. Empty, singleton, pair, and
    three-state characterization locks concrete impurity precedence, sticky
    captured impurity, unresolved target absorption, symbol equality, map
...
- [x] Gate: normalized states, proof outcomes, hazards, diagnostics/evidence,
...
- [x] Public API, CLI, JSON/SARIF, diagnostics/evidence, seeded fuzz, NuGet,
  VSIX, native assets, and package-consumer checks pass.
- [ ] No migrated legacy semantic path remains reachable.
- [x] Two final audits find no remaining high- or medium-severity parallel
...
| Phase 7 canonical purity metadata merge | This commit | 104,975 | -2,701 |
...
| Phase 7 canonical purity metadata merge | `PurityAnalysisStateMerger` now performs one ordered impurity/capture pass and one generic common-key intersection around a single canonical all-path state merge. Three direct tests characterize empty/singleton, pair, and three-state metadata/path behavior; the affected CFG/operation/delegate/alias/evidence batch passes 492/492; MainSmtAnalyzer passes 487/487; and all main lanes pass 5,554 plus the same two MainGeneral skips. The Release solution warning-as-error build has zero warnings and errors, and architecture violation buckets remain zero. Production LOC falls by 102 to 104,975 across 444 files, or -2,701 from the rewrite start; tracked test LOC is 145,672. |
...
- Last confirmed fact: all 8,583 lines in the legacy deletion-map family are
  ...
  fallback with no partial CFG state. At that checkpoint, assignment completion
  remained unchanged.
  ...
  The combined completed-statement CFG owner is now landed locally. It owns
  completed if/switch/loop/lock policy plus foreach protocol completion, and the
  637-line control-flow completion owner and completion-specific branch regions
  are deleted. Switch exclusions and abrupt-if completion were corrected from
  full-lane reproductions before the legacy paths were removed. The focused
  batch passes 339/339; all six lanes pass 6,101 tests with the same two
  documented skips; and the Release warning-as-error solution build is clean.
  Production LOC is 106,279, or -1,397 from the rewrite start; authoritative
  tracked test LOC is 144,658. GitHub PR #79 remains green at 8/8 checks; no CI
  edit was necessary because commit `1ebaebea` already fixed the README race.
  Assignment completion now also enters through the CFG operation owner.
  Expression and computed-update semantic owners are deleted; unsupported
  assignment values retain conservative invalidation and independent normal-
  completion facts. The direct collector still returns typed `Unsupported`
  when exact value transfer is unavailable. The focused differential passes
  ...
  The residual unsupported-transition helper clone is evidence-backed
  intentional because its two lowerers own different provenance roots. The
  generic operation-lowering facade and its two orphaned helpers are deleted
  after exact search proved they had only three test callers; those fixtures now
  exercise the production assignment adapter and pass 44/44. The second final
  semantic-owner audit found no high/medium parallel implementation. Production
  LOC is 105,982, or -1,694 from the rewrite start; tracked test LOC is 144,700.
  Normal-completion sequencing now has one canonical coordinator. Every CFG,
  loop, statement, using, and declaration caller consumes its immutable
  transition directly, and the legacy 127-line owner plus loop wrapper are
  deleted. The focused batch passes 330/330, all six lanes pass 6,102 tests with
  ...
  The generated test-impact inventory is current in `983c151d`. Root/nested
  characterization now locks symbol versions and complete typed fallback
  identity. A direct reuse of completed-statement region finalization was
  rejected after 31/53 focused failures and reverted; block completion requires
  an explicit finalization policy before either structural route can be removed.
  ...
  thrown-type exclusion with proven exact runtime identity. Production LOC was
  106,118, or -1,558 from the rewrite start; all six lanes passed 6,118 tests with
  ...
  substantially wider semantic surface. CI/CD remained green and current; no
  patch was warranted. The first cached execution-root trace now serves
  default-limit, seedless, acyclic before-current queries while preserving exact
  normalized state, evidence, versions, conservative fallback, cancellation,
  concurrency, and truncation isolation. The canonical worklist also subsumes the
  old invalidated-if completion summary, deleting 54 lines. Adjacent loop,
  abrupt-if, and terminal-switch owners were each removed experimentally and
  restored after broader gates proved distinct proof, exception-flow, and
  ...
  tracked test LOC to 145,215. Compact and invariant roots now serialize as
  ...
  The compact invariant-query view and its three query-only leaf carriers are
  now one ordered raw JSON document plus only the metrics required by explain,
  exit gates, and aggregate summaries. The 15 byte hashes, 96 source-query
  tests, canonical 606-case Tooling lane, package-byte check, and Release
  warning-as-error solution build are green. Production LOC is 105,458 across
  444 files and tracked test LOC is 145,267.
  The compact scope carrier and its linked test-project sources are now deleted;
  one private builder feeds the root and line raw documents without retaining an
  equivalent carrier. The 15 byte hashes, 96 source-query tests, 39 impacted-test
  inventory cases, package-preconditioned 606-case Tooling lane, and Release
  ...
  Purity metadata merging now has one 93-line coordinator around canonical
  all-path state merging. Three direct cases, the 492-case affected batch,
  MainSmtAnalyzer 487/487, and all main lanes at 5,554 passes plus the same two
  skips are green. Release warning-as-error is clean. Production LOC is 104,975
