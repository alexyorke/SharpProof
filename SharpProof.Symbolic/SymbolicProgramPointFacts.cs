...
internal static partial class SymbolicProgramPointFacts
{
    ...
    private static IEnumerable<(ISymbol Symbol, SmtFormula Bound, IReadOnlyList<ISymbol> BoundSymbols)>
        EnumerateForLoopInitializerBounds(
            ForStatementSyntax forStatement,
            SemanticModel semanticModel,
            CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var initializer in EnumerateForLoopStrictUpperBoundInitializers(forStatement, semanticModel,
                     cancellationToken))
        {
            if (!TryCreateSymbolSmtValue(initializer.Symbol, out var symbolFormula) ||
                symbolFormula.Kind != SmtValueKind.Int ||
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var initializer in EnumeratePreLoopStrictUpperBoundInitializers(loopStatement, semanticModel,
                     cancellationToken))
        {
            if (!TryCreateSymbolSmtValue(initializer.Symbol, out var symbolFormula) ||
                symbolFormula.Kind != SmtValueKind.Int ||
    ...
    private static IEnumerable<(ISymbol Symbol, SmtFormula Bound, IReadOnlyList<ISymbol> BoundSymbols)>
        EnumeratePreLoopInitializerBounds(
            StatementSyntax loopStatement,
            SemanticModel semanticModel,
            CancellationToken cancellationToken)
    {
    ...
    private static IEnumerable<(ISymbol Symbol, SmtFormula UpperBound, IReadOnlyList<ISymbol> BoundSymbols)>
        EnumeratePreLoopStrictUpperBoundInitializers(
            StatementSyntax loopStatement,
            SemanticModel semanticModel,
            CancellationToken cancellationToken)
    {
    ...
    private static IEnumerable<(ISymbol Symbol, SmtFormula UpperBound, IReadOnlyList<ISymbol> BoundSymbols)>
        EnumerateForLoopStrictUpperBoundInitializers(
            ForStatementSyntax forStatement,
            SemanticModel semanticModel,
            CancellationToken cancellationToken)
    {
        ...
        List<SmtFormula> facts)
    {
        ...
        if (entryFacts.Length == 0) return Array.Empty<SmtFormula>();
        ...
        IReadOnlyList<SmtFormula> entryFacts)
    {
        ...
        {
            if (!existingKeys.Add(GetFormulaKey(entryFact))) continue;
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            case IfStatementSyntax ifStatement when ReferenceEquals(ifStatement.Statement, block):
                AddBranchConditionFacts(ifStatement.Condition, true, semanticModel, cancellationToken, builder);
                break;
            case ElseClauseSyntax { Parent: IfStatementSyntax ifStatement, Statement: var statement }
                when ReferenceEquals(statement, block):
                AddBranchConditionFacts(ifStatement.Condition, false, semanticModel, cancellationToken, builder);
                break;
            case WhileStatementSyntax whileStatement when ReferenceEquals(whileStatement.Statement, block):
                AddBranchConditionFacts(whileStatement.Condition, true, semanticModel, cancellationToken, builder);
            ...
            case ForStatementSyntax forStatement when ReferenceEquals(forStatement.Statement, block):
                if (forStatement.Condition != null)
                    AddBranchConditionFacts(forStatement.Condition, true, semanticModel, cancellationToken, builder);
            ...
            case ForEachStatementSyntax forEachStatement when ReferenceEquals(forEachStatement.Statement, block):
                AddForeachBodyEntryFacts(
                    ...
                    semanticModel.GetDeclaredSymbol(forEachStatement, cancellationToken),
                    forEachStatement,
    ...
    {
        if (temporaryFacts.Count == 0) return;
        ...
        List<SmtFormula> facts)
    {
            ...
            expressionStatement.Expression is AssignmentExpressionSyntax assignment)
        {
            ...
            RemoveFactsInvalidatedByMutationTarget(assignment.Left, semanticModel, cancellationToken, facts);
            ...
            if (assignedSymbol is ILocalSymbol or IParameterSymbol)
            {
                ...
                var coalesceAssignmentIsKnownNullableNoValue =
                    assignment.IsKind(SyntaxKind.CoalesceAssignmentExpression) &&
                    IsKnownNullableNoValueSymbol(facts, originalAssignedSymbol);
                if (!coalesceAssignmentIsKnownNoOp) RemoveFactsReferencingSymbol(facts, originalAssignedSymbol);

                if (assignment.IsKind(SyntaxKind.SimpleAssignmentExpression))
                {
                    AddAssignedValueFacts(originalAssignedSymbol, assignment.Right, semanticModel, cancellationToken,
                        facts);
                }
                else if (assignment.IsKind(SyntaxKind.CoalesceAssignmentExpression))
                {
                    if (!coalesceAssignmentIsKnownNoOp)
                    {
                        if (coalesceAssignmentIsKnownNullableNoValue)
                            AddAssignedValueFacts(originalAssignedSymbol, assignment.Right, semanticModel,
                                cancellationToken, facts);
                        else
                            AddCoalesceAssignmentFacts(originalAssignedSymbol, assignment.Right, previousAssignedValue,
                                semanticModel, cancellationToken, facts);
                    }
                }
                         ...
                         SymbolicReachabilityService.TryCreateCompoundAssignmentFact(
                             ...
                             semanticModel,
                             cancellationToken,
                             ExpressionReferencesSymbol(assignment.Right, originalAssignedSymbol, semanticModel,
                                 cancellationToken),
            ...
            TryGetCurrentSymbolValue(facts, mutatedSymbol, out var previousMutatedValue))
        {
            ...
            if (SymbolicReachabilityService.TryCreateIncrementOrDecrementFact(mutatedSymbol, previousMutatedValue,
                    delta, out var mutationFact)) facts.Add(mutationFact);
        ...
        if (statement is BlockSyntax block)
            ...
            AddCompletedSwitchStatementFacts(switchStatement, factsBeforeStatement, semanticModel, cancellationToken,
                facts);
    ...
    private static void AddCompletedScopedBlockFacts(
        ...
        ICollection<SmtFormula> facts)
    {
        if (StatementDefinitelyExits(block, semanticModel, cancellationToken)) return;
        ...
        foreach (var statement in block.Statements)
        {
            ...
            if (StatementDefinitelyExits(statement, semanticModel, cancellationToken)) break;
        }
    ...
    private static void AddCompletedCatchBranch(
        ...
        CancellationToken cancellationToken)
    {
        if (StatementDefinitelyExits(catchClause.Block, semanticModel, cancellationToken)) return;
    ...
    private static void AddCompletedUsingStatementFacts(
        ...
        ICollection<SmtFormula> facts)
    {
        if (StatementDefinitelyExits(usingStatement.Statement, semanticModel, cancellationToken)) return;
        ...
        ICollection<SmtFormula> facts)
    {
        var visibleBranchFacts = completedBranches
            .Select(branch =>
                FilterVisibleBranchFacts(branch.Facts, branch.Statement, semanticModel, cancellationToken))
            .ToArray();
        if (visibleBranchFacts.Length == 0) return;
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        AddTopLevelNotNullParameterNormalCompletionFacts(expression, statement, semanticModel, cancellationToken,
            facts);
    ...
    {
        var key = GetFormulaKey(fact);
        if (facts.Any(existing => string.Equals(GetFormulaKey(existing), key, StringComparison.Ordinal))) return;
        ...
        ICollection<SmtFormula> facts)
    {
        if (StatementDefinitelyExits(ifStatement.Statement, semanticModel, cancellationToken) &&
            (ifStatement.Else?.Statement == null ||
             !AnyConditionSymbolInvalidatedInStatement(ifStatement.Condition, ifStatement.Else.Statement, semanticModel,
                 cancellationToken)))
            AddBranchConditionFacts(
                ifStatement.Condition,
                false,
        ...
        if (ifStatement.Else?.Statement is { } elseStatement &&
            ...
            !AnyConditionSymbolInvalidatedInStatement(ifStatement.Condition, ifStatement.Statement, semanticModel,
                cancellationToken))
            AddBranchConditionFacts(
                ifStatement.Condition,
                true,
        ...
        AddCompletedIfImplicitElseMergedFacts(ifStatement, factsBeforeStatement, semanticModel, cancellationToken,
            facts);
    }
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        var falseBranchExits = falseBranch != null &&
                               StatementDefinitelyExits(falseBranch, semanticModel, cancellationToken);

        if (trueBranchExits &&
            falseBranch is { } survivingFalseBranch &&
            !falseBranchExits &&
            !AnyConditionSymbolInvalidatedInStatement(ifStatement.Condition, survivingFalseBranch, semanticModel,
                cancellationToken))
        {
    ...
    {
        var existingKeys = new HashSet<string>(facts.Select(GetFormulaKey), StringComparer.Ordinal);
        foreach (var branchFact in FilterVisibleBranchFacts(branchFacts, survivingBranch, semanticModel,
                     cancellationToken))
        {
            var key = GetFormulaKey(branchFact);
            if (existingKeys.Add(key)) facts.Add(branchFact);
        }
    }
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        AddBranchConditionFacts(ifStatement.Condition, false, semanticModel, cancellationToken, falseBranchFacts);

        AddIdenticalBranchFacts(trueBranchFacts, falseBranchFacts, facts);

        if (AnyConditionSymbolInvalidatedInStatement(ifStatement.Condition, ifStatement.Statement, semanticModel,
                cancellationToken) ||
            !TryCreateBranchConditionFormula(ifStatement.Condition, true, semanticModel, cancellationToken,
                out var trueCondition) ||
            !TryCreateBranchConditionFormula(ifStatement.Condition, false, semanticModel, cancellationToken,
                out var falseCondition))
            return;
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        if (AnyConditionSymbolInvalidatedInStatement(ifStatement.Condition, ifStatement.Statement, semanticModel,
                cancellationToken) ||
            AnyConditionSymbolInvalidatedInStatement(ifStatement.Condition, elseStatement, semanticModel,
                cancellationToken) ||
            !TryCreateBranchConditionFormula(ifStatement.Condition, true, semanticModel, cancellationToken,
                out var trueCondition) ||
            !TryCreateBranchConditionFormula(ifStatement.Condition, false, semanticModel, cancellationToken,
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        {
            var key = GetFormulaKey(fact);
            if (existingKeys.Contains(key) || !falseBranchKeys.Contains(key)) continue;
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        foreach (var trueFact in trueBranchFacts.Where(fact => !currentKeys.Contains(GetFormulaKey(fact))))
        {
            ...
            {
                if (string.Equals(trueMergeableFact.FactKey, falseMergeableFact.FactKey,
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        foreach (var section in switchStatement.Sections)
        {
            ...
            if (!sectionMutatesConditionSymbols) sectionFacts.Add(sectionCondition);
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        if (branchFactsByTarget.Length == 0) return;
        ...
        foreach (var target in candidateTargets)
        {
            var factChoices = branchFactsByTarget
                ...
                .ToArray();
            if (factChoices.Select(static fact => fact.FactKey).Distinct(StringComparer.Ordinal).Count() == 1) continue;
        ...
        out SmtFormula breakCondition)
    {
        ...
        if (ifStatement == null ||
            ...
            AnyConditionSymbolInvalidatedBeforeStatement(ifStatement.Condition, loopBody, ifStatement.SpanStart,
                semanticModel, cancellationToken) ||
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (catchClause.Filter?.FilterExpression is { } filterExpression &&
            !AnyReferencedSymbolAssignedBeforeUse(
                ...
                semanticModel,
                cancellationToken))
            AddBranchConditionFacts(filterExpression, true, semanticModel, cancellationToken, facts);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var fact in declarationFacts) facts.Add(fact);
    }
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        AddReferenceNullCondition(facts, expressionSyntax, false, semanticModel, cancellationToken);
    }
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        AddReferenceNullCondition(facts, lockStatement.Expression, false, semanticModel, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        AddReferenceNullCondition(facts, expressionSyntax, false, semanticModel, cancellationToken);
        AddFiniteForeachIterationFact(facts, expressionSyntax, iterationSymbol, foreachStatement, semanticModel,
            cancellationToken);
    ...
    private static void AddFiniteForeachIterationFact(
        ...
        CancellationToken cancellationToken)
    {
        if (iterationSymbol == null) return;
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var elementExpression in elementExpressions)
        {
            if (ExpressionReferencesSymbol(elementExpression, iterationSymbol.OriginalDefinition, semanticModel,
                    cancellationToken)) return;
            ...
            else if (!allReferenceElementsDefinitelyNonNull) return;
        ...
        if (finiteDomainFact != null) facts.Add(finiteDomainFact);
        ...
        ImmutableArray<SmtFormula> elementValueFormulas)
    {
        ...
        {
            if (!TryCreateAssignedValueFact(iterationSymbol.OriginalDefinition, elementValueFormula,
                    out var elementValueFact)) return;
        ...
        out ImmutableArray<SmtFormula> valueFormulas)
    {
        ...
        if (semanticModel.GetSymbolInfo(UnwrapExpression(expressionSyntax), cancellationToken).Symbol
                ?.OriginalDefinition is not { } receiverSymbol ||
            receiverSymbol is not ILocalSymbol and not IParameterSymbol ||
        ...
        out int elementCount)
    {
        ...
        {
            var statement = containingBlock.Statements[index];
            if (statement.SpanStart >= containingStatement.SpanStart) continue;
                     ...
                     !CSharpSyntaxFacts.IsNestedCallableBoundary(candidate)))
        {
            ...
            if (mutatedExpression != null)
            {
                ...
                foreach (var receiverSymbol in GetMutatedReceiverSymbols(mutatedExpression, semanticModel,
                             cancellationToken)) RemoveFactsReferencingSymbol(facts, receiverSymbol);
            }
    ...
    private static void RemoveFactsInvalidatedByMutationTarget(
        ...
        List<SmtFormula> facts)
    {
        ...
        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken) &&
            TryCreateAssignedValueFact(assignedSymbol, effectiveValueExpression, semanticModel, cancellationToken,
                facts, out var fact))
            facts.Add(fact);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken) &&
            TryCreateAssignedValueNonZeroFact(assignedSymbol, effectiveValueExpression, semanticModel,
                cancellationToken, facts, out var nonZeroFact))
            facts.Add(nonZeroFact);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken))
            AddSwitchExpressionAssignedValueFacts(assignedSymbol, effectiveValueExpression, semanticModel,
                cancellationToken, facts);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken))
            AddMathAbsRemainderAssignedRangeFacts(assignedSymbol, effectiveValueExpression, semanticModel,
                cancellationToken, facts);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken))
            AddAssignedSourceSymbolSnapshotFacts(assignedSymbol, effectiveValueExpression, semanticModel,
                cancellationToken, facts);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken) &&
            SymbolicReachabilityService.TryCreateStringContentAssignedValueFact(assignedSymbol,
                effectiveValueExpression, semanticModel, cancellationToken, out var stringContentFact))
        ...
        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken))
            AddNotNullIfNotNullAssignedNullStateFacts(assignedSymbol, effectiveValueExpression, semanticModel,
                cancellationToken, facts);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken) &&
            SymbolicReachabilityService.TryCreateBuiltInLengthAssignedValueFact(assignedSymbol,
                effectiveValueExpression, semanticModel, cancellationToken, out var lengthFact))
            facts.Add(lengthFact);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken) &&
            SymbolicReachabilityService.TryCreateReferenceBackedLengthFact(assignedSymbol, effectiveValueExpression,
                semanticModel, cancellationToken, out var referenceLengthFact))
            AddUniqueFact(facts, referenceLengthFact);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken) &&
            SymbolicReachabilityService.TryCreateCollectionExpressionLengthLowerBoundFact(assignedSymbol,
                effectiveValueExpression, out var lowerBoundLengthFact))
            AddUniqueFact(facts, lowerBoundLengthFact);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken))
            SymbolicReachabilityService.AddArrayDimensionLengthAssignedValueFacts(assignedSymbol,
                effectiveValueExpression, semanticModel, cancellationToken, facts);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken))
            SymbolicReachabilityService.AddReferenceBackedArrayDimensionLengthFacts(assignedSymbol,
                effectiveValueExpression, semanticModel, cancellationToken, facts);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken))
            AddFiniteArrayElementAssignedValueFacts(assignedSymbol, effectiveValueExpression, semanticModel,
                cancellationToken, facts);
        ...
        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken) &&
            SymbolicReachabilityService.TryCreateReferenceBackedStringContentFact(assignedSymbol,
                effectiveValueExpression, semanticModel, cancellationToken, out var referenceStringFact))
            AddUniqueFact(facts, referenceStringFact);

        if (!ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel, cancellationToken))
        {
            AddStructuralReferenceNullStateAssignedValueFacts(assignedSymbol, effectiveValueExpression, semanticModel,
                cancellationToken, facts);
            SymbolicReachabilityService.AddNullableAssignedValueFacts(assignedSymbol, effectiveValueExpression,
                semanticModel, cancellationToken, facts);
            AddAsExpressionAssignedValueFacts(assignedSymbol, effectiveValueExpression, semanticModel,
                cancellationToken, facts);
            AddConditionalAccessAssignedValueFacts(assignedSymbol, effectiveValueExpression, semanticModel,
                cancellationToken, facts);
        }

        AddTupleElementAssignedValueFacts(assignedSymbol, effectiveValueExpression, semanticModel, cancellationToken,
            facts);
        ...
        if (hasThrowGuard &&
            ...
            AddBranchConditionFacts(
                ...
                facts);
                 ...
                 !ExpressionReferencesSymbol(effectiveValueExpression, assignedSymbol, semanticModel,
                     cancellationToken))
            AddReferenceNonNullFact(effectiveValueExpression, semanticModel, cancellationToken, facts);
    }
    ...
    private static void AddTupleElementSourceSymbolSnapshotFacts(
        ...
        List<SmtFormula> facts)
    {
        if (!TryGetTupleElementStorageNames(assignedSymbol, 0, out var targetElementNames) ||
            !TryGetTupleElementStorageNames(sourceSymbol, targetElementNames.Length, out var sourceElementNames))
            return;
        ...
        List<SmtFormula> facts)
    {
        ...
        for (var dimension = 0; dimension < sourceArrayType.Rank; dimension++)
            if (TryCreateTupleElementArrayDimensionLengthFormula(sourceTupleSymbol, sourceElementName, dimension,
                    out var sourceDimensionLength) &&
                TryCreateTupleElementArrayDimensionLengthFormula(targetTupleSymbol, targetElementName, dimension,
                    out var targetDimensionLength))
        ...
        SmtFormula targetFormula)
    {
        ...
        foreach (var fact in facts.ToArray())
        {
            ...
            if (existingKeys.Add(key)) facts.Add(substituted);
        }
    }
        ...
        out SmtFormula substituted)
    {
        ...
        switch (formula)
        {
            ...
            case SmtIntegerBinaryTerm integerBinary:
                var integerLeftChanged = TrySubstituteFormula(integerBinary.Left, sourceFormula, targetFormula,
                    out var integerLeft);
                var integerRightChanged = TrySubstituteFormula(integerBinary.Right, sourceFormula, targetFormula,
                    out var integerRight);
            ...
            case SmtStringConcatTerm stringConcat:
                var stringConcatLeftChanged = TrySubstituteFormula(stringConcat.Left, sourceFormula, targetFormula,
                    out var stringConcatLeft);
                var stringConcatRightChanged = TrySubstituteFormula(stringConcat.Right, sourceFormula, targetFormula,
                    out var stringConcatRight);
            ...
            case SmtStringContainsFormula stringContains:
                var stringContainsValueChanged = TrySubstituteFormula(stringContains.Value, sourceFormula,
                    targetFormula, out var stringContainsValue);
                var stringContainsSearchChanged = TrySubstituteFormula(stringContains.Search, sourceFormula,
                    targetFormula, out var stringContainsSearch);
            ...
            case SmtStringStartsWithFormula stringStartsWith:
                var stringStartsWithValueChanged = TrySubstituteFormula(stringStartsWith.Value, sourceFormula,
                    targetFormula, out var stringStartsWithValue);
                var stringStartsWithPrefixChanged = TrySubstituteFormula(stringStartsWith.Prefix, sourceFormula,
                    targetFormula, out var stringStartsWithPrefix);
            ...
            case SmtStringEndsWithFormula stringEndsWith:
                var stringEndsWithValueChanged = TrySubstituteFormula(stringEndsWith.Value, sourceFormula,
                    targetFormula, out var stringEndsWithValue);
                var stringEndsWithSuffixChanged = TrySubstituteFormula(stringEndsWith.Suffix, sourceFormula,
                    targetFormula, out var stringEndsWithSuffix);
            ...
            case SmtConditionalFormula conditional:
                var conditionChanged = TrySubstituteFormula(conditional.Condition, sourceFormula, targetFormula,
                    out var condition);
                var whenTrueChanged =
                    TrySubstituteFormula(conditional.WhenTrue, sourceFormula, targetFormula, out var whenTrue);
                var whenFalseChanged = TrySubstituteFormula(conditional.WhenFalse, sourceFormula, targetFormula,
                    out var whenFalse);
    ...
    private static bool TryCreateStringContentFormula(ISymbol symbol, out SmtFormula formula)
    {
        ...
        return SymbolicFactFactory.TryCreateStringContentFormula(SymbolicFactFactory.GetSmtVariableName(symbol), type,
            out formula);
    }
        ...
        out SmtFormula? formula)
    {
        ...
        if (containingStatement == null ||
            ...
            !TryGetFiniteElementIndex(
                ...
                cancellationToken,
                out var priorIndex) ||
            semanticModel.GetSymbolInfo(UnwrapExpression(elementAccess.Expression), cancellationToken).Symbol
                ?.OriginalDefinition is not { } receiverSymbol ||
            receiverSymbol is not ILocalSymbol and not IParameterSymbol ||
            SymbolicFactFactory.GetTrackedSymbolType(receiverSymbol) is not IArrayTypeSymbol { Rank: 1 } arrayType ||
        ...
        List<SmtFormula> facts)
    {
        ...
        for (var dimension = 0; dimension < arrayType.Rank; dimension++)
            if (TryCreateTupleElementArrayDimensionLengthFormula(tupleSymbol, elementName, dimension,
                    out var targetDimensionLength) &&
        ...
        List<SmtFormula> facts)
    {
        if (!TryCreateSymbolSmtValue(assignedSymbol, out var targetFormula) ||
            targetFormula is not { Kind: SmtValueKind.Reference })
        {
            AddNullableCoalesceAssignmentFacts(assignedSymbol, rightExpression, semanticModel, cancellationToken,
                facts);
            return;
        }
        ...
        AddCoalesceAssignmentRightNullImplication(targetFormula, rightExpression, semanticModel, cancellationToken,
            facts);
        ...
        List<SmtFormula> facts)
    {
        if (!TryCreateNullableHasValueFormula(assignedSymbol, out var targetHasValue) ||
            !TryGetNullableUnderlyingType(SymbolicFactFactory.GetTrackedSymbolType(assignedSymbol),
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        if (requiresNonNullValue &&
            !AnyConditionSymbolInvalidatedInStatement(effectiveValueExpression, statement, semanticModel,
                cancellationToken))
            AddReferenceNonNullFact(effectiveValueExpression, semanticModel, cancellationToken, facts);
    }
    ...
    private static void AddReferenceNonNullFact(
        ...
        ICollection<SmtFormula> facts)
    {
        AddReferenceNullCondition(facts, expression, false, semanticModel, cancellationToken);
    }
    ...
    private static bool TryCreateMemberSmtValue(SmtFormula receiverFormula, ISymbol memberSymbol,
        out SmtFormula formula)
    {
        ...
        List<SmtFormula> facts)
    {
        ...
        foreach (var fact in asFacts) AddUniqueFact(facts, fact);
    }
        ...
        List<SmtFormula> facts)
    {
        ...
        for (var index = 0; index < targetSymbols.Count; index++)
        {
            ...
            if (TryCreateAssignedValueFact(targetSymbols[index]!, valueFormulas[index], out var fact)) facts.Add(fact);
    ...
    private static void AddTupleElementDerivedTargetFacts(
        ...
        List<SmtFormula> facts)
    {
        if (TryCreateBuiltInLengthFormulaForReference(sourceFormula,
                SymbolicFactFactory.GetTrackedSymbolType(targetSymbol), out var sourceLength) &&
            TryCreateBuiltInLengthFormula(targetSymbol, out var targetLength))
            AddSubstitutedCurrentFacts(facts, sourceLength, targetLength);

        if (TryCreateStringContentFormulaForReference(sourceFormula,
                SymbolicFactFactory.GetTrackedSymbolType(targetSymbol), out var sourceString) &&
            TryCreateStringContentFormula(targetSymbol, out var targetString))
            AddSubstitutedCurrentFacts(facts, sourceString, targetString);

        if (SymbolicFactFactory.GetTrackedSymbolType(targetSymbol) is not IArrayTypeSymbol
            {
                Rank: > 1
            } arrayType) return;

        for (var dimension = 0; dimension < arrayType.Rank; dimension++)
            if (TryCreateArrayDimensionLengthFormulaForReference(sourceFormula, arrayType, dimension,
                    out var sourceDimensionLength) &&
    ...
    {
        valueFormulas = ImmutableArray<SmtFormula>.Empty;
        var receiverSymbol = semanticModel.GetSymbolInfo(UnwrapExpression(expression), cancellationToken).Symbol
            ?.OriginalDefinition;
    ...
    {
        value = null!;
        if (!TryCreateSymbolSmtValue(symbol, out var targetFormula)) return false;
    ...
    {
        hasValue = false;
        if (!TryCreateNullableHasValueFormula(symbol, out var targetHasValue)) return false;
    ...
    private static bool IsFormulaPair(SmtFormula expectedLeft, SmtFormula expectedRight, SmtFormula actualLeft,
        SmtFormula actualRight)
    {
    ...
    private static bool TryCreateBuiltInLengthFormula(ISymbol symbol, out SmtFormula formula)
    {
        ...
        return SymbolicFactFactory.TryCreateBuiltInLengthFormula(SymbolicFactFactory.GetSmtVariableName(symbol), type,
            out formula);
    }
    ...
    {
        output = null!;
        if (input.Kind != SmtValueKind.String) return false;
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        foreach (var branchFact in branchFacts)
        {
            if (facts.Contains(branchFact)) continue;
