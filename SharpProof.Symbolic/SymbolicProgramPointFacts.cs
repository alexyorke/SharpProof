...
namespace SharpProof.Symbolic
{
    internal static class SymbolicProgramPointFacts
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                AddReachabilityCondition(ref state, andCondition.Left, mustBeTrue: true, semanticModel, cancellationToken);
                AddReachabilityCondition(ref state, andCondition.Right, mustBeTrue: true, semanticModel, cancellationToken);
                return;
            }
            ...
            {
                AddReachabilityCondition(ref state, orCondition.Left, mustBeTrue: false, semanticModel, cancellationToken);
                AddReachabilityCondition(ref state, orCondition.Right, mustBeTrue: false, semanticModel, cancellationToken);
                return;
            }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (recursivePattern.PropertyPatternClause is not { Subpatterns.Count: > 0 })
            {
            ...
            out ITypeSymbol? memberType)
        {
            ...
            if (subpattern.NameColon?.Name is not ExpressionSyntax memberSyntax)
            {
                return false;
            }
            ...
            if (!TryCreateMemberDerivedTerm(receiver, memberSymbol, memberKind, out memberTerm))
            {
                return false;
            }
            ...
            out SymbolicTerm length)
        {
            ...
            var context = new SymbolicLoweringContext(semanticModel, cancellationToken);
            if (SymbolicIrLowerer.TryLowerBuiltInLengthTerm(expressionSyntax, context, out length))
            {
                return true;
            }
            ...
            string provenanceRoot)
        {
            ...
            {
                if (!TryCreateArrayDimensionLengthTerm(targetReference, arrayType, dimension, out var targetDimensionLength) ||
            ...
            ITypeSymbol? convertedType)
        {
            ...
            if (SymbolEqualityComparer.Default.Equals(sourceType, convertedType))
            {
                return false;
            }
        ...
        {
            return StateConditionProvesIntegerRelation(state, term, requireStrictlyPositive: true);
        }

        private static bool StateProvesNonNegativeInteger(SymbolicState state, SymbolicTerm term)
        {
            return StateConditionProvesIntegerRelation(state, term, requireStrictlyPositive: false);
        }
        ...
        {
            if (term.Kind != SmtValueKind.Int)
            {
                return false;
            }

            return state.PathConditions.Any(condition => ConditionProvesIntegerRelation(condition, term, requireStrictlyPositive)) ||
            ...
            bool requireStrictlyPositive)
        {
            return condition switch
            {
                SymbolicFactCondition factCondition => FactProvesIntegerRelation(factCondition.Fact, term, requireStrictlyPositive),
            ...
            bool termOnLeft)
        {
            return (termOnLeft, op) switch
            {
                ...
                (true, SymbolicRelationOperator.GreaterThanOrEqual) => requireStrictlyPositive ? constant > 0 : constant >= 0,
            ...
            string provenanceRoot)
        {
            ...
            state = state.AddPathCondition(new SymbolicBinaryCondition(SymbolicConditionOperator.Or, targetIsNull, sourceNonNull));
            state = state.AddPathCondition(new SymbolicBinaryCondition(SymbolicConditionOperator.Or, targetIsNull, runtimeTypeTest));
            ...
            SymbolicLoweringContext context)
        {
            ...
            {
                return constantValue.Value is string;
            }

            if (expression is CastExpressionSyntax castExpression &&
                context.SemanticModel.GetTypeInfo(castExpression.Type, context.CancellationToken).Type?.SpecialType == SpecialType.System_String)
            {
