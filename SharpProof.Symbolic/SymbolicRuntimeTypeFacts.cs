...
internal static class SymbolicRuntimeTypeFacts
{
        ...
        foreach (var (block, containingStatement) in EnumerateContainingBlocks(useNode).Reverse())
            foreach (var statement in block.Statements)
            {
                ...
                if (statement is LocalDeclarationStatementSyntax localDeclaration)
                {
                    ...
                    if (StatementMayMutateSymbol(statement, symbol, semanticModel, cancellationToken)) currentValue = null;
    ...
    private static bool TupleAssignmentMutatesSymbol(
        ...
        CancellationToken cancellationToken)
    {
        if (UnwrapRuntimeTypeExpression(assignment.Left) is not TupleExpressionSyntax leftTuple) return false;
