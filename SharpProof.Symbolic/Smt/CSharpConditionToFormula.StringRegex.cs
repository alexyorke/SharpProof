...
internal static partial class CSharpConditionToFormula
{
    ...
    {
        formula = null;
        if (inlineDepth >= MaxSourcePredicateInlineDepth) return false;
        ...
        int inlineDepth)
    {
        ...
        if (semanticModel.GetOperation(invocationExpression, cancellationToken) is not IInvocationOperation
            ...
            !TryGetLocalDelegateReceiver(invocationExpression, semanticModel, cancellationToken,
    ...
    {
        localSymbol = null!;
        var receiver = invocationExpression.Expression switch
        {
        ...
        out ExpressionSyntax initializer)
    {
        ...
        for (var index = declarationIndex + 1; index < invocationIndex; index++)
            if (CountLocalIdentifierReferences(declarationBlock.Statements[index], delegateLocal, semanticModel,
                    cancellationToken) != 0)
                return false;
    ...
    {
        formula = null;
        if (!TryGetStaticSourceBooleanMethodGroup(initializer, semanticModel, cancellationToken,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (allowedParameters.Count == 0) return false;

        foreach (var identifier in GetLambdaBodyNode(lambdaExpression)?.DescendantNodesAndSelf()
                     .OfType<IdentifierNameSyntax>() ?? Enumerable.Empty<IdentifierNameSyntax>())
        {
            var symbol = semanticModel.GetSymbolInfo(identifier, cancellationToken).Symbol;
            if (symbol == null ||
                (!allowedParameters.Contains(symbol) &&
                 !IsAllowedLambdaParameterMemberAccess(identifier, allowedParameters, semanticModel,
                     cancellationToken)))
                return false;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var receiverSymbol = semanticModel.GetSymbolInfo(UnwrapExpression(memberAccess.Expression), cancellationToken)
            .Symbol;
        ...
        int inlineDepth)
    {
        formula = null;
        if (semanticModel.GetOperation(invocationExpression, cancellationToken) is not IInvocationOperation
            invocationOperation) return false;
        ...
        int inlineDepth)
    {
        ...
        var isOrdinalIgnoreCase =
            IsSupportedOrdinalIgnoreCaseStringEqualsInvocation(invocationOperation, semanticModel, cancellationToken);
        if (!isOrdinal && !isOrdinalIgnoreCase) return false;
        ...
        if (method.IsStatic)
        {
            if (invocationOperation.Arguments.Length < 2 ||
                ...
                !TryTranslateStringValue(leftExpression, semanticModel, cancellationToken, out var left,
                    getSymbolVersion, inlineDepth) ||
                left == null ||
                !TryTranslateStringValue(rightExpression, semanticModel, cancellationToken, out var right,
                    getSymbolVersion, inlineDepth) ||
                right == null)
                return false;
        ...
        if (invocationOperation.Arguments.Length < 1 ||
            ...
            !TryTranslateStringValue(receiverExpression, semanticModel, cancellationToken, out var receiver,
                getSymbolVersion, inlineDepth) ||
            receiver == null ||
            !TryTranslateStringValue(argumentExpression, semanticModel, cancellationToken, out var argument,
                getSymbolVersion, inlineDepth) ||
            argument == null ||
    ...
    {
        var method = invocationOperation.TargetMethod;
        if (method.ContainingType?.SpecialType != SpecialType.System_String) return false;
        ...
        if (parameters.Length == 1) return IsStringParameter(parameters[0]);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (method.IsStatic)
            return parameters.Length == 3 &&
                   ...
                   IsStringParameter(parameters[1]) &&
                   IsStringComparisonParameter(parameters[2]) &&
                   HasOrdinalIgnoreCaseStringComparison(invocationOperation.Arguments, semanticModel,
                       cancellationToken);
        ...
        int inlineDepth)
    {
        ...
        else
        {
            ...
            if (leftConstant == null) return false;
        ...
        if (!TryCreateStringNonNullFormula(subjectExpression, semanticModel, cancellationToken, out var subjectNonNull,
                getSymbolVersion, inlineDepth) ||
            subjectNonNull == null)
            return false;
        ...
        int inlineDepth)
    {
        ...
        if (memberAccessExpression.Name.Identifier.ValueText != "Success" ||
            ...
            UnwrapExpression(memberAccessExpression.Expression) is not InvocationExpressionSyntax
                invocationExpression ||
        ...
        int inlineDepth)
    {
        ...
        if (method.IsStatic)
        {
            ...
            pattern = TryGetConstantString(invocationOperation.Arguments[1].Value.Syntax as ExpressionSyntax,
        ...
        else
        {
            ...
            if (requiresEncodableOptions && !CanEncodeRegexOptions(options)) return false;
        }

        if (inputExpression == null ||
            pattern == null ||
            !TryTranslateStringValue(inputExpression, semanticModel, cancellationToken, out var inputFormula,
                getSymbolVersion, inlineDepth) ||
            inputFormula == null)
            return false;
        ...
        out bool requiresEncodableOptions)
    {
        ...
        if (invocationOperation.Arguments.Length == 1) return true;

        if (invocationOperation.Arguments.Length == 2 &&
            method.Parameters.Length == 2 &&
            method.Parameters[1].Type.SpecialType == SpecialType.System_Int32 &&
            TryGetIntegralConstantValue(invocationOperation.Arguments[1].Value.Syntax as ExpressionSyntax,
                semanticModel, cancellationToken, out var startAt) &&
            startAt == 0)
        {
        ...
        int inlineDepth)
    {
        ...
        if (method.Name is not "Contains" and not "StartsWith" and not "EndsWith") return false;
        ...
        var hasOrdinalIgnoreCaseComparison =
            HasOrdinalIgnoreCaseStringComparison(invocationOperation.Arguments, semanticModel, cancellationToken);
        ...
        if (invocationOperation.Arguments[0].Value.Syntax is not ExpressionSyntax searchExpression ||
            ...
            searchFormula == null ||
            !TryTranslateStringValue(receiverExpression, semanticModel, cancellationToken, out var receiverFormula,
                getSymbolVersion, inlineDepth) ||
        ...
        out SmtFormula? formula)
    {
        ...
        if (parameterType?.SpecialType != SpecialType.System_Char) return false;

        var constantValue = semanticModel.GetConstantValue(argumentExpression, cancellationToken);
        if (constantValue is not { HasValue: true, Value: char character }) return false;
        ...
        int inlineDepth)
    {
        ...
        if (IsSupportedOrdinalIgnoreCaseStringIndexOfInvocation(invocationOperation, semanticModel,
                cancellationToken) &&
            TryGetConstantStringPredicateArgument(searchExpression, invocationOperation.Arguments[0].Parameter?.Type,
                semanticModel, cancellationToken, out var constantSearch))
        {
            formula = new SmtRegexMatchFormula(
                receiverFormula,
        ...
        int inlineDepth)
    {
        ...
        if (!method.IsStatic ||
            ...
            !TryTranslateValue(valueExpression, semanticModel, cancellationToken, out var referenceFormula,
                getSymbolVersion, inlineDepth) ||
            referenceFormula is not { Kind: SmtValueKind.Reference } ||
            !TryTranslateStringValue(valueExpression, semanticModel, cancellationToken, out var stringFormula,
                getSymbolVersion, inlineDepth) ||
            stringFormula == null)
            return false;
        ...
        var isWhiteSpace = new SmtRegexMatchFormula(stringFormula, @"\A\s*\z");
        ...
        out RegexOptions options)
    {
        ...
        if (invocationExpression.Expression is not MemberAccessExpressionSyntax memberAccess) return false;

        var receiver = UnwrapExpression(memberAccess.Expression);
        if (TryGetRegexPatternFromCreationOrGeneratedFactory(receiver, semanticModel, cancellationToken, out pattern,
    ...
    private static bool TryGetRegexPatternFromCreationOrGeneratedFactory(
        ...
        out RegexOptions options)
    {
        return TryGetRegexPatternFromObjectCreation(expression, semanticModel, cancellationToken, out pattern,
                   out options) ||
               TryGetRegexPatternFromGeneratedRegexFactory(expression, semanticModel, cancellationToken, out pattern,
                   out options);
    }
        ...
        out RegexOptions options)
    {
        ...
        if (HasRegexFieldConstructorAssignment(fieldSymbol, semanticModel.Compilation, cancellationToken)) return false;
    ...
    private static bool HasRegexFieldConstructorAssignment(
        ...
        CancellationToken cancellationToken)
    {
        if (fieldSymbol.ContainingType == null) return false;
        ...
        foreach (var syntaxReference in fieldSymbol.ContainingType.DeclaringSyntaxReferences)
        {
            ...
            foreach (var constructor in typeDeclaration.Members.OfType<ConstructorDeclarationSyntax>())
            {
                ...
                if (isStaticConstructor != fieldSymbol.IsStatic) continue;
        ...
        out RegexOptions options)
    {
        ...
        var rawPattern = TryGetConstantString(objectCreationOperation.Arguments[0].Value.Syntax as ExpressionSyntax,
            semanticModel, cancellationToken);
        ...
        out RegexOptions options)
    {
        ...
        foreach (var variable in localDeclaration.Declaration.Variables)
        {
            ...
            if (!IsSameSymbol(declaredSymbol, regexSymbol)) continue;
        ...
        out RegexOptions options)
    {
        ...
        for (var index = startIndex; index < arguments.Length; index++)
        {
            ...
            if (!TryGetIntegralConstantValue(arguments[index].Value.Syntax as ExpressionSyntax, semanticModel,
                    cancellationToken, out var argumentOptions)) return false;
    ...
    private static string CreateInlineRegexOptionLetters(RegexOptions options)
    {
        ...
        if ((options & RegexOptions.ExplicitCapture) != 0) letters += "n";

        if ((options & RegexOptions.Singleline) != 0) letters += "s";

        if ((options & RegexOptions.IgnorePatternWhitespace) != 0) letters += "x";
    ...
    {
        if (optionLetters.Length == 0) return pattern;
        ...
        if (bodyEnd < bodyStart) return pattern;
        ...
        CancellationToken cancellationToken)
    {
        foreach (var argument in arguments)
        {
            ...
            return TryGetIntegralConstantValue(argument.Value.Syntax as ExpressionSyntax, semanticModel,
                       cancellationToken, out var comparison) &&
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (constantValue is { HasValue: true, Value: string value }) return value;
