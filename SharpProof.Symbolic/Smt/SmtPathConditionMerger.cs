...
internal static class SmtPathConditionMerger
{
    ...
    {
        if (pathConditionSets.Count == 0) return ImmutableArray<SmtFormula>.Empty;
        ...
        IReadOnlyList<ImmutableArray<SmtFormula>> pathConditionSets)
    {
        ...
        foreach (var condition in pathConditionSets[0])
        {
            ...
            if (commonKeys.Contains(key) && emittedKeys.Add(key)) builder.Add(condition);
        }
    ...
    private static void AddConditionalMergedPathConditions(
        ...
        SmtPathConditionMergeOptions options)
    {
        if (pathConditionSets.Count < 2) return;
        ...
        var stateFacts = pathConditionSets
            ...
            .ToArray();
        if (stateFacts.Any(static state => state.FactsByTarget.Count == 0)) return;
    ...
    {
        if (formulas.Count == 0) return new SmtBooleanConstant(true);
    ...
    private sealed class StatePathFacts
    {
            ...
            SmtPathConditionMergeOptions options)
        {
            ...
            branchConditions = localBranchConditions.ToImmutableArray();
