...
internal static partial class CSharpConditionToFormula
{
    ...
    private static readonly
        ...
        s_sourceBooleanFormulaCache = new();

    private static readonly
        ConditionalWeakTable<Compilation,
            ConcurrentDictionary<ExpressionFormulaCacheKey, SourceBooleanFormulaCacheEntry>> s_expressionFormulaCache =
            new();
        ...
        ISet<string>? nonZeroDivisors)
    {
        ...
        if (expression is PrefixUnaryExpressionSyntax prefixUnary &&
            ...
            TryTranslateCore(prefixUnary.Operand, semanticModel, cancellationToken, out var operand, getSymbolVersion,
                inlineDepth, nonZeroDivisors) &&
            operand != null)
        {
        ...
        if (expression is InvocationExpressionSyntax invocationExpression)
        {
            if (TryTranslateKnownStringBooleanInvocation(invocationExpression, semanticModel, cancellationToken,
                    out var knownStringInvocationFormula, getSymbolVersion, inlineDepth) &&
            ...
            if (TryTranslateSourceBooleanInvocation(invocationExpression, semanticModel, cancellationToken,
                    out var invocationFormula, getSymbolVersion, inlineDepth) &&
        ...
        if (expression is BinaryExpressionSyntax binaryExpression)
        {
            ...
            if (binaryExpression.IsKind(SyntaxKind.LogicalAndExpression) &&
                TryTranslateCore(binaryExpression.Left, semanticModel, cancellationToken, out var leftAnd,
                    getSymbolVersion, inlineDepth, nonZeroDivisors) &&
                leftAnd != null)
            {
                var rightNonZeroDivisors = AddNonZeroDivisorFacts(
                    ...
                    getSymbolVersion,
                    inlineDepth);
                if (TryTranslateCore(binaryExpression.Right, semanticModel, cancellationToken, out var rightAnd,
                        getSymbolVersion, inlineDepth, rightNonZeroDivisors) &&
                    rightAnd != null)
                {
            ...
            if (binaryExpression.IsKind(SyntaxKind.LogicalOrExpression) &&
                TryTranslateCore(binaryExpression.Left, semanticModel, cancellationToken, out var leftOr,
                    getSymbolVersion, inlineDepth, nonZeroDivisors) &&
                leftOr != null)
            {
                var rightNonZeroDivisors = AddNonZeroDivisorFacts(
                    ...
                    getSymbolVersion,
                    inlineDepth);
                if (TryTranslateCore(binaryExpression.Right, semanticModel, cancellationToken, out var rightOr,
                        getSymbolVersion, inlineDepth, rightNonZeroDivisors) &&
                    rightOr != null)
                {
        ...
        int inlineDepth)
    {
        ...
        if (IsIdentityPreservingReferenceConversion(asExpression.Left, targetTypeSyntax, semanticModel,
                cancellationToken))
        {
        ...
        if (!TryCreateRuntimeTypeTestFormula(sourceFormula, targetType, out var runtimeTypeTest)) return false;
    ...
    {
        formula = null;
        if (!IsSupportedNullableValueComparison(binaryExpression.Kind())) return false;
        ...
        if (leftIsNullable &&
            ...
            TryTranslateComparison(binaryExpression.Kind(), leftNullableValue, rightNullableValue,
                out var nullableComparison) &&
        ...
        if (rightIsNullable &&
            rightNullableValue != null &&
            TryTranslateValue(binaryExpression.Left, semanticModel, cancellationToken, out var leftValue,
                getSymbolVersion, inlineDepth) &&
            leftValue != null &&
        ...
        ISet<string>? nonZeroDivisors)
    {
        ...
        var rightIsNullableValue = TryTranslateNullableValueMemberAccess(
            ...
            getSymbolVersion,
            inlineDepth);
        if (!leftIsNullableValue && !rightIsNullableValue) return false;
        ...
        formula = comparison;
        if (leftIsNullableValue) formula = new SmtBinaryFormula(SmtBinaryOperator.And, leftHasValue, formula);
        ...
        out SmtFormula? formula)
    {
        ...
        var rightIsTypeOf =
            TryGetTypeOfType(binaryExpression.Right, semanticModel, cancellationToken, out var rightType);
        ...
        if ((leftIsTypeOf &&
             IsNullReferenceComparisonOperand(binaryExpression.Right, semanticModel, cancellationToken)) ||
            (rightIsTypeOf &&
             IsNullReferenceComparisonOperand(binaryExpression.Left, semanticModel, cancellationToken)))
        {
            formula = new SmtBooleanConstant(binaryExpression.IsKind(SyntaxKind.NotEqualsExpression));
            return true;
        }
        ...
        bool requireLocalOrParameterSource)
    {
             ...
             parameterIndex++)
        {
            if (!string.Equals(
                    ...
                    parameterName,
                    StringComparison.Ordinal) ||
                !TryGetPropertyArgumentExpression(propertyReferenceOperation, parameterIndex,
                    out var argumentExpression))
                continue;
    ...
    {
        formula = null;
        if (!TryTranslateValue(expression, semanticModel, cancellationToken, out var value, getSymbolVersion,
                inlineDepth) ||
        ...
        if (!TryCreateRuntimeTypeTestFormula(value, testedType, out var runtimeTypeTest)) return false;
    ...
    private static bool IsTypeKnownAssignableTo(ITypeSymbol? sourceType, ITypeSymbol? targetType)
    {
        ...
        if (targetType.SpecialType == SpecialType.System_Object) return true;
        ...
        int inlineDepth = 0)
    {
        ...
        builder.Add(new SmtBinaryFormula(
            SmtBinaryOperator.Or,
            new SmtUnaryFormula(SmtUnaryOperator.Not,
                new SmtBinaryFormula(SmtBinaryOperator.And, sourceNonNull, runtimeTypeTest)),
            targetNonNull));
        builder.Add(new SmtBinaryFormula(
            SmtBinaryOperator.Or,
    ...
    {
        expression = UnwrapExpression(expression);
        if (semanticModel.GetConstantValue(expression, cancellationToken) is
    ...
    {
        formula = null!;
        if (!TryGetUnsignedCastOperand(leftExpression, semanticModel, cancellationToken, out var indexExpression,
                out var leftUnsignedType) ||
            !TryGetUnsignedCastOperand(rightExpression, semanticModel, cancellationToken, out var lengthExpression,
            ...
            !TryTranslateValue(indexExpression, semanticModel, cancellationToken, out var indexFormula,
                getSymbolVersion, inlineDepth) ||
            indexFormula is not { Kind: SmtValueKind.Int } ||
            !TryTranslateValue(lengthExpression, semanticModel, cancellationToken, out var lengthFormula,
                getSymbolVersion, inlineDepth) ||
        ...
        out SmtFormula? formula)
    {
        ...
        var callableSyntax = methodSymbol.DeclaringSyntaxReferences
            ...
            .FirstOrDefault();
        if (callableSyntax == null) return false;
    ...
    private static ConcurrentDictionary<SourceBooleanFormulaCacheKey, SourceBooleanFormulaCacheEntry>
        GetSourceBooleanFormulaCache(
            Compilation compilation)
    {
    ...
    {
        formula = null;
        if (switchStatement.Sections.Count == 0) return false;
        ...
        foreach (var section in switchStatement.Sections)
        {
            if (!TryGetSwitchSectionReturnExpression(section, out var returnExpression) ||
                ...
                !TryTranslate(returnExpression, semanticModel, cancellationToken, out var sectionValue, null,
                    inlineDepth) ||
        ...
        for (var index = startIndex; index >= 0; index--)
            result = new SmtConditionalFormula(sectionConditions[index], sectionValues[index], result,
                SmtValueKind.Bool);
        ...
        out SmtFormula? formula)
    {
        ...
        for (var index = guards.Count - 1; index >= 0; index--)
        {
            ...
            if (!TryTranslate(guard.Condition, semanticModel, cancellationToken, out var conditionFormula, null,
                    inlineDepth) ||
                conditionFormula is not { Kind: SmtValueKind.Bool } ||
                !TryTranslate(guard.ReturnExpression, semanticModel, cancellationToken, out var guardReturnFormula,
                    null, inlineDepth) ||
        ...
        ISet<string> localVariableNames)
    {
        foreach (var variable in localDeclaration.Declaration.Variables)
        {
            ...
            var localVariableName = GetVariableName(localSymbol, null);
    ...
    {
        formula = null;
        if (!TryTranslate(condition, semanticModel, cancellationToken, out var conditionFormula, null, inlineDepth) ||
            conditionFormula is not { Kind: SmtValueKind.Bool } ||
            !TryTranslate(whenTrue, semanticModel, cancellationToken, out var whenTrueFormula, null, inlineDepth) ||
            whenTrueFormula is not { Kind: SmtValueKind.Bool } ||
            !TryTranslate(whenFalse, semanticModel, cancellationToken, out var whenFalseFormula, null, inlineDepth) ||
            whenFalseFormula is not { Kind: SmtValueKind.Bool })
            return false;
        ...
        out IReadOnlyList<SmtVariableSubstitution> substitutions)
    {
        ...
        foreach (var argument in invocationOperation.Arguments)
        {
            ...
            if (parameter == null ||
                ...
                !TryAddPredicateArgumentSubstitution(parameter, argumentExpression, semanticModel, cancellationToken,
                    getSymbolVersion, inlineDepth, builder))
            {
                substitutions = Array.Empty<SmtVariableSubstitution>();
                return false;
            }
        ...
        List<SmtVariableSubstitution> builder)
    {
        if (!TryGetValueKind(parameter.Type, out var parameterKind) ||
            !TryTranslateValue(argumentExpression, semanticModel, cancellationToken, out var argumentFormula,
                getSymbolVersion, inlineDepth) ||
            argumentFormula == null ||
            argumentFormula.Kind != parameterKind)
            return false;

        var formalVariable = new SmtVariable(GetVariableName(parameter, null), parameterKind);
        ...
        if (parameter.Type.SpecialType == SpecialType.System_String &&
            TryTranslateStringValue(argumentExpression, semanticModel, cancellationToken, out var argumentStringFormula,
                getSymbolVersion, inlineDepth) &&
        ...
        IReadOnlyList<SmtVariableSubstitution> substitutions)
    {
        ...
        foreach (var substitution in substitutions)
        {
            if (TrySubstituteMemberVariable(variable, substitution.SimpleMemberPrefix, substitution.Replacement,
                    out var simpleMemberReplacement) ||
                TrySubstituteMemberVariable(variable, substitution.FormulaMemberPrefix, substitution.Replacement,
                    out simpleMemberReplacement))
        ...
        int inlineDepth = 0)
    {
        ...
        if (expression is BinaryExpressionSyntax stringAsExpression &&
            ...
            IsIdentityPreservingReferenceConversion(stringAsExpression.Left, stringAsType, semanticModel,
                cancellationToken) &&
            TryTranslateValue(stringAsExpression.Left, semanticModel, cancellationToken, out var asReference,
                getSymbolVersion, inlineDepth) &&
        ...
        if (expression is ConditionalExpressionSyntax conditionalExpression &&
            ...
            TryTranslateStringValue(conditionalExpression.WhenTrue, semanticModel, cancellationToken, out var whenTrue,
                getSymbolVersion, inlineDepth) &&
            whenTrue != null &&
            TryTranslateStringValue(conditionalExpression.WhenFalse, semanticModel, cancellationToken,
                out var whenFalse, getSymbolVersion, inlineDepth) &&
            whenFalse != null)
        {
        ...
        if (expression is BinaryExpressionSyntax coalesceExpression &&
            ...
            TryTranslateValue(coalesceExpression.Left, semanticModel, cancellationToken, out var coalesceReference,
                getSymbolVersion, inlineDepth) &&
            coalesceReference is { Kind: SmtValueKind.Reference } &&
            TryTranslateStringValue(coalesceExpression.Left, semanticModel, cancellationToken, out var coalesceLeft,
                getSymbolVersion, inlineDepth) &&
            coalesceLeft != null &&
            TryTranslateStringValue(coalesceExpression.Right, semanticModel, cancellationToken, out var coalesceRight,
                getSymbolVersion, inlineDepth) &&
            coalesceRight != null)
        {
            formula = new SmtConditionalFormula(
        ...
        if (expression is BinaryExpressionSyntax addExpression &&
            ...
            TryTranslateStringConcatOperand(addExpression.Left, semanticModel, cancellationToken, out var concatLeft,
                getSymbolVersion, inlineDepth) &&
            concatLeft != null &&
            TryTranslateStringConcatOperand(addExpression.Right, semanticModel, cancellationToken, out var concatRight,
                getSymbolVersion, inlineDepth) &&
            concatRight != null)
        {
        ...
        {
            formula = new SmtVariable(GetVariableName(symbol.OriginalDefinition, getSymbolVersion) + ".String",
                SmtValueKind.String);
            return true;
        }

        if (expression is MemberAccessExpressionSyntax memberAccess &&
            semanticModel.GetSymbolInfo(memberAccess.Name, cancellationToken)
                .Symbol is IPropertySymbol or IFieldSymbol &&
            TryTranslateValue(memberAccess.Expression, semanticModel, cancellationToken, out var receiver,
                getSymbolVersion, inlineDepth) &&
            receiver != null)
        {
            ...
            if (semanticModel.GetSymbolInfo(memberAccess.Name, cancellationToken).Symbol is IFieldSymbol fieldSymbol &&
                TryGetTupleElementStorageName(memberAccess, fieldSymbol, semanticModel, cancellationToken,
                    out var storageName))
            {
            ...
            formula = new SmtVariable(receiverName + "." + memberAccess.Name.Identifier.ValueText + ".String",
                SmtValueKind.String);
            return true;
        }
    ...
    {
        formula = null;
        if (!IsStringExpression(interpolatedStringExpression, semanticModel, cancellationToken)) return false;
    ...
    {
        if (parts.Count == 0) return new SmtStringConstant(string.Empty);

        var formula = parts[0];
        for (var index = 1; index < parts.Count; index++) formula = new SmtStringConcatTerm(formula, parts[index]);
    ...
    {
        formula = null;
        if (!TryTranslateStringValue(expression, semanticModel, cancellationToken, out var stringFormula,
                getSymbolVersion, inlineDepth) ||
            stringFormula == null)
            return false;

        if (TryTranslateValue(expression, semanticModel, cancellationToken, out var referenceFormula, getSymbolVersion,
                inlineDepth) &&
        ...
        int inlineDepth)
    {
        ...
        if (!TryTranslateStringValue(binaryExpression.Left, semanticModel, cancellationToken, out var left,
                getSymbolVersion, inlineDepth) ||
            left == null ||
            !TryTranslateStringValue(binaryExpression.Right, semanticModel, cancellationToken, out var right,
                getSymbolVersion, inlineDepth) ||
            right == null)
            return false;
        ...
        int inlineDepth)
    {
        ...
        if (TryGetPrefixSubstringComparisonParts(
            ...
            TryGetPrefixSubstringComparisonParts(
                ...
                out prefix))
        {
            if (!TryTranslateStringValue(receiverExpression, semanticModel, cancellationToken, out var receiverFormula,
                    getSymbolVersion, inlineDepth) ||
                receiverFormula == null ||
                !TryCreateStringNonNullFormula(receiverExpression, semanticModel, cancellationToken,
                    out var receiverNonNull, getSymbolVersion, inlineDepth) ||
        ...
        out string prefix)
    {
        ...
        if (constantPrefix == null) return false;
        ...
        int inlineDepth)
    {
        ...
        if (!TryCreateStringNonNullFormula(leftExpression, semanticModel, cancellationToken, out var leftNonNull,
                getSymbolVersion, inlineDepth) ||
            leftNonNull == null ||
            !TryCreateStringNonNullFormula(rightExpression, semanticModel, cancellationToken, out var rightNonNull,
                getSymbolVersion, inlineDepth) ||
            rightNonNull == null)
            return valuesEqual;
        ...
        CancellationToken cancellationToken)
    {
        ...
        return semanticModel.GetOperation(binaryExpression,
                   cancellationToken) is ITupleBinaryOperation tupleOperation &&
        ...
        out ImmutableArray<IFieldSymbol> fields)
    {
        ...
        if (!IsSystemValueTupleType(namedType)) return false;
        ...
        foreach (var field in namedType.GetMembers().OfType<IFieldSymbol>())
        {
            ...
            if (string.Equals(field.Name, "Rest", StringComparison.Ordinal)) return false;

            if (TryGetTupleElementIndex(field.Name, out var elementIndex)) candidates.Add((elementIndex, field));
        }
        ...
        ISet<string>? nonZeroDivisors)
    {
        ...
        {
            if (tupleExpression.Arguments.Count != fields.Length) return false;
        ...
        if (tupleSymbol is not ILocalSymbol and not IParameterSymbol) return false;
        ...
        foreach (var field in fields)
        {
            ...
            symbolBuilder.Add(new SmtVariable(GetVariableName(tupleSymbol, getSymbolVersion) + "." + storageName,
                kind));
        }
    ...
    private static bool IsSystemValueTupleType(INamedTypeSymbol type)
    {
        ...
        return string.Equals(originalDefinition.ContainingNamespace.ToDisplayString(), "System",
                   StringComparison.Ordinal) &&
        ...
        int inlineDepth = 0)
    {
        ...
        if (expression is BinaryExpressionSyntax coalesceExpression &&
            coalesceExpression.IsKind(SyntaxKind.CoalesceExpression) &&
            IsStringExpression(expression, semanticModel, cancellationToken) &&
            TryCreateStringNonNullFormula(coalesceExpression.Left, semanticModel, cancellationToken,
                out var coalesceLeftNonNull, getSymbolVersion, inlineDepth) &&
            coalesceLeftNonNull != null &&
            TryCreateStringNonNullFormula(coalesceExpression.Right, semanticModel, cancellationToken,
                out var coalesceRightNonNull, getSymbolVersion, inlineDepth) &&
        ...
        if (expression is ConditionalExpressionSyntax conditionalExpression &&
            TryTranslate(conditionalExpression.Condition, semanticModel, cancellationToken, out var conditionFormula,
                getSymbolVersion, inlineDepth) &&
            conditionFormula != null &&
            TryCreateStringNonNullFormula(conditionalExpression.WhenTrue, semanticModel, cancellationToken,
                out var whenTrueNonNull, getSymbolVersion, inlineDepth) &&
            whenTrueNonNull != null &&
            TryCreateStringNonNullFormula(conditionalExpression.WhenFalse, semanticModel, cancellationToken,
                out var whenFalseNonNull, getSymbolVersion, inlineDepth) &&
