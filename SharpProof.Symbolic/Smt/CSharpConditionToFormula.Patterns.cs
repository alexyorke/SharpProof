...
internal static partial class CSharpConditionToFormula
{
    ...
    {
        return TryCollectBranchAssumptions(expression, branchWhenTrue, semanticModel, cancellationToken, formulas,
            null);
    }
        ...
        int inlineDepth = 0)
    {
        ...
        if (!IsSupportedBuiltInElementAccessReceiver(receiverType)) return false;
        ...
        Func<ISymbol, int>? getSymbolVersion = null)
    {
        ...
        foreach (var memberAccess in expression.DescendantNodesAndSelf().OfType<MemberAccessExpressionSyntax>())
        {
            if (!IsKnownNonNegativeIntegralMemberAccess(memberAccess, semanticModel, cancellationToken) ||
                !TryTranslateValue(memberAccess, semanticModel, cancellationToken, out var lengthFormula,
                    getSymbolVersion) ||
                lengthFormula is not { Kind: SmtValueKind.Int })
                continue;
        ...
        foreach (var invocation in expression.DescendantNodesAndSelf().OfType<InvocationExpressionSyntax>())
            if (semanticModel.GetOperation(invocation, cancellationToken) is IInvocationOperation invocationOperation)
                AddKnownStringInvocationDomainFacts(invocationOperation, semanticModel, cancellationToken, formulas,
                    getSymbolVersion);
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        ...
        {
            AddStringNonNullDomainFact(regexInputExpression, semanticModel, cancellationToken, formulas,
                getSymbolVersion);
            return;
        }
        ...
        if (invocationOperation.Instance?.Syntax is ExpressionSyntax receiverExpression)
            AddStringNonNullDomainFact(receiverExpression, semanticModel, cancellationToken, formulas,
                getSymbolVersion);
    ...
    private static void AddStringNonNullDomainFact(
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        if (TryCreateStringNonNullFormula(expression, semanticModel, cancellationToken, out var nonNullFormula,
                getSymbolVersion) &&
            nonNullFormula != null)
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        ...
        if (expression is BinaryExpressionSyntax binaryExpression)
        {
            ...
            {
                AddBranchAssumptions(binaryExpression.Left, true, semanticModel, cancellationToken, formulas,
                    getSymbolVersion);
                AddBranchAssumptions(binaryExpression.Right, true, semanticModel, cancellationToken, formulas,
                    getSymbolVersion);
                return;
            }
            ...
            {
                AddBranchAssumptions(binaryExpression.Left, false, semanticModel, cancellationToken, formulas,
                    getSymbolVersion);
                AddBranchAssumptions(binaryExpression.Right, false, semanticModel, cancellationToken, formulas,
                    getSymbolVersion);
                return;
            }
        ...
        if (branchWhenTrue &&
            TryCreateTypeTestNonNullBranchFact(expression, semanticModel, cancellationToken, out var typeTestNonNull,
                getSymbolVersion))
            formulas.Add(typeTestNonNull);

        AddNullComparisonOperandImplications(expression, branchWhenTrue, semanticModel, cancellationToken, formulas,
            getSymbolVersion);
        AddConditionalAccessStringEqualityBranchFacts(expression, branchWhenTrue, semanticModel, cancellationToken,
            formulas, getSymbolVersion);
        AddNotNullWhenBranchFacts(expression, branchWhenTrue, semanticModel, cancellationToken, formulas,
            getSymbolVersion);
        AddMemberNotNullWhenBranchFacts(expression, branchWhenTrue, semanticModel, cancellationToken, formulas);
        if (TryAddInlineAssignmentBranchFacts(expression, branchWhenTrue, semanticModel, cancellationToken, formulas,
                getSymbolVersion)) return;
        ...
        bool rejectOtherReferencesAssignedSymbol)
    {
        ...
        if (assignmentCandidate is not AssignmentExpressionSyntax assignment ||
            ...
            !TryTranslateValue(otherExpression, semanticModel, cancellationToken, out var otherFormula,
                getSymbolVersion) ||
            otherFormula == null ||
        ...
        out ISymbol assignedSymbol)
    {
        ...
        if (!assignment.IsKind(SyntaxKind.SimpleAssignmentExpression) ||
            ...
            semanticModel.GetSymbolInfo(UnwrapExpression(assignment.Left), cancellationToken).Symbol is not ISymbol
                assignmentTarget ||
            assignmentTarget is not ILocalSymbol and not IParameterSymbol ||
            ExpressionReferencesSymbol(assignment.Right, assignmentTarget.OriginalDefinition, semanticModel,
                cancellationToken) ||
        ...
        if (targetFormula is { Kind: SmtValueKind.Reference } &&
            TryCreateAsExpressionAssignmentFacts(
                ...
                getSymbolVersion))
        {
            foreach (var fact in asFacts) formulas.Add(fact);

            return true;
        }

        if (!TryTranslateValue(assignment.Right, semanticModel, cancellationToken, out var assignedValue,
                getSymbolVersion) ||
            assignedValue == null ||
    ...
    {
        formulas.Clear();
        foreach (var formula in replacement) formulas.Add(formula);
    }
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        ...
        if (conditionalCandidate is not ConditionalAccessExpressionSyntax conditionalAccess ||
            !TryCreateStringNonNullFormula(otherCandidate, semanticModel, cancellationToken, out var otherNonNull,
                getSymbolVersion) ||
            otherNonNull is not SmtBooleanConstant { Value: true } ||
            !TryTranslateStringValue(otherCandidate, semanticModel, cancellationToken, out var otherString,
                getSymbolVersion) ||
            otherString == null ||
            !TryTranslateValue(conditionalAccess.Expression, semanticModel, cancellationToken, out var receiver,
                getSymbolVersion) ||
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        ...
        if (testedExpression == null ||
            !TryTranslateValue(testedExpression, semanticModel, cancellationToken, out var testedValue,
                getSymbolVersion) ||
    ...
    private static bool PatternMatchImpliesReferenceNonNull(PatternSyntax pattern)
    {
        ...
        if (pattern is DeclarationPatternSyntax or TypePatternSyntax or RecursivePatternSyntax) return true;
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        ...
        if (IsNullLikeReferenceComparisonOperand(binaryExpression.Left, semanticModel, cancellationToken) &&
            TryCreateOperandNonNullImplication(binaryExpression.Right, semanticModel, cancellationToken,
                out var rightImplication, getSymbolVersion))
        {
        ...
        if (IsNullLikeReferenceComparisonOperand(binaryExpression.Right, semanticModel, cancellationToken) &&
            TryCreateOperandNonNullImplication(binaryExpression.Left, semanticModel, cancellationToken,
                out var leftImplication, getSymbolVersion))
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        ...
        if (sourceExpression == null ||
            !TryTranslateValue(sourceExpression, semanticModel, cancellationToken, out var sourceFormula,
                getSymbolVersion) ||
    ...
    {
        expression = UnwrapExpression(expression);
        if (expression is not IsPatternExpressionSyntax isPatternExpression) return;

        if (TryAddNullablePatternBindingFacts(isPatternExpression, semanticModel, cancellationToken, formulas,
                getSymbolVersion)) return;
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        ...
        switch (pattern)
        {
            case VarPatternSyntax varPattern:
                AddDesignationBindingFact(matchedValue, varPattern.Designation, semanticModel, cancellationToken,
                    formulas, getSymbolVersion, out _);
                return;
            case DeclarationPatternSyntax declarationPattern:
                AddDesignationBindingFact(matchedValue, declarationPattern.Designation, semanticModel,
                    cancellationToken, formulas, getSymbolVersion, out _);
                AddDesignationNonNullFact(declarationPattern.Designation, semanticModel, cancellationToken, formulas,
                    getSymbolVersion);
                return;
            case RecursivePatternSyntax recursivePattern:
                ...
                AddDesignationNonNullFact(recursivePattern.Designation, semanticModel, cancellationToken, formulas,
                    getSymbolVersion);
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        ...
        for (var patternIndex = 0; patternIndex < listPattern.Patterns.Count; patternIndex++)
        {
            ...
            if (subpattern is SlicePatternSyntax) continue;
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        ...
        foreach (var subpattern in subpatterns.Value)
        {
            ...
            if (pathCondition != null) formulas.Add(pathCondition);
    ...
    {
        localValue = null;
        if (!TryCreateDesignationFormula(designation, semanticModel, cancellationToken, getSymbolVersion,
                out var designationValue) ||
        ...
        if (TryCreateDesignationStringFormula(designation, semanticModel, cancellationToken, getSymbolVersion,
                out var designationString) &&
    ...
    {
        formula = null!;
        if (referenceFormula.Kind != SmtValueKind.Reference) return false;
        ...
        int inlineDepth)
    {
        ...
        return TryTranslatePattern(value, expression.Pattern, semanticModel, cancellationToken, out formula,
            getSymbolVersion, valueType, inlineDepth);
    }
        ...
        int inlineDepth)
    {
        ...
        if (pattern is RelationalPatternSyntax relationalPattern &&
            ...
            TryTranslateRelationalPatternComparison(relationalPattern.OperatorToken.Kind(), valueFormula,
                relationalValue, out var comparison))
        {
        ...
        int inlineDepth = 0)
    {
        ...
        if (pattern is ParenthesizedPatternSyntax parenthesizedPattern)
            return TryTranslatePattern(value, parenthesizedPattern.Pattern, semanticModel, cancellationToken,
                out formula, getSymbolVersion, valueType, inlineDepth);
        ...
        if (pattern is RecursivePatternSyntax recursivePattern)
            return TryTranslateRecursivePattern(value, valueType, recursivePattern, semanticModel, cancellationToken,
        ...
        out SmtFormula? formula)
    {
        ...
        if (!TryCreateRuntimeTypeTestFormula(value, patternType, out var runtimeTypeTest)) return false;
    ...
    {
        formula = null;
        var current = ShouldRequireRecursivePatternNonNull(value, valueType)
        ...
        if (propertySubpatterns != null)
            ...
            {
                if (!TryTranslatePropertySubpattern(value, subpattern, semanticModel, cancellationToken,
                        out var subpatternFormula, getSymbolVersion, inlineDepth) ||
        ...
        int inlineDepth)
    {
        ...
        if (!TryTranslatePattern(memberValue, subpattern.Pattern, semanticModel, cancellationToken, out formula,
                getSymbolVersion, memberType, inlineDepth) ||
            formula == null)
            return false;

        if (pathCondition != null) formula = new SmtBinaryFormula(SmtBinaryOperator.And, pathCondition, formula);
        ...
        out SmtFormula? lengthFormula)
    {
        ...
        if (!TryGetListPatternLengthMemberName(valueType, out var memberName)) return false;
