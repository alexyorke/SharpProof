...
internal static class SymbolicSmtFormulaLowerer
{
    ...
    {
        return TryLowerTerm(formula, null, null, null, out term);
    }
        ...
        out SymbolicTerm term)
    {
        switch (formula)
        {
            ...
            case SmtConditionalFormula conditional:
                if (sourceNode != null &&
                    provenance != null &&
                    TryLowerConditionCore(conditional.Condition, sourceNode, provenance, evidenceKey,
                        out var condition) &&
    ...
    private static SymbolicTerm CreateSymbolicVariableTerm(SmtVariable variable)
    {
        ...
        if (TryParseIndexedVariable(variable, out var indexedTerm)) return indexedTerm;
        ...
        out SymbolicTerm term)
    {
        ...
        var indexText = name.Substring(indexStart + 1, name.Length - indexStart - 2);
        if (indexText.Length == 0 ||
            indexText[0] == '^' ||
            !long.TryParse(indexText, NumberStyles.Integer, CultureInfo.InvariantCulture, out var index) ||
            index < 0 ||
    ...
    private static bool TryNormalizeIndexedReceiverName(string receiverText, out string receiverName)
    {
        const string variablePrefix = "SmtVariable { Kind = Reference, Name = ";
        const string variableSuffix = " }";

        if (receiverText.StartsWith(variablePrefix, StringComparison.Ordinal) &&
            receiverText.EndsWith(variableSuffix, StringComparison.Ordinal) &&
            receiverText.Length > variablePrefix.Length + variableSuffix.Length)
        {
            receiverName = receiverText.Substring(
                variablePrefix.Length,
                receiverText.Length - variablePrefix.Length - variableSuffix.Length);
            return true;
        }

        if (receiverText.IndexOf('{') >= 0 || receiverText.IndexOf('}') >= 0)
        {
