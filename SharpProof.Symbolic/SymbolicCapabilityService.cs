...
namespace SharpProof.Symbolic
{
    internal sealed class SymbolicCapabilityService
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var sourceSpan = SymbolicSourceLocation.GetNodeSourceSpan(syntaxTree, target.Declaration.Span, cancellationToken);
            ...
            CancellationToken cancellationToken)
        {
            ...
            switch (target.Kind)
            {
                ...
                default:
                    throw new NotSupportedException("Capability queries support point, position, line, or node targets only.");
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            var methodDisplayName = TryGetDeclaredSymbol(declaration, semanticModel, cancellationToken)?.ToDisplayString() ?? methodName;
            ...
            CancellationToken cancellationToken)
        {
            return declaration switch
            {
                ...
                AccessorDeclarationSyntax accessorDeclaration => semanticModel.GetDeclaredSymbol(accessorDeclaration, cancellationToken),
                LocalFunctionStatementSyntax localFunctionStatement => semanticModel.GetDeclaredSymbol(localFunctionStatement, cancellationToken),
                OperatorDeclarationSyntax operatorDeclaration => semanticModel.GetDeclaredSymbol(operatorDeclaration, cancellationToken),
                ConversionOperatorDeclarationSyntax conversionOperatorDeclaration => semanticModel.GetDeclaredSymbol(conversionOperatorDeclaration, cancellationToken),
                _ => null,
            };
        }
        ...
        private sealed class AnalysisSession
        {
            ...
            public CapabilitySummary Analyze(SyntaxNode declaration, SemanticModel semanticModel)
            {
                ...
                try
                {
                    var rootOperation = GetMethodBodyRootOperation(declaration, semanticModel, _cancellationToken);
                    if (rootOperation == null)
                    {
                        var unsupported = CapabilitySummary.Unknown(SymbolicCapabilityUnknownReason.UnsupportedTarget);
                        if (declaredMethodSymbol != null)
                        {
                            _methodCache[declaredMethodSymbol] = unsupported;
                        }
                    ...
                    foreach (var operation in rootOperation.DescendantsAndSelf())
                    {
                        ...
                        if (!IsVisibleOperation(operation, declaration))
                        {
                            continue;
                        }
                        ...
                        foreach (var site in AnalyzeOperation(operation, semanticModel))
                        {
                            ...
                            if (site.IsUnknown)
                            {
                                unknownReasons.Add(site.UnknownReason);
                            }
                        }
                    }
                    ...
                    if (declaredMethodSymbol != null)
                    {
                        _methodCache[declaredMethodSymbol] = summary;
                    }
                ...
                finally
                {
                    if (declaredMethodSymbol != null)
                    {
                        _activeMethods.Remove(declaredMethodSymbol);
                    }
                }
            }
                ...
                switch (operation)
                {
                    case ILockOperation:
                        yield return CapabilitySiteData.Proven(
                            ...
                            operation,
                            "lock",
                            symbolDisplayName: string.Empty);
                        yield break;

                    case IDynamicMemberReferenceOperation dynamicMemberReferenceOperation
                        when dynamicMemberReferenceOperation.Parent is IDynamicInvocationOperation or IDynamicIndexerAccessOperation:
                        yield break;
                    ...
                    case IInvocationOperation invocation:
                        foreach (var site in AnalyzeSymbolUsage(invocation.TargetMethod, invocation, "invocation", invocation.TargetMethod))
                        {
                            yield return site;
                        }
                        yield break;
                    ...
                    case IPropertyReferenceOperation propertyReferenceOperation:
                        foreach (var site in AnalyzePropertyUsage(propertyReferenceOperation))
                        {
                            yield return site;
                        }
                        yield break;
            ...
            private IEnumerable<CapabilitySiteData> AnalyzePropertyUsage(IPropertyReferenceOperation propertyReferenceOperation)
            {
                var accessor = propertyReferenceOperation.Property.GetMethod ?? propertyReferenceOperation.Property.SetMethod;
                foreach (var site in AnalyzeSymbolUsage(accessor, propertyReferenceOperation, "property_access", propertyReferenceOperation.Property))
                {
                    yield return site;
                }
            }

            private IEnumerable<CapabilitySiteData> AnalyzeFieldUsage(IFieldSymbol fieldSymbol, IFieldReferenceOperation fieldReferenceOperation)
            {
                ...
                ISymbol? displaySymbol)
            {
                ...
                if (ShouldTreatAsDynamicDispatch(methodSymbol, operation))
                {
                    yield return CapabilitySiteData.Unknown(
                        operation,
                        siteKind,
                        SymbolicCapabilityUnknownReason.DynamicDispatch,
                        displaySymbol?.ToDisplayString(CapabilitySymbolDisplayFormat) ?? methodSymbol.ToDisplayString(CapabilitySymbolDisplayFormat));
                    yield break;
                }
            ...
            private static bool ShouldTreatAsDynamicDispatch(IMethodSymbol methodSymbol, IOperation operation)
            {
                ...
                if (originalMethod.ContainingType?.TypeKind == TypeKind.Interface)
                {
                ...
                if (!originalMethod.IsVirtual && !originalMethod.IsOverride)
                {
                    return false;
                }
            ...
            private static IOperation? GetReceiverOperation(IOperation operation)
            {
                ...
                out SemanticModel semanticModel)
            {
                ...
                foreach (var syntaxReference in methodSymbol.DeclaringSyntaxReferences)
                {
                    ...
                    if (!IsMethodLikeDeclaration(syntax))
                    {
                        continue;
                    }
            ...
            private static bool TryClassifySymbolCapabilities(ISymbol symbol, out SharpProofCapability capabilities)
            {
                ...
                if (IsNativeInteropSymbol(originalSymbol))
                {
                ...
                capabilities = NormalizeCapabilities(capabilities);
                return capabilities != SharpProofCapability.None || IsKnownCapabilityNeutralSymbol(namespaceName, typeName, memberName);
            }

            private static bool ShouldTreatMetadataSymbolAsUnknown(ISymbol symbol)
            {
                var originalSymbol = symbol.OriginalDefinition;
                if (originalSymbol.Locations.Any(static location => location.IsInSource))
                {
                    return false;
                }
            ...
            private static SharpProofCapability ClassifyKnownSymbolFamily(
                ...
                ISymbol symbol)
            {
                if (typeName == "System.Console")
                {
                    return SharpProofCapability.Console;
                }
                ...
                ISymbol symbol)
            {
                ...
                return symbol.ContainingNamespace?.ToDisplayString().StartsWith("System.Reflection", StringComparison.Ordinal) == true ||
                    typeName == "System.Activator"
            ...
            {
                if (ContainsAny(memberName, "Read", "OpenRead", "ReadAll", "ReadLines", "Exists", "Enumerate", "Get", "Length", "AvailableFreeSpace", "TotalSize"))
                {
                    return SharpProofCapability.FileRead;
                }

                if (ContainsAny(memberName, "Write", "Append", "Create", "Delete", "Move", "Set", "Replace", "Copy", "Encrypt", "Decrypt"))
                {
            ...
            {
                if (ContainsAny(memberName, "Read", "CopyTo"))
                {
                    return SharpProofCapability.IO;
                }

                if (ContainsAny(memberName, "Write", "Flush", "SetLength"))
                {
                    return SharpProofCapability.IO;
                }
        ...
        private sealed class CapabilitySiteData
        {
                ...
                SymbolicCapabilityUnknownReason unknownReason)
            {
                ...
                Identity =
                    operation.Syntax.SpanStart.ToString() + "|" +
                    operation.Syntax.Span.Length.ToString() + "|" +
                    siteKind + "|" +
                    Capabilities.ToString() + "|" +
                    unknownReason.ToString() + "|" +
                    symbolDisplayName;
            }
