...
{
    private const string NotNullIfNotNullAttributeMetadataName =
        "System.Diagnostics.CodeAnalysis.NotNullIfNotNullAttribute";

    internal static bool TryGetNotNullIfNotNullParameterName(IMethodSymbol methodSymbol, out string parameterName)
    {
        if (TryGetNotNullIfNotNullParameterName(methodSymbol.GetReturnTypeAttributes(), out parameterName)) return true;

        if (!SymbolEqualityComparer.Default.Equals(methodSymbol, methodSymbol.OriginalDefinition) &&
            TryGetNotNullIfNotNullParameterName(methodSymbol.OriginalDefinition.GetReturnTypeAttributes(),
                out parameterName))
            return true;
    ...
    internal static bool TryGetNotNullIfNotNullParameterName(IPropertySymbol propertySymbol, out string parameterName)
    {
        if (TryGetNotNullIfNotNullParameterName(propertySymbol.GetAttributes(), out parameterName) ||
            TryGetNotNullIfNotNullParameterName(
                propertySymbol.GetMethod?.GetReturnTypeAttributes() ?? ImmutableArray<AttributeData>.Empty,
                out parameterName))
            return true;

        if (!SymbolEqualityComparer.Default.Equals(propertySymbol, propertySymbol.OriginalDefinition) &&
            (TryGetNotNullIfNotNullParameterName(propertySymbol.OriginalDefinition.GetAttributes(),
                 out parameterName) ||
             TryGetNotNullIfNotNullParameterName(
                 propertySymbol.OriginalDefinition.GetMethod?.GetReturnTypeAttributes() ??
                 ImmutableArray<AttributeData>.Empty, out parameterName)))
            return true;
