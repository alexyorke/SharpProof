...
internal static class SymbolicStateInvalidator {
    ...
    internal static void InvalidateNestedMutations(
        ...
        SemanticModel semanticModel,
        CancellationToken cancellationToken)
            => state = ApplyNestedMutationInvalidations(state, LowerNestedMutations(root, semanticModel, cancellationToken));
    ...
    internal static void InvalidateMutationTarget(
        ...
        SemanticModel semanticModel,
        CancellationToken cancellationToken) {
        var invalidations = SymbolicMutationInventory.LowerTargetInvalidations(mutatedExpression, semanticModel, cancellationToken);
    ...
    internal static void InvalidateSymbol(ref SymbolicState state, ISymbol symbol, SyntaxNode source)
        => state = SymbolicOperationTransferKernel.Invalidate(
            state,
            [new SymbolicInvalidationTarget(SymbolicFactFactory.GetSmtVariableName(symbol.OriginalDefinition))],
            source.Span,
    ...
    internal static bool IsCurrentInstanceMemberReference(
        ...
        if (expression is IdentifierNameSyntax &&
            SymbolicMutationInventory.GetMutatedSymbol(expression, semanticModel,
                cancellationToken) is { IsStatic: false } and (IFieldSymbol or IPropertySymbol))
            return true;
