...
internal static class SymbolicReachabilityService
{
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        ...
        formula = SmtFormulaFactory.CreateReferenceNullComparison(testedValue, false);
        return true;
    }
        ...
        Func<ISymbol, int>? getSymbolVersion)
    {
        ...
        formula = SmtFormulaFactory.CreateReferenceNullComparison(sourceFormula, false);
        return true;
    }
        ...
        ICollection<SymbolicFact> facts)
    {
        ...
        if (designation is not SingleVariableDesignationSyntax singleVariableDesignation) return false;
        ...
        IEnumerable<SmtFormula>? pathConditions = null)
    {
        ...
        if (IsFormulaAlwaysFalse(new SmtUnaryFormula(SmtUnaryOperator.Not, formula), basePathConditions,
                smtAnalysis)) return true;
        ...
        IReadOnlyCollection<SmtFormula>? pathConditions = null)
    {
        ...
        if (expression is BinaryExpressionSyntax binaryExpression)
        {
            ...
            {
                var left = EvaluateKnownConditionTruth(binaryExpression.Left, semanticModel, cancellationToken,
                    smtAnalysis, pathConditions);
                var right = EvaluateKnownConditionTruth(binaryExpression.Right, semanticModel, cancellationToken,
                    smtAnalysis, pathConditions);
        ...
        Func<ISymbol, int>? getTargetSymbolVersion = null)
    {
        ...
        fact = SmtFormulaFactory.CreateEquality(
            SmtFormulaFactory.CreateReferenceNullComparison(targetReferenceFormula, false),
            valueNonNullFormula);
        return true;
    }
        ...
        Func<ISymbol, int>? getTargetSymbolVersion = null)
    {
        ...
        fact = SmtFormulaFactory.CreateEquality(
            SmtFormulaFactory.CreateReferenceNullComparison(targetFormula, false),
            valueNonNullFormula);
        return true;
    }
        ...
        bool requireLocalOrParameterSource)
    {
        ...
        var sourceNonNull = SmtFormulaFactory.CreateReferenceNullComparison(sourceFormula, false);
    ...
    {
        if (!SymbolicIrFormulaEncoder.TryEncode(condition, out var formula)) return false;
        ...
        ICollection<SmtFormula> facts)
    {
        ...
        else if (SymbolicTypeFacts.TryGetNullableUnderlyingType(SymbolicFactFactory.GetTrackedSymbolType(targetSymbol),
        ...
        out NullableValueParts parts)
    {
        ...
        if (TryTranslateIrNullLikeNullableValueParts(expression, context, out parts)) return true;
        ...
        if (TryTranslateIrNullableWrappedValueParts(expression, context, out parts)) return true;
        ...
        out NullableValueParts parts)
    {
        ...
        if (!SymbolicIrFormulaEncoder.TryEncode(hasValue, out var hasValueFormula)) return false;
        ...
        out SymbolicTerm valueTerm)
    {
        ...
        if (conditionalAccess.WhenNotNull is not MemberBindingExpressionSyntax memberBinding ||
            context.SemanticModel.GetSymbolInfo(memberBinding.Name, context.CancellationToken).Symbol is not
            { } memberSymbol ||
    ...
    {
        if (!SymbolicTypeFacts.TryGetNullableUnderlyingType(SymbolicFactFactory.GetTrackedSymbolType(symbol),
                out var underlyingType) ||
            !SymbolicFactFactory.TryGetValueKind(
                underlyingType,
                SymbolicFactFactory.IsSupportedSmtIntegralOrEnumType,
    ...
    internal static void AddReferenceBackedArrayDimensionLengthFacts(
        ...
        ICollection<SmtFormula> facts)
    {
        if (!TryCreateReferenceSymbolTerm(targetSymbol, null, out var targetReferenceTerm) ||
            !SymbolicIrFormulaEncoder.TryEncodeTerm(targetReferenceTerm, out var targetReference))
            return;
    ...
    internal static void AddArrayDimensionLengthAssignedValueFacts(
        ...
        ICollection<SmtFormula> facts)
    {
        if (SymbolicFactFactory.GetTrackedSymbolType(targetSymbol) is not IArrayTypeSymbol targetArrayType) return;
    ...
    {
        inRange = null!;
        if (indexExpressions.Count != arrayType.Rank) return false;
    ...
    {
        fact = null!;
        if (!TryCreateSymbolSmtValue(targetSymbol, out var targetFormula)) return false;
        ...
        var updatedValue = delta > 0
            ...
            : new SmtIntegerBinaryTerm(SmtIntegerBinaryOperator.Subtract, previousValue,
                new SmtIntegerConstant(-delta));
        fact = new SmtBinaryFormula(SmtBinaryOperator.Equal, targetFormula, updatedValue);
        return true;
    }
