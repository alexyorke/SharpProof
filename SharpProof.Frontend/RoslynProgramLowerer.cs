...
public sealed class RoslynProgramLowerer(
    ...
    public FrontendProgramLoweringResult Lower(ControlFlowGraph graph) {
        ...
        return new LoweringSession(_factory, graph, _isKnownPure).Lower();
    }

    private sealed class LoweringSession(
        IrFactory factory, ControlFlowGraph graph,
        ...
        internal FrontendProgramLoweringResult Lower() {
            ...
            return new FrontendProgramLoweringResult(
                ...
                _expressions.CreateCaptureBindings(), [.. _abstentions]);
        }
        ...
        private void LowerBlock(BasicBlock source) {
            ...
            foreach (var operation in source.Operations) {
                var identity = CreateOperation(
                    source, operationOrdinal++, operation.Kind);
                if (LowerStatement(block, identity, operation)) return;
            }
            LowerTerminator(
                source, block,
                CreateOperation(source, operationOrdinal, OperationKind.None));
        }
        ...
        private void LowerDeclaration(
            IrBlockId block, OperationId operation,
        ...
        private void LowerDeclarator(
            ...
            var target = _expressions.GetVariable(
                declarator.Symbol, declarator.Symbol.Type);
        ...
        private void LowerAssignment(
            ...
            var variable = _expressions.GetReferencedVariable(
                assignment.Target, unwrapConversions: false);
        ...
        private IrVariableTerm? LowerInvocation(
            ...
            var arguments = LowerValues(
                block, operation,
                invocation.Arguments.Select(static argument => argument.Value));
            var resultType = _expressions.GetTypeId(invocation.Type);
            var member = _expressions.GetMember(
                invocation.TargetMethod, receiver, "call:", invocation.Type, arguments);
            ...
            _builder.Call(block, operation, target, member, receiver, arguments);

            var mutated = invocation.Arguments
                .Where(static argument => argument.Parameter?.RefKind is
                    RefKind.Ref or RefKind.Out)
                .Select(argument =>
                    _expressions.GetReferencedVariable(argument.Value))
            ...
            try {
                switch (target) {
                    case IFieldReferenceOperation field:
                        var fieldReceiver =
                            LowerOptionalValue(block, operation, field.Instance);
                        var fieldMember = _expressions.GetMember(
                            field.Field, fieldReceiver, "field:", field.Type);
                        return LocationLowering.FromLocation(
                            _builder.MemberLocation(fieldMember, fieldReceiver));
                    case IPropertyReferenceOperation property:
                        var propertyReceiver =
                            LowerOptionalValue(block, operation, property.Instance);
                        var propertyArguments = LowerValues(
                            block, operation,
                        ...
                        return LocationLowering.FromLocation(
                            _builder.MemberLocation(
                                propertyMember, propertyReceiver, propertyArguments));
        ...
        private void LowerReturn(
            IrBlockId block, OperationId operation, IOperation? value) =>
            _builder.Return(
                block, operation, LowerOptionalValue(block, operation, value));
        ...
        private void Havoc(
            IrBlockId block, OperationId operation, IrHavocKind kind,
        ...
        private void Observe(
            OperationId operation, FrontendSubsetClassification classification) {
        ...
        private static bool IsDirectInvocation(IInvocationOperation invocation) {
