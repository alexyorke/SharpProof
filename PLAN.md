...
- Production inventory currently reports 112,014 C# production lines across
  178 files. The largest modules are `PurelySharp.Symbolic` at 53,269 lines,
...
- Nullable term lowering now lives in `SymbolicIrLowerer.Nullable.cs`, reducing
  ...
  helpers now live in `SymbolicIrLowerer.Types.cs`, and operator mapping/
  comparison helpers now live in `SymbolicIrLowerer.Operators.cs`;
  `SymbolicIrLowerer.cs` is about 442 lines.
