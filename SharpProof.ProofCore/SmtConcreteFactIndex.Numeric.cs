...
internal sealed partial class SmtConcreteFactIndex {
    ...
    private bool TryAddIntegerIntervalFact(SmtFormula formula, out bool hasContradiction) {
        if (!SmtComparisonOperatorFacts.TryGetIntegerComparison(formula, out var term, out var op, out var constant))
    ...
    private bool TryAddAffineIntegerComparisonFact(SmtFormula formula, out bool hasContradiction) {
        ...
        if (TryGetKnownInteger(left, out var leftKnown) &&
            ...
            return TryClassifyConstantComparison(leftKnown, op, rightKnown, out hasContradiction);

        if (TryGetKnownInteger(right, out var rightConstant) &&
            TryGetAffineIntegerTerm(left, 0, out var leftAffine))
            return AddAffineIntegerComparisonFact(leftAffine, op, rightConstant, out hasContradiction);
        ...
        return AddAffineIntegerComparisonFact(difference, op, 0, out hasContradiction);
    }
    private bool AddAffineIntegerComparisonFact(
        ...
        if (term.BaseTerm == null ||
            term.Scale == 0)
            return TryClassifyConstantComparison(term.Offset, op, constant, out hasContradiction);
        ...
        return AddIntegerIntervalFact(normalizedTerm, normalizedOp, normalizedConstant, out hasContradiction);
    }
    private static bool TryClassifyConstantComparison(long left, SmtBinaryOperator op, long right, out bool hasContradiction) {
        if (!TryEvaluateConstantComparison(left, op, right, out var constantContradiction, out var constantTautology)) {
            hasContradiction = false;
            return false;
        }
    ...
    private bool AddIntegerIntervalFact(SmtFormula term, SmtBinaryOperator op, long constant, out bool hasContradiction) {
        term = NormalizeAliases(term);
    ...
    private bool TryNormalizeAffineIntegerComparison(
        ...
        if (affine.BaseTerm == null ||
            affine.Scale == 0)
            return TryEvaluateConstantComparison(affine.Offset, op, constant, out hasContradiction, out isTautology);
    ...
    private static bool TryInvertPositiveScaleComparison(
        ...
        out long normalizedConstant,
        out bool hasContradiction,
        out bool isTautology) => TryInvertPositiveScaleComparison(
            op,
    ...
    private bool TryGetAffineIntegerTerm(SmtFormula formula, int depth, out SmtAffineIntegerTerm affine) => SmtAffineIntegerTerm.TryCreate(
            formula,
