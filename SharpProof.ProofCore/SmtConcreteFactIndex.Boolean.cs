...
internal sealed partial class SmtConcreteFactIndex {
    ...
    private bool TryEvaluateBoolean(SmtFormula formula, out bool value, int conditionalBranchDepth)
        => TryEvaluateBoolean(formula, out value, conditionalBranchDepth, true);
    private bool TryEvaluateBoolean(SmtFormula formula, out bool value, int conditionalBranchDepth, bool allowDirectFact) {
    ...
    private bool TryAddBooleanFact(SmtFormula formula, bool value, out bool hasContradiction) {
    ...
    private bool TryAddBooleanFactCore(SmtFormula formula, bool value, out bool hasContradiction) {
        hasContradiction = false;
        ...
        if (formula is SmtBinaryFormula binary) {
            if (binary.Operator == SmtBinaryOperator.And) {
                ...
                var addedEquivalence = TryAddBooleanEquivalenceFact(binary, value, out var equivalenceContradiction);
    ...
    private bool TryAddKnownBooleanComparisonFact(SmtFormula formula, bool value, out bool hasContradiction) {
        hasContradiction = false;
    ...
    private bool AddExactBoolean(SmtFormula formula, bool value, out bool hasContradiction) {
        hasContradiction = false;
        formula = NormalizeAliases(formula);
        var canonical = FindBooleanCanonical(formula, out var isNegatedFromCanonical);
    ...
    private bool AddExactBooleanWithoutInference(SmtFormula formula, bool value, out bool hasContradiction) {
        hasContradiction = false;
    ...
    private bool TryAddBooleanEquivalenceFact(SmtBinaryFormula formula, bool value, out bool hasContradiction) {
        hasContradiction = false;
    ...
    private bool TryGetBooleanRelationTerm(SmtFormula formula, out SmtFormula term, out bool isNegated) {
    ...
    private bool UnionBooleanEquivalences(SmtFormula left, SmtFormula right, bool differs, out bool hasContradiction) {
    ...
    private void MergeBooleanFacts(SmtFormula canonical, SmtFormula alias, bool aliasDiffersFromCanonical, out bool hasContradiction) {
        hasContradiction = false;
    ...
    private bool TryEvaluateComparison(SmtBinaryFormula binary, out bool value, int conditionalBranchDepth) {
    ...
    private bool TryEvaluateConditionalComparison(SmtBinaryFormula binary, out bool value, int conditionalBranchDepth) {
    ...
    private bool TryEvaluateConditionalBranches(
        ...
        if (TryGetDirectKnownBooleanValue(condition, out var conditionValue))
            return TryEvaluateBoolean(conditionValue ? whenTrue : whenFalse, out value, conditionalBranchDepth + 1);
    ...
    private SmtConcreteFactIndex ForkWithBooleanAssumption(SmtFormula formula, bool value, out bool hasContradiction) {
    ...
    private bool TryClassifyBooleanFromFacts(SmtFormula formula, out bool value, int conditionalBranchDepth) {
