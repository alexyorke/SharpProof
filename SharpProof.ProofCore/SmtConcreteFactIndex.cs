...
internal sealed partial class SmtConcreteFactIndex {
    ...
    private readonly Dictionary<SmtFormula, (SmtFormula Parent, bool Differs)> _aliases = [];
    private readonly Dictionary<SmtFormula, (SmtFormula Parent, bool Differs)> _booleanEquivalences = [];
    private readonly Dictionary<SmtFormula, bool> _exactBooleans = [];
    private readonly Dictionary<SmtFormula, string> _exactStrings = [];
    private readonly Dictionary<SmtFormula, HashSet<string>> _excludedStrings = [];

    private readonly Dictionary<SmtFormula, SmtIntegerInterval> _integerIntervals = [];
    private readonly Dictionary<SmtFormula, bool> _referenceNullStates = [];
    ...
    internal SmtConcreteFactIndex() => _workBudget = new SyntacticWorkBudget(MaxSyntacticWorkItems);
    ...
    internal static SmtConcreteFactIndex Create(IEnumerable<SmtFormula> formulas, out bool hasContradiction) {
    ...
    private void AddAll(IEnumerable<SmtFormula> formulas, out bool hasContradiction) {
        ...
        var formulaArray = formulas as SmtFormula[] ?? [.. formulas];
    ...
    private bool TryAddAffineIntegerEqualityFact(SmtFormula formula, out bool hasContradiction) {
        ...
        if (SmtAffineIntegerTerm.TrySubtract(left, right, out var difference)) {
            ...
            if (TrySolveSingleAffineEquality(difference, out var solvedTerm, out var solvedConstant, out hasContradiction)) {
                if (hasContradiction) return true;

                return AddIntegerIntervalFact(solvedTerm, SmtBinaryOperator.Equal, solvedConstant, out hasContradiction);
            }
        }
        return TryAddUnitAffineAlias(left, right, out hasContradiction);
    }
    private bool TryAddUnitAffineAlias(SmtAffineIntegerTerm left, SmtAffineIntegerTerm right, out bool hasContradiction) {
        hasContradiction = false;
        ...
        if (offset == 0) {
            ...
            replacement = new SmtIntegerBinaryTerm(SmtIntegerBinaryOperator.Add, baseTerm, new SmtIntegerConstant(offset));
        }
    ...
    private static SmtFormula CreateAffineTerm(SmtAffineIntegerTerm term) {
        ...
        var scaled = term.Scale switch {
            ...
            _ => new SmtIntegerBinaryTerm(SmtIntegerBinaryOperator.Multiply, term.BaseTerm, new SmtIntegerConstant(term.Scale))
        };

        return term.Offset == 0
            ? scaled
            : new SmtIntegerBinaryTerm(SmtIntegerBinaryOperator.Add, scaled, new SmtIntegerConstant(term.Offset));
    }
    private static bool TryGetIntegerBinaryComparison(
        ...
        if (!SmtComparisonOperatorFacts.TryExtract(formula, out var binary, out var negationCount) ||
                binary.Left.Kind != SmtValueKind.Int ||
                binary.Right.Kind != SmtValueKind.Int)
            return false;
    ...
    private static bool TryGetAliasComparison(SmtFormula formula, out SmtFormula left, out SmtFormula right) {
        left = null!;
        right = null!;
        if (formula is not SmtBinaryFormula { Operator: SmtBinaryOperator.Equal } binary ||
    ...
    private static bool CanAliasTerm(SmtFormula formula) => formula is not SmtBooleanConstant and
    ...
    private SmtFormula NormalizeAliases(SmtFormula formula) {
        ...
        return NormalizeAliases(formula, []);
    }
    private SmtFormula NormalizeAliases(SmtFormula formula, HashSet<SmtFormula> visiting) {
        ...
        var normalized = SmtFormulaTraversal.MapChildren(formula, child => NormalizeAliases(child, visiting));
    ...
    private static bool ReferencesFormula(SmtFormula formula, SmtFormula candidate)
        => !SmtFormulaTraversal.IsWithinDepth(formula, MaxFormulaReferenceDepth + 1) ||
