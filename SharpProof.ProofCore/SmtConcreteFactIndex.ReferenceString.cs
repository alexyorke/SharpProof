...
internal sealed partial class SmtConcreteFactIndex {
    ...
    private static bool TryGetReferenceNullComparison(SmtFormula formula, out SmtFormula term, out bool isNull) {
        term = null!;
        isNull = false;
    ...
    internal bool TryGetKnownReferenceNullState(SmtFormula formula, out bool isNull) {
        ...
        if (formula is SmtConditionalFormula { Kind: SmtValueKind.Reference } conditional &&
            TryEvaluateBoolean(conditional.Condition, out var conditionValue))
            return TryGetKnownReferenceNullState(conditionValue ? conditional.WhenTrue : conditional.WhenFalse, out isNull);

        isNull = false;
        return false;
    }
    private void AddStringLengthFact(SmtFormula stringFormula, int length, out bool hasContradiction) {
    ...
    private bool TryGetKnownStringLength(SmtFormula formula, out long length) {
        ...
        switch (formula) {
            ...
            case SmtConditionalFormula conditional
                when conditional.Kind == SmtValueKind.String &&
                     TryEvaluateBoolean(conditional.Condition, out var conditionValue):
                return TryGetKnownStringLength(conditionValue ? conditional.WhenTrue : conditional.WhenFalse, out length);
    ...
    private static bool TryGetStringComparison(SmtFormula formula, out SmtFormula term, out SmtBinaryOperator op, out string value) {
        term = null!;
        op = default;
        value = string.Empty;
        if (!SmtComparisonOperatorFacts.TryExtract(formula, out var binary, out var negationCount))
