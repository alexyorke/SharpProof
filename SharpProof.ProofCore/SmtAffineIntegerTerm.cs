...
internal readonly struct SmtAffineIntegerTerm(SmtFormula? baseTerm, long scale, long offset) {
    ...
    internal static bool TryCreate(
        ...
        return TryCreateCore(formula, 0, maxDepth, normalize, resolveConstant, resolveWholeFormula, canUseBaseTerm, out affine);
    }
    internal static bool TryAdd(SmtAffineIntegerTerm left, SmtAffineIntegerTerm right, out SmtAffineIntegerTerm result)
        => TryCombine(left, right, false, out result);
    internal static bool TrySubtract(SmtAffineIntegerTerm left, SmtAffineIntegerTerm right, out SmtAffineIntegerTerm result)
        => TryCombine(left, right, true, out result);
    internal static bool TryScale(SmtAffineIntegerTerm value, long scale, out SmtAffineIntegerTerm result) {
        result = default;
    ...
    private static bool TryCreateCore(
        ...
        switch (formula) {
            ...
            case SmtIntegerBinaryTerm binary:
                if (TryCreateBinary(binary, depth, maxDepth, normalize, resolveConstant, resolveWholeFormula, canUseBaseTerm, out affine))
                    return true;
    ...
    private static bool TryCreateBinary(
        ...
        if (binary.Operator is not (SmtIntegerBinaryOperator.Add or SmtIntegerBinaryOperator.Subtract) ||
            !TryCreateCore(binary.Left, depth + 1, maxDepth, normalize, resolveConstant, resolveWholeFormula,
                canUseBaseTerm, out var left) ||
            !TryCreateCore(
                binary.Right,
                depth + 1,
                maxDepth,
    ...
    private static bool TryCreateBaseTerm(SmtFormula formula, Func<SmtFormula, bool> canUseBaseTerm, out SmtAffineIntegerTerm affine) {
...
internal static class SmtIntegerArithmetic {
    internal static bool TryEvaluateBinary(SmtIntegerBinaryOperator op, long left, long right, out long value) {
