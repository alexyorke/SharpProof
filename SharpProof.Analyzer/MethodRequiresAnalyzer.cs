...
internal static class MethodRequiresAnalyzer
{
        ...
        SharpProofAttributeIdentityPolicy attributePolicy)
    {
        ...
        var contracts =
            RequiresContractHelpers.ValidContracts(callSite.Value.Method, attributePolicy, context.CancellationToken);
        if (contracts.Length == 0) return;
        ...
        foreach (var contract in contracts)
        {
            ...
            if (proof.TruthValue == SymbolicTruthValue.ProvenFalse)
            {
                ReportIfNotSuppressed(
                    context,
                    baseline,
                    CreateNotProvenDiagnostic(callSite.Value.Method, contract.Condition, location, contract.Location,
                        proof));
                continue;
            }
