...
internal partial class PurityAnalysisEngine
{
    ...
    {
        if (symbol == null || enforcePureAttributeSymbol == null) return false;

        if (HasPureExternalAttribute(symbol) || HasRecognizedExternalPureAttribute(symbol)) return true;
    ...
    {
        return
            string.Equals(attributeData.AttributeClass?.ToDisplayString(), fullyQualifiedMetadataName,
                StringComparison.Ordinal) ||
            string.Equals(
