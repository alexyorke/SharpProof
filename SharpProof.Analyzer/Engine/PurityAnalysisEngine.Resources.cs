...
internal partial class PurityAnalysisEngine
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var usingOperation in ExecutionVisibility.VisibleDescendants(methodBodyOperation)
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!IsStraightLineUsingStatement(usingOperation.Syntax)) continue;

            nextState = AddUsingStatementDisposeFacts(nextState, usingOperation, nextState);
        }

        foreach (var usingDeclaration in ExecutionVisibility.VisibleDescendants(methodBodyOperation)
                     .OfType<IUsingDeclarationOperation>())
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!IsStraightLineUsingStatement(usingDeclaration.Syntax)) continue;
    ...
    {
        var nextState = currentState;
        foreach (var declarationGroup in ExecutionVisibility.VisibleDescendants(methodBodyOperation)
                     .OfType<IVariableDeclarationGroupOperation>())
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!IsStraightLineUsingStatement(declarationGroup.Syntax)) continue;
        ...
        foreach (var deconstructionAssignment in ExecutionVisibility.VisibleDescendants(methodBodyOperation)
                     .OfType<IDeconstructionAssignmentOperation>())
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!IsStraightLineUsingStatement(deconstructionAssignment.Syntax)) continue;
        ...
        foreach (var assignmentSyntax in methodBodyOperation.Syntax.DescendantNodes()
                     .OfType<AssignmentExpressionSyntax>())
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!IsStraightLineUsingStatement(assignmentSyntax)) continue;
        ...
        foreach (var invocation in ExecutionVisibility.VisibleDescendants(methodBodyOperation)
                     .OfType<IInvocationOperation>())
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (!IsStraightLineUsingStatement(invocation.Syntax)) continue;
    ...
    private static PurityAnalysisState AddDeconstructedResourceAcquisitionFacts(
        ...
        CancellationToken cancellationToken)
    {
        if (!IsDeconstructionAssignmentSyntax(assignmentSyntax.Left)) return nextState;
                     ...
                     cancellationToken))
        {
            ...
            if (valueOperation == null) continue;
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (tryStatement.Finally?.Block is not { } finallyBlock) continue;
