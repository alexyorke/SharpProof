...
internal static partial class ImpurityCatalog
{
    ...
    private static bool IsStringBuilderSemanticImpure(ISymbol symbol)
    {
        ...
        {
            if (methodSymbol.IsImplicitlyDeclared) return false;

            if (methodSymbol.MethodKind == MethodKind.PropertySet) return true;
    ...
    private static bool IsArrayMutationSemanticImpure(ISymbol symbol)
    {
        ...
        if (methodSymbol.Name != "Sort") return false;
    ...
    private static bool IsComparisonDelegate(ITypeSymbol typeSymbol)
    {
        return typeSymbol is INamedTypeSymbol namedType &&
               ...
               string.Equals(namedType.OriginalDefinition.ToDisplayString(), "System.Comparison<T>",
                   StringComparison.Ordinal);
    }

    private static bool IsThreadingSemanticImpure(ISymbol symbol)
    {
        if (symbol.ContainingType is not INamedTypeSymbol containingType) return false;
    ...
    {
        if (symbol is IPropertySymbol propertySymbol) return !propertySymbol.IsImplicitlyDeclared;
    ...
    private static bool IsXmlLinqSemanticImpure(ISymbol symbol)
    {
        ...
        if (!IsExactType(containingType, "System.Xml.Linq.XElement")) return false;

        if (methodSymbol.IsStatic) return string.Equals(methodSymbol.Name, "Load", StringComparison.Ordinal);
    ...
    private static bool IsDiagnosticsTracingSemanticImpure(ISymbol symbol)
    {
        ...
        if (IsExactType(containingType, "System.Diagnostics.Activity")) return IsActivitySemanticImpure(symbol);
    ...
    private static bool IsIoStreamTextSemanticImpure(ISymbol symbol)
    {
        ...
        if (IsExactType(containingType, "System.IO.Stream"))
            return IsOrdinaryMethodNamed(methodSymbol, "Flush", "Read", "Seek", "Write", "Close", "CopyToAsync",
    ...
    {
        if (!IsAssemblyLoadContextOrDerived(symbol.ContainingType)) return false;
