...
internal partial class MethodInvocationPurityRule : IPurityRule
{
        ...
        PurityAnalysisEngine.PurityAnalysisState currentState)
    {
        ...
        if (TryCheckDoubleDispose(invocationOperation, invokedMethodSymbol, context, currentState,
                out var doubleDisposeResult)) return doubleDisposeResult;

        if (TryCheckUseAfterDispose(invocationOperation, invokedMethodSymbol, context, currentState,
                out var useAfterDisposeResult)) return useAfterDisposeResult;
        ...
        if (TryCheckUnsafeReadUnalignedPurity(invocationOperation, context, currentState,
                out var unsafeReadUnalignedResult)) return unsafeReadUnalignedResult;
        ...
        if (IsLinqEnumerableInvocation(invokedMethodSymbol, context.SemanticModel.Compilation))
        {
                 ...
                 argumentIndex++)
            {
                ...
                if (!delegateTargetResult.IsPure) return delegateTargetResult;
        ...
        if (!ShouldDeferToSpecializedDispatchPurity(invokedMethodSymbol) &&
            DispatchedMemberResolution.IsPotentiallyDispatchedMethod(invokedMethodSymbol,
                  ...
                  && !SymbolicDispatchFacts.IsBaseReference(invocationOperation.Instance)))
        {
            ...
            var knownReceiverType = exactReceiverType ??
                                    GetStableInitializerReceiverType(invocationOperation.Instance, context,
                                        currentState) ??
                                    GetKnownReceiverType(invocationOperation.Instance);
            if (knownReceiverType == null) knownReceiverType = GetKnownStaticInterfaceReceiverType(invokedMethodSymbol);
        ...
        var isImmutableHashSetCreateRangeWithComparer =
            IsImmutableHashSetCreateRangeWithComparer(originalDefinitionSymbol);
        ...
        if (TryCheckEqualityComparerDispatchPurity(invocationOperation, context,
                out var equalityComparerDispatchResult)) return equalityComparerDispatchResult;
        ...
        if (TryCheckCollectionEqualityDispatchPurity(invocationOperation, context, currentState,
                out var collectionEqualityDispatchResult)) return collectionEqualityDispatchResult;

        if (TryCheckMemoryExtensionsDefaultEqualityDispatchPurity(invocationOperation, context,
                out var memoryExtensionsEqualityDispatchResult)) return memoryExtensionsEqualityDispatchResult;
        ...
        if (TryCheckCollectionComparisonDispatchPurity(invocationOperation, context,
                out var collectionComparisonDispatchResult)) return collectionComparisonDispatchResult;
        ...
        if (dispatchWasProvenPure) return PurityAnalysisEngine.PurityAnalysisResult.Pure;
