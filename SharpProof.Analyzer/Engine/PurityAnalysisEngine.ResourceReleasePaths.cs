...
internal partial class PurityAnalysisEngine
{
        ...
        out PurityAnalysisResult result)
    {
        ...
        foreach (var resource in ownedResources)
        {
            ...
            var syntax = containingMethodSymbol.DeclaringSyntaxReferences.FirstOrDefault()
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (syntaxReference.GetSyntax(cancellationToken) is not MethodDeclarationSyntax
                {
                    Body: { } body
        ...
        CancellationToken cancellationToken)
    {
        if (statement is ReturnStatementSyntax returnStatement)
            return new ResourceReleasePathSummary(
                initiallyReleased ||
                IsReturnedSymbol(returnStatement, resourceSymbol, semanticModel, cancellationToken),
        ...
        if (statement is WhileStatementSyntax or ForStatementSyntax or ForEachStatementSyntax
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!hasDefault) fallthroughStates.Add(initiallyReleased);
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var invocation in statement.DescendantNodesAndSelf().OfType<InvocationExpressionSyntax>())
        {
            ...
            if (relatedSymbols.Contains(disposedSymbol)) return true;
        }
        ...
        foreach (var syntaxReference in containingMethodSymbol.DeclaringSyntaxReferences)
        {
            ...
            foreach (var declarator in methodDeclaration.Body.DescendantNodes().OfType<VariableDeclaratorSyntax>())
            {
                ...
                if (declarator.Initializer?.Value == null ||
                    methodSemanticModel.GetDeclaredSymbol(declarator, cancellationToken) is not ILocalSymbol
                        resourceLocal ||
                    methodSemanticModel.GetOperation(declarator.Initializer.Value, cancellationToken) is not
                ...
                var aliases = methodDeclaration.Body.DescendantNodes()
                    ...
                    .Where(aliasDeclarator => aliasDeclarator.SpanStart > declarator.SpanStart &&
                                              ...
                                              methodSemanticModel.GetSymbolInfo(aliasDeclarator.Initializer.Value,
                                                  cancellationToken).Symbol is ILocalSymbol initializerSymbol &&
                                              SymbolEqualityComparer.Default.Equals(initializerSymbol, resourceLocal))
                    .Select(aliasDeclarator =>
                        methodSemanticModel.GetDeclaredSymbol(aliasDeclarator, cancellationToken))
                    .OfType<ILocalSymbol>()
                    .ToArray();
        ...
        CancellationToken cancellationToken)
    {
        foreach (var assignment in searchRoot.DescendantNodes().OfType<AssignmentExpressionSyntax>())
        {
            ...
            if (assignment.SpanStart <= spanStart ||
                semanticModel.GetSymbolInfo(assignment.Left, cancellationToken)
                    .Symbol is not ILocalSymbol assignedLocal ||
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var returnStatement in searchRoot.DescendantNodes().OfType<ReturnStatementSyntax>())
        {
            ...
            if (returnStatement.SpanStart <= spanStart ||
                ...
                semanticModel.GetSymbolInfo(returnStatement.Expression, cancellationToken).Symbol is not
                { } returnedSymbol)
                continue;

            if (symbolSet.Contains(returnedSymbol)) return true;
        }
