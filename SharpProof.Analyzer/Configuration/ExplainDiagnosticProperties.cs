...
namespace SharpProof.Analyzer.Configuration
{
    internal static class ExplainDiagnosticProperties
    {
            ...
            string? impliedConditionText = null)
        {
            if (location != null && location != Location.None && location.IsInSource)
            {
                ...
                if (!string.IsNullOrWhiteSpace(normalizedPath))
                {
                    ...
                    properties = properties
                        ...
                        .SetItem(SharpProofDiagnostics.ExplainQueryProperty, CreateExplainQuery(explainPath, line, column, impliedConditionText));
                }
            }
            ...
            {
                properties = properties.SetItem(SharpProofDiagnostics.ExplainUnknownReasonProperty, normalizedUnknownReason);
            }
        ...
        internal static string? NormalizeToken(string? value)
        {
            ...
            foreach (var character in trimmed)
            {
                ...
                {
                    if (char.IsUpper(character) && lastWasLowerOrDigit && !lastWasSeparator)
                    {
                        builder.Append('_');
                    }
