...
internal static class EffectSummarySymbolKeyFactory
{
    ...
    private static readonly SymbolDisplayFormat EffectSummaryNonGenericContainingTypeFormat = new(
        typeQualificationStyle: SymbolDisplayTypeQualificationStyle.NameAndContainingTypesAndNamespaces,
        ...
        string alternateContainingType)
    {
        ...
        if (string.IsNullOrWhiteSpace(originalContainingType)) return ImmutableArray<string>.Empty;
    ...
    internal static void AddSymbolKey(ImmutableHashSet<string>.Builder keys, string? value)
    {
        if (!string.IsNullOrWhiteSpace(value))
        {
            ...
            foreach (var compatibilityKey in GetMetadataRefKindCompatibilityKeys(trimmed)) keys.Add(compatibilityKey);
        }
    }

    internal static IEnumerable<string> GetMetadataRefKindCompatibilityKeys(string key)
    {
        if (key.Contains("out ", StringComparison.Ordinal)) yield return key.Replace("out ", "ref ");
    ...
    {
        return CreateMetadataSummaryKey(methodSymbol, false, false);
    }

    private static string CreateMetadataExactSummaryKey(IMethodSymbol methodSymbol)
    {
        return CreateMetadataSummaryKey(methodSymbol, true, false);
    }

    private static string CreatePositionalEffectSummaryKey(IMethodSymbol methodSymbol)
    {
        return CreatePositionalSummaryKey(methodSymbol, false);
    }

    private static string CreatePositionalExactSummaryKey(IMethodSymbol methodSymbol)
    {
        return CreatePositionalSummaryKey(methodSymbol, true);
    }

    private static string CreateMetadataPositionalEffectSummaryKey(IMethodSymbol methodSymbol)
    {
        return CreateMetadataSummaryKey(methodSymbol, false, true);
    }

    private static string CreateMetadataPositionalExactSummaryKey(IMethodSymbol methodSymbol)
    {
        return CreateMetadataSummaryKey(methodSymbol, true, true);
    }

    private static string CreateMetadataDefinitionEffectSummaryKey(IMethodSymbol methodSymbol)
    {
        return CreateMetadataDefinitionSummaryKey(methodSymbol, false);
    }

    private static string CreateMetadataDefinitionExactSummaryKey(IMethodSymbol methodSymbol)
    {
        return CreateMetadataDefinitionSummaryKey(methodSymbol, true);
    }

    private static string CreatePositionalSummaryKey(IMethodSymbol methodSymbol, bool includeReturnType)
    {
        var containingTypeName = FormatSummaryType(methodSymbol.ContainingType, true);
        var methodName = methodSymbol.MethodKind == MethodKind.Constructor
            ? ".ctor"
            : methodSymbol.Name;
        var parameterList = string.Join(
            ", ",
            methodSymbol.Parameters.Select(parameter => FormatSummaryParameter(parameter, true)));
        if (!includeReturnType) return containingTypeName + "." + methodName + "(" + parameterList + ")";

        var returnType = methodSymbol.MethodKind == MethodKind.Constructor
            ? "void"
            : FormatSummaryReturnType(methodSymbol, true);
        return containingTypeName + "." + methodName + "(" + parameterList + ")->" + returnType;
    }
        ...
        bool useMetadataTypeNames = false)
    {
        switch (typeSymbol)
        {
            ...
            case ITypeParameterSymbol typeParameter:
                if (!useOrdinalGenericParameters) return typeParameter.Name;
            ...
            case INamedTypeSymbol namedType:
                ...
                if (typeArguments.Length == 0) return typeName;

                var formattedTypeArguments = string.Join(
                    ", ",
                    typeArguments.Select(argument =>
                        FormatSummaryType(argument, useOrdinalGenericParameters, useMetadataTypeNames)));
    ...
    private static void AppendFlattenedTypeArguments(INamedTypeSymbol namedType,
        ImmutableArray<ITypeSymbol>.Builder builder)
    {
        if (namedType.ContainingType != null) AppendFlattenedTypeArguments(namedType.ContainingType, builder);

        foreach (var typeArgument in namedType.TypeArguments) builder.Add(typeArgument);
    }
}