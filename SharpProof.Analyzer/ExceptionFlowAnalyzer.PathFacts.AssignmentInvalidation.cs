...
internal static partial class ExceptionFlowAnalyzer
{
        ...
        List<SmtFormula> facts)
    {
        ...
        foreach (var targetSymbol in targetSymbols) RemoveFactsReferencingSymbol(facts, targetSymbol);

        if (targetSymbols.Count != leftTuple.Arguments.Count ||
            UnwrapFactExpression(assignment.Right) is not TupleExpressionSyntax rightTuple ||
            rightTuple.Arguments.Count != leftTuple.Arguments.Count ||
            rightTuple.Arguments.Any(argument =>
                ExpressionReferencesAnySymbol(argument.Expression, targetSymbols, semanticModel, cancellationToken)))
            return true;
