...
Updated on 2026-09-24. The findings below were audited against baseline `1d96799e6` (`Fix contract semantics, worker ownership, and evidence recovery`). In this working tree, the compound-assignment false-proof, managed exception-region false-proof, completion-analysis recursion-budget and call-graph blowup, rotating-seed fuzz coverage, pilot-validation, managed struct receiver-write, qualification evidence-admission, qualification receipt snapshot-binding, MSBuild published-result invocation binding, advisory attribute-alias activation, B16 pilot-review handoff, B27 solver-incompleteness classification, B67 suppression claim omission, and B74 release-resume findings have been fixed and verified, so they are removed from the active backlog. Proposed fixes for the other findings have not been implemented. The active backlog contains **59 findings**: 0 P0, 0 P1, 21 P2, and 38 P3. Former candidate C1 is now B6; no separate candidate remains in this audit. B18 onward come from a fifth pass on 2026-09-22 that ran a real analyzer built from an unchanged `git archive` of HEAD with SDK 9.0.318 outside the container (the pinned 9.0.316 SDK was not installed).
...
### B73. A resealed return-attribute claim can be rebound to an unrelated call span

**Confidence: Confirmed at source-rebinding validation; downstream proof impact is unproven. Priority: P2.**

- **Location:** `SharpProof.CompilerArtifact/CompilerSourceRebinding.cs`, `IsAttribute`.
- **Observed boundary:** a return-attribute claim for `[return: Positive()]` was moved to the same-length `SideFxOp()` invocation, and its claim authority, manifest hash, and feature-scope fingerprint were updated to match. Serialization and `CompilerSourceRebinding.Validate` accepted the artifact. The current check inspects only the first and last characters of the span; it does not bind the span to an attribute in the owner's source syntax.
- **Impact:** source rebinding accepts a claim whose authority points at an unrelated invocation. Whether this can lead to a false proof through later worker processing has not been demonstrated.
- **Proposed fix:** bind claims to actual attribute-list spans while preserving the compiler-neutral runtime closure required by `RuntimeDependencyTests.WorkerAndLauncherAssembliesHaveCompilerNeutralRuntimeClosures`. A Roslyn-based fix adds a forbidden runtime dependency; a safe dependency-free parser and inactive-preprocessor handling remain unresolved.
- **Proposed regression:** retain the resealed same-length relocation probe with a valid return-attribute control when a compiler-neutral binding approach is available.
