...
### 12. Routine publication-maintenance failures escape as unclassified task exceptions
...
- Affected code:
  ...
  and `SharpProof.BuildTasks/ResetPublishedVerification.cs`, `Execute`
  (approximately lines 18-35).
...
- Expected: each MSBuild task logs a concise error and returns false, consistent
  with its explicit cancellation or failure result.
- Actual impact: invalidation catches only requested cancellation. Reset catches
  `ArgumentException`, `IOException`, and `UnauthorizedAccessException`, but
  misses routine `InvalidOperationException` failures produced by publication
  metadata validation and reset. Those expected paths escape `ITask.Execute`,
  producing generic MSB4018 failures and bypassing task classification.
...
- Regression test: induce invalid local paths, lock timeout, deletion denial,
  and invalid publication metadata in both tasks; assert `Execute` returns false
  with the intended diagnostic and does not throw.
...
- Affected code: `SharpProof.BuildTasks/InvalidatePublishedResult.cs`, `Execute`
  ...
  `SharpProof.BuildTasks/RunVerifier.cs`, `Dispose` and `Cancel` (approximately
  lines 95-99 and 1271-1290), and
  `SharpProof.Worker/Program.cs`, the `Console.CancelKeyPress` handler and cleanup
  (approximately lines 65-108), and `Tools/SharpProof.Fuzz/Program.cs`, its
  top-level `Console.CancelKeyPress` handler and cancellation-source lifetime.
...
- Actual impact: both paths can call `Cancel` on a disposed source and throw
  `ObjectDisposedException` from the cancellation callback/thread.
  `RunVerifier.Dispose` is also unsynchronized with an already-entered `Cancel`:
  it can dispose the cancellation signal or process while `Cancel` is setting or
  terminating it. The fuzz executable never unregisters its anonymous console
  handler before the top-level `using` disposes the source. Removing an event
  handler or leaving `Execute` prevents new callbacks but does not wait for one
  already running.
...
### 43. Unescaped path syntax is reinterpreted by MSBuild item processing
...
- Affected code:
  ...
  153).
- Normal trigger: a supported Linux project, intermediate path, or compiler
  output path contains a legal MSBuild item-syntax character such as `;`, `*`,
  `?`, or a metadata-expression-shaped sequence.
...
- Actual impact: the target passes the raw scalar to list-valued `Directories`
  ...
  separators and wildcard syntax as file expansion, so directory operations and
  compiler-output alias validation operate on fragments or an expanded file set
  instead of the validated literal path. Cleanup first validates the unsplit
  scalar and then reparses it as a list. This is distinct from bug 38:
  apostrophes break expression parsing, while these paths parse successfully and
  are reinterpreted as item syntax.
...
- Regression test: build and clean projects beneath directories containing `;`,
  `*`, `?`, and metadata-shaped text; assert one exact invocation directory is
  created/removed and compiler outputs arrive at invalidation as literal,
  unexpanded paths.
...
### 77. Requires reachability misses nested positional and list-pattern captures
...
- Actual: whole-input designation handles direct, parenthesized, and binary
  ...
  `RecursivePatternSyntax` subpatterns and has no `ListPatternSyntax` case at
  all. The is-pattern operation is treated as a nonexecuting observation, so
  the invocation loses its reachable source and a diagnostic such as `SP0027`
  is omitted.
...
### 79. Conditional helpers and nested conversions evade cache-soundness diagnostics
...
- Normal trigger: a helper returns a conditional expression selecting
  `Answer.Unknown` or `Answer.Proven`, or a noncacheable semantic answer is
  wrapped in two or more parentheses/conversions, and the result is written to
  a proof cache.
...
- Actual: returned-value extraction handles member access, identifiers, and
  ...
  only the helper method name, so the unknown branch is missed. Separately,
  `IsNonCacheableSemanticAnswer` unwraps only one conversion or parenthesized
  operation before switching on the result, so a second wrapper can hide the
  same forbidden answer.
- Evidence confidence: High from the finite returned-expression cases and the
  cache-write data flow.
- Recommended fix: recursively unwrap harmless conversions/parentheses and
  inspect every return branch of conditional and switch expressions.
- Regression test: cover helpers with unknown values in either conditional
  branch and in switch arms, plus answers under multiple nested wrappers.
...
### 88. Lexical parent-segment collapse bypasses symlink-prefix validation

- Severity: Medium
- Affected code: `SharpProof.Host/LinuxPathIdentity.cs`, `Canonicalize`
  (approximately lines 48-102).
- Normal trigger: a configured Linux path contains an existing symbolic-link
  segment followed by `..`, for example `/workspace/link/../result.json`.
- Expected: path identity either follows ordinary POSIX component resolution
  when determining the requested target or rejects the input because its
  component walk traverses a symbolic link.
- Actual: `Path.GetFullPath` lexically removes `link/..` before the `lstat`
  prefix walk begins. The symlink segment is never inspected, and the returned
  path names `/workspace/result.json`; ordinary POSIX resolution of the original
  input can name a different file based on the symlink target. Publication,
  validation, or cleanup can therefore operate on a silently redirected local
  path while the no-symlink check reports success.
- Evidence confidence: High from operation ordering and POSIX component
  semantics; static only.
- Recommended fix: reject `.` and `..` components before lexical collapse, or
  perform an fd-relative, no-follow component walk that preserves original
  component order and returns the verified identity.
- Regression test: create a symlink whose target has a different parent, pass a
  `link/../file` path, and assert canonicalization rejects it rather than
  returning the lexically collapsed sibling.

### 89. Member initializers are analyzed against only one constructor

- Severity: High
- Affected code: `SharpProof.Analyzer.Core/AnalyzerFeaturePipeline.cs`,
  `AnalyzeMemberInitializer` (approximately lines 435-512).
- Normal trigger: a type has two unsuppressed instance constructors whose
  preconditions establish different facts, and a field or property initializer
  calls a method with a requirement that only the first constructor proves.
- Expected: because the initializer executes for every applicable constructor,
  its calls are checked under every unsuppressed constructor contract; a
  requirement not established on any path produces the corresponding
  diagnostic.
- Actual: constructors are ordered, suppression is skipped, and the first
  remaining constructor is selected with `break`. The initializer is analyzed
  once using that constructor as its caller, and only that constructor receives
  the semantic outcome. A later constructor with weaker or incompatible entry
  facts is never checked, so its reachable requirement violation can be missed.
- Evidence confidence: High from the constructor-selection loop and the caller-
  specific `RequiresCallSiteAnalyzer.Analysis` binding.
- Recommended fix: analyze the initializer for every applicable unsuppressed
  constructor, combine outcomes deterministically, and deduplicate identical
  source diagnostics without discarding a refuting constructor path.
- Regression test: use two constructors where only the first proves an
  initializer call's requirement and assert the second path still reports the
  violation; repeat with different suppression combinations.

### 90. API-spec public-key-token casing is accepted but cannot match

- Severity: Low
- Affected code: `SharpProof.Specs/ApiSpecTable.cs`, declaration validation;
  `SharpProof.Specs/ApiSpecContentDigest.cs`, assembly digesting; and
  `SharpProof.CompilerCollector/CompilerArtifact/CompilerSpecificationPackProvider.cs`,
  `MatchesAssembly`.
- Normal trigger: a public `ApiSpecAssemblyIdentity` uses a valid 16-character
  hexadecimal public-key token containing uppercase `A-F`.
- Expected: either validation requires one canonical token spelling or matching
  treats hexadecimal token text case-insensitively, consistently with the
  content digest.
- Actual: declaration validation accepts either case, and content hashing
  uppercases tokens, but compiler matching renders the assembly token in
  lowercase and compares it to the declaration with ordinal equality. An
  accepted uppercase declaration therefore has the same content identity as
  its lowercase form but never matches the intended assembly, silently
  disabling that specification.
- Evidence confidence: High from the validator, digest normalization, and
  matching comparison.
- Recommended fix: normalize tokens to lowercase at declaration construction or
  validation and use that canonical value for uniqueness, hashing, and matching.
- Regression test: construct otherwise identical upper- and lowercase approved
  identities and assert they canonicalize identically and both resolve the same
  assembly, or assert the uppercase spelling is rejected explicitly.

### 91. Native-payload preparation reuses partial downloads and shared extraction state

- Severity: Medium
- Affected code: `eng/container/Prepare-NativePayload.ps1`, archive download and
  extraction setup (approximately lines 31-54).
- Normal trigger: a Z3 download is interrupted after creating the archive file,
  or two local Docker builds prepare the same version concurrently under the
  default temporary download root.
- Expected: each attempt either publishes one digest-verified archive and
  isolated extraction tree or fails without poisoning a later retry or racing
  another build.
- Actual: `Invoke-WebRequest` writes directly to the final archive path without
  an explicit timeout or retry. A later invocation skips any existing file and
  fails its hash instead of replacing a partial download. All invocations also
  share one version-named extraction directory, which they recursively remove
  and recreate, so concurrent builds can delete or overwrite one another's
  extraction state.
- Evidence confidence: High from the existence guard, direct `-OutFile`, and
  fixed extraction path.
- Recommended fix: download to a unique temporary file with a bounded timeout
  and retry policy, verify its digest, then atomically publish it; use a unique
  extraction directory per invocation and remove only that directory.
- Regression test: interrupt a fake download after writing a prefix and verify
  the next run replaces it successfully; run two preparations concurrently and
  assert both produce the exact catalog-owned payload.

### 92. Differential-oracle assemblies remain loaded for the process lifetime

- Severity: Low
- Affected code: `SharpProof.Testing/IrCSharpDifferentialOracle.cs`, `Compare`
  (approximately lines 40-82).
- Normal trigger: run a large or repeatedly extended C#/IR differential matrix
  in one test process.
- Expected: generated executable probes are unloaded after comparison so memory
  use is bounded by active cases rather than cumulative case count.
- Actual: every successful comparison emits a new assembly image and loads it
  with `Assembly.Load(byte[])`. The load is not associated with a collectible
  `AssemblyLoadContext`, so generated assemblies and their metadata remain for
  the process lifetime. The current small matrix is unlikely to exhaust memory,
  but longer property or fuzz runs accumulate monotonically.
- Evidence confidence: High for lifetime retention; the original report's OOM
  impact at only 200 cases was overstated and is not retained.
- Recommended fix: load each probe through a collectible
  `AssemblyLoadContext`, release reflection objects, call `Unload`, and structure
  tests so collection can be observed outside the loading method's stack.
- Regression test: execute many unique probes, retain weak references to their
  collectible contexts, force collection after each batch, and assert the
  contexts become unreachable while comparisons remain identical.

### 93. Release-authority closure omits implicit build and version inputs

- Severity: High
- Affected code: `scripts/Get-SharpProofReleaseAuthorityClosure.ps1` and
  `eng/acceptance/contract.json`, `releaseAuthorityClosure.paths`.
- Normal trigger: change an implicit release input such as `global.json`,
  `NuGet.Config`, `Directory.Packages.props`, `Directory.Build.props`,
  `SharpProof.Release.props`, or `SharpProof.PackageMetadata.props` without
  changing a path discovered by the closure walker.
- Expected: the release-authority identity changes whenever an input capable of
  changing SDK selection, dependency resolution, compilation, package version,
  or package metadata changes.
- Actual: the walker starts from selected scripts/workflows and follows only
  recognized literal path references. MSBuild and NuGet implicitly import the
  omitted central files, so they can change released bytes or metadata while
  the declared closure remains unchanged and its equality test still passes.
- Evidence confidence: High from the declared path set, walker grammar, and
  standard implicit imports used by the release commands.
- Recommended fix: seed the closure with all implicit SDK/MSBuild/NuGet and
  package-version authorities, then retain reference walking for explicit
  script inputs. Document why every root is authoritative.
- Regression test: mutate each central input independently in a fixture and
  assert the derived closure identity changes and the release-authority gate
  rejects the stale declaration.

### 94. Tracked OpenCode plugin imports an undeclared package

- Severity: Low
- Affected code: `.opencode/plugins/oh-my-goal.js`, `opencode.json`, and the
  generated `.opencode/package.json` dependency manifest.
- Normal trigger: start OpenCode from a fresh checkout without a separately
  installed global or parent copy of `oh-my-goal`.
- Expected: every tracked local plugin import resolves from the repository's
  declared OpenCode dependencies.
- Actual: `opencode.json` loads the local plugin, which re-exports the bare
  package `oh-my-goal`, while the dependency manifest declares only
  `@opencode-ai/plugin`. Normal module resolution therefore has no repository-
  owned package to load and startup fails with a module-resolution error.
- Evidence confidence: High from the tracked import/configuration and local
  dependency manifest.
- Recommended fix: declare and lock the intended `oh-my-goal` package version,
  or remove the local re-export and configure a supported installed plugin
  directly.
- Regression test: install only declared dependencies in a clean temporary
  checkout and load every configured OpenCode plugin successfully.

### 95. Partial-term SMT fuzz failures are not minimized

- Severity: Low
- Affected code: `Tools/SharpProof.Fuzz/FuzzRunner.cs`, the
  `partial-term-smt` branch that materializes retained failures (approximately
  lines 320-340).
- Normal trigger: the partial-term SMT differential oracle reports a mismatch.
- Expected: as with frontend and finite-domain SMT mismatches, the retained
  failure contains a minimized reproducer so that its reported `Minimized`
  field and the detail generated from it describe a reduced failing case.
- Actual: the branch reruns the original case and writes the same printed
  formula to both `Original` and `Minimized`. The adjacent frontend branch uses
  `CSharpStructuralShrinker.Minimize`, and the finite-domain branch uses
  `IrStructuralShrinker.MinimizeAsync`; only the partial-term oracle omits the
  promised reduction. This makes its failure artifact materially harder to
  diagnose and can leave the detail tied only to a large unreduced formula.
- Evidence confidence: High from the three side-by-side failure-materialization
  branches.
- Recommended fix: add a partial-term case shrinker, or adapt the IR structural
  shrinker while preserving at least one concrete scenario that reproduces the
  mismatch, then rerun the oracle on that minimized case.
- Regression test: inject a reducible partial-term mismatch and assert the
  retained failure's minimized formula is smaller, still mismatches, and drives
  the retained detail.

### 96. The dogfood script explicitly disables the analyzer it loads

- Severity: Medium
- Affected code: `scripts/Invoke-SharpProofDogfood.ps1`, the per-project build
  invocation (approximately lines 48-59), together with
  `SharpProof.Analyzer.Core/SharpProofAnalyzerEngine.cs`.
- Normal trigger: run the repository's dogfood script.
- Expected: `SharpProofDogfood=true` loads the freshly built analyzer and runs
  its advisory diagnostics over each selected repository project.
- Actual: the same command passes `/p:SharpProofProfile=off`. The analyzer
  engine exits immediately for the Off profile, so the script verifies that
  the analyzer payload can be loaded but performs no SharpProof analysis. A
  repository regression can therefore pass the advertised dogfood gate without
  any dogfood diagnostics executing.
- Evidence confidence: High from the script arguments and the analyzer's first
  profile gate.
- Recommended fix: run with the advisory profile (and the intended feature set)
  while retaining the explicit dogfood analyzer injection.
- Regression test: add a temporary selected project containing a deterministic
  advisory diagnostic and assert the dogfood command reports it.

### 97. Spec-result rewriting stops at sequence accesses and opaque calls

- Severity: Medium
- Affected code: `SharpProof.Worker/SpecResultDomainProjection.cs`, `Rewrite`
  and its local `Visit` switch (approximately lines 78-129).
- Normal trigger: a path condition, completion condition, or postcondition
  nests a projected API-spec result fact inside an `IrSequenceAccessTerm` index
  or an `IrOpaqueTerm` receiver or argument. For example, an array index can use
  the length of a sequence returned by a specified call.
- Expected: the recursive projection walk replaces every nested nullness and
  length use of a projected result, independent of the enclosing IR term kind.
- Actual: the switch rebuilds unary, binary, conditional, cast, and length
  terms, but returns every sequence-access and opaque term unchanged. Their
  children are never visited, leaving original result facts in predicates even
  though the worker introduced proxy variables for those facts. This can turn a
  supported obligation into an avoidable unknown or prove against the wrong
  unprojected expression.
- Evidence confidence: High from the incomplete recursive switch and the
  repository's complete child map in `SharpProof.Ir/IrTraversal.cs`.
- Recommended fix: rebuild both missing composite term kinds after recursively
  visiting their receiver/sequence, index, and argument children; keep opaque
  operation identity and purity unchanged.
- Regression test: place a projected `Length(result)` beneath a sequence index
  and beneath an opaque argument, then assert both are rewritten to the length
  proxy.

### 98. IR member interning ignores the member name

- Severity: Low
- Affected code: `SharpProof.Ir/IrFactory.cs`, `GetOrCreateMember`
  (approximately lines 162-191).
- Normal trigger: callers intern two member descriptions with the same identity,
  declaring type, return type, static flag, and parameter types but different
  names. The public factory API accepts all of those fields independently.
- Expected: fields observable through `GetMemberInfo` participate in the
  structural interning key, or inconsistent duplicate descriptions are
  rejected.
- Actual: the method interns `name` into `nameId` but omits that ID from the
  `StructuralKey`. The second call returns the first member ID, and
  `GetMemberInfo` exposes the first name rather than the supplied second name.
- Evidence confidence: High from the constructed key and returned member info.
- Recommended fix: include `nameId.Value` in the structural key, or make the
  identity authoritative and reject a conflicting name explicitly.
- Regression test: intern otherwise-identical members with two names and assert
  either distinct IDs or a documented argument exception, never silent reuse.

### 99. Atomic-file staging can make a valid destination filename too long

- Severity: Medium
- Affected code: `SharpProof.Ir/AtomicFile.cs`, `Prepare` (approximately lines
  115-122), and all worker, cache, and artifact writers that use it.
- Normal trigger: publish to an existing valid destination whose final path
  component is close to the filesystem's per-component limit.
- Expected: atomic staging works for every destination filename the supported
  local filesystem admits.
- Actual: the temporary filename is the complete destination filename plus a
  dot, 32 hexadecimal GUID characters, and `.tmp`. On a filesystem with the
  common 255-byte `NAME_MAX`, a valid long destination component therefore
  becomes an invalid staging component and fails before any bytes are written.
- Evidence confidence: High from the filename construction and ordinary Linux
  component-length semantics.
- Recommended fix: use a bounded hidden staging leaf in the destination
  directory, derived from a hash or truncated destination prefix plus unique
  suffix, without appending to the complete leaf.
- Regression test: publish to a valid destination component near `NAME_MAX` in
  the canonical container and assert replacement and cleanup succeed.

### 100. Normal-completion analysis misses expression-bodied properties and indexers

- Severity: Low
- Affected code: `SharpProof.Effects/ManagedAbstractFlow.cs`,
  `DefiniteOperationFacts.MethodCanCompleteNormally` and `GetBody`
  (approximately lines 1918-1951 and 2429-2437).
- Normal trigger: effect reachability depends on a source property or indexer
  accessor whose expression body never completes normally, such as an
  expression-bodied getter that always throws or calls a known diverging
  method.
- Expected: the accessor body participates in the same conservative
  normal-completion analysis as expression-bodied methods, accessors, and local
  functions.
- Actual: when Roslyn does not return an operation for the declaration itself,
  `GetBody` handles only `BaseMethodDeclarationSyntax`, explicit
  `AccessorDeclarationSyntax`, and local functions. It omits expression-bodied
  `BasePropertyDeclarationSyntax`, so the missing operation is treated as able
  to complete and later source-order effects remain spuriously reachable.
- Evidence confidence: Medium-High from the syntax fallback and its deliberately
  permissive `operation == null` result.
- Recommended fix: return the expression body for properties and indexers (and
  bind the relevant accessor symbol), preserving permissive fallback only for
  genuinely unsupported syntax.
- Regression test: analyze expression-bodied throwing property and indexer
  getters followed by an effect and assert the later effect is unreachable.

### 101. Published analyzer release metadata omits the ContractFor diagnostics

- Severity: Low
- Affected code: `SharpProof.Analyzer/AnalyzerReleases.Unshipped.md`,
  `SharpProof.Analyzer/SharpProofAnalyzer.cs`, and
  `SharpProof.Analyzer.Core/ContractForValidation/ContractForDiagnosticDescriptors.generated.cs`.
- Normal trigger: inspect or validate the diagnostic release inventory for the
  packaged analyzer.
- Expected: every descriptor exposed by `SharpProofAnalyzer.SupportedDiagnostics`
  appears in the shipped or unshipped analyzer release metadata.
- Actual: the analyzer exposes ContractFor diagnostics `SPCF0001` through
  `SPCF0008`, but the unshipped table lists only `SP` diagnostics and the
  shipped table does not supply those eight IDs. Because the descriptors are
  compiled in the referenced Analyzer.Core assembly, the build's release-
  tracking analyzer does not diagnose the omission in the outer analyzer
  project.
- Evidence confidence: High from the generated descriptor inventory, public
  `SupportedDiagnostics` delegation, and both release files.
- Recommended fix: add all eight diagnostics with their actual categories,
  default severities, and notes to the unshipped release table.
- Regression test: compare the packaged analyzer's supported IDs with the union
  of shipped and unshipped release metadata.

### 102. Generated-code detection misses the conventional non-self-closing header

- Severity: Low
- Affected code: `SharpProof.Analyzer.Core/AnalyzerGeneratedCodePolicy.cs`,
  `GeneratedHeaderTokens` and `IsExactGeneratedHeader`.
- Normal trigger: a generated C# file has an ordinary first-line
  `// <auto-generated>` marker, without a self-closing slash, and a filename
  that lacks one of the recognized generated suffixes.
- Expected: the analyzer's generated-code exclusion recognizes conventional
  auto-generated headers consistently with Roslyn tooling.
- Actual: the exact token list accepts only self-closing forms such as
  `<auto-generated/>` and `<auto-generated />` (plus `autogenerated`
  variants). The conventional non-self-closing form is treated as source code,
  so SharpProof diagnostics and manifest selection can run on code the producer
  marked generated.
- Evidence confidence: High from the exact token comparison and missing test
  case.
- Recommended fix: accept the conventional opening form using the same bounded
  first-comment policy, without broad substring matching.
- Regression test: cover both single-line and block-comment non-self-closing
  headers, plus near-miss text that must remain ordinary source.

### 103. Deconstruction assignment targets are treated as delegate consumption

- Severity: Medium
- Affected code: `SharpProof.Analyzer.Core/RequiresCallSiteTreeAnalyzer.cs`,
  the local-reference walk and `IsAssignmentTarget` helper (approximately lines
  589-632 and 1267-1276).
- Normal trigger: a tracked local or tuple component containing a delegate is
  overwritten by a deconstruction assignment, rather than introduced by a
  deconstruction declaration.
- Expected: references on the left of a deconstruction assignment are writes;
  they kill the previous tracked value at the assignment commit and do not make
  that previous delegate reachable.
- Actual: `IsAssignmentTarget` recognizes only a
  `SimpleAssignmentExpressionSyntax` ancestor, and the follow-up requires an
  enclosing `ISimpleAssignmentOperation`. A target beneath an
  `IDeconstructionAssignmentOperation` falls through as a read/consumption, so
  the overwritten delegate can be propagated as reachable and produce a false
  call-site precondition diagnostic.
- Evidence confidence: High from the syntax/operation restriction and the
  separate existing coverage for deconstruction declarations, which does not
  exercise assignment to existing locals.
- Recommended fix: recognize deconstruction targets and determine their tuple
  path and commit point from `IDeconstructionAssignmentOperation`, preserving
  the existing RHS-before-commit exception handling.
- Regression test: initialize an existing delegate local with a failing nested
  function, overwrite it through flat and nested deconstruction assignments,
  and assert the old function is not considered reachable; retain a case that
  invokes the newly assigned delegate.
...
claims that every confirmed entry was remediated in this pass. The numeric gaps
24, 37, and 58 are intentionally retained so historical bug references remain
stable after those invalid entries were removed.
