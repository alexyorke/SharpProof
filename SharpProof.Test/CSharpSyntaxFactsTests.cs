...
public class CSharpSyntaxFactsTests {
    ...
    public void TryGetCompoundAssignmentBinaryKind_RejectsSimpleAssignment() {
        Assert.That(CSharpSyntaxFacts.TryGetCompoundAssignmentBinaryKind(SyntaxKind.SimpleAssignmentExpression, out var binaryKind),
            Is.False);
    ...
    public void TryGetIncrementOrDecrementOperand_RejectsNonMutation()
        => Assert.That(CSharpSyntaxFacts.TryGetIncrementOrDecrementOperand(SyntaxFactory.ParseExpression("value + 1"), out _,
        out _), Is.False);
    [Test]
    public void GetContainingExecutionRoot_ExtendedPolicy_SelectsNearestRequestedBoundary() {
        ...
        Assert.That(CSharpSyntaxFacts.GetContainingExecutionRoot(binaryExpressions[0], ExtendedExecutionRoots),
            Is.TypeOf<EqualsValueClauseSyntax>());
        Assert.That(CSharpSyntaxFacts.GetContainingExecutionRoot(binaryExpressions[1], ExtendedExecutionRoots),
            Is.TypeOf<PropertyDeclarationSyntax>());
        Assert.That(CSharpSyntaxFacts.GetContainingExecutionRoot(binaryExpressions[2], ExtendedExecutionRoots),
    ...
    public void MethodBodyOperationResolver_SelectsSharedBodyOrExpressionTaxonomy(string source, SyntaxKind declarationKind,
    ...
    public void MethodBodyOperationResolver_RetainsDeclarationFallback(string source, SyntaxKind declarationKind,
        bool includeConversionOperators) {
        var semanticModel = CreateSemanticModel(source, out var root);
        var declaration = root.DescendantNodes().Single(node => node.IsKind(declarationKind));
        var expected = semanticModel.GetOperation(declaration);

        var actual = MethodBodyOperationResolver.GetMethodBodyRootOperation(declaration, semanticModel, CancellationToken.None,
