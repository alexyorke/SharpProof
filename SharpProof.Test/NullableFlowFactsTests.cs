...
public sealed class ContractFixture
{
    ...
    public static void Reads()
    {
        ...
        Assert.That(
            NullableFlowFacts.GetParameterInputState(inputs.Parameters[0]),
            Is.EqualTo(NullableFlowFactState.MaybeNull));
        Assert.That(
            NullableFlowFacts.GetParameterInputState(inputs.Parameters[1]),
            Is.EqualTo(NullableFlowFactState.NotNull));

        var tryRead = fixture.GetMembers("TryRead").OfType<IMethodSymbol>().Single();
        Assert.That(
            NullableFlowFacts.GetParameterOutputState(tryRead.Parameters[0], false),
            Is.EqualTo(NullableFlowFactState.MaybeNull));
        Assert.That(
            NullableFlowFacts.GetParameterOutputState(tryRead.Parameters[0], true),
            Is.EqualTo(NullableFlowFactState.NotNull));
        Assert.That(
            NullableFlowFacts.GetParameterOutputState(tryRead.Parameters[1], true),
            Is.EqualTo(NullableFlowFactState.NotNull));
        Assert.That(
            NullableFlowFacts.GetParameterOutputState(tryRead.Parameters[1], false),
            Is.EqualTo(NullableFlowFactState.Unknown));
        ...
        Assert.That(
            NullableFlowFacts.GetMethodReturnState(maybe),
            Is.EqualTo(NullableFlowFactState.MaybeNull));
        Assert.That(
            NullableFlowFacts.GetMethodReturnState(always),
            Is.EqualTo(NullableFlowFactState.NotNull));

        var echo = fixture.GetMembers("Echo").OfType<IMethodSymbol>().Single();
        Assert.That(
            NullableFlowFacts.TryGetNotNullIfNotNullParameterName(echo, out var parameterName),
            Is.True);
        ...
        Assert.That(
            ...
            Is.EquivalentTo(new[] { "_value" }));
        Assert.That(
            NullableFlowFacts.TryResolveInstanceMemberTarget(fixture, "_value", out var member),
            Is.True);
        ...
        Assert.That(
            NullableFlowFacts.TryGetDoesNotReturnIfValue(throwIf.Parameters[0], out var doesNotReturnIf),
            Is.True);
        ...
        var flowValue = root.DescendantNodes()
            ...
            .Expression;
        Assert.That(
            NullableFlowFacts.GetExpressionState(flowValue, semanticModel, CancellationToken.None),
            Is.EqualTo(NullableFlowFactState.NotNull));
        ...
        Assert.That(
            NullableFlowFacts.GetExpressionState(invocations["Always"], semanticModel, CancellationToken.None),
            Is.EqualTo(NullableFlowFactState.NotNull));
        Assert.That(
            NullableFlowFacts.GetExpressionState(invocations["Maybe"], semanticModel, CancellationToken.None),
            Is.EqualTo(NullableFlowFactState.MaybeNull));
    }
