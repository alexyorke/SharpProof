...
public sealed class UnknownContractDiagnosticTests {
    ...
    [Test]
    public async Task InterfaceZeroAllocationsContractDoesNotVerifyMissingBody() {
        var diagnosticIds = (await AnalyzerTestHost.GetDiagnosticsAsync("""
            using SharpProof.Attributes;
            public interface IWorker {
                [ZeroAllocations]
    ...
    [Test]
    public async Task InterfacePurityContractDoesNotVerifyMissingBody() {
        var diagnosticIds = (await AnalyzerTestHost.GetDiagnosticsAsync("""
            using SharpProof.Attributes;
            public interface IWorker {
                [EnforcePure]
    ...
    [Test]
    public async Task EnforcePureAutoPropertyGetterDoesNotReportViolation() {
        var diagnosticIds = (await AnalyzerTestHost.GetDiagnosticsAsync("""
            using SharpProof.Attributes;
    ...
    [Test]
    public async Task ZeroAllocationsAutoPropertyGetterDoesNotReportViolation() {
        var diagnosticIds = (await AnalyzerTestHost.GetDiagnosticsAsync("""
            using SharpProof.Attributes;
            public sealed class Worker {
                [ZeroAllocations]
    ...
    [Test]
    public async Task CapabilityRestrictedAutoPropertyGetterDoesNotReportViolation() {
        var diagnosticIds = (await AnalyzerTestHost.GetDiagnosticsAsync("""
            using SharpProof.Attributes;
            public sealed class Worker {
                [AllowedCapabilities(SharpProofCapability.None)]
            ...
            """)).Select(static diagnostic => diagnostic.Id);
        Assert.Multiple(() => {
            Assert.That(diagnosticIds, Does.Not.Contain("SP0015"));
            Assert.That(diagnosticIds, Does.Not.Contain("SP0016"));
        });
    }
    [Test]
