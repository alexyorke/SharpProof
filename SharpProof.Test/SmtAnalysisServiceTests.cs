...
public class SmtAnalysisServiceTests {
    ...
    [Test]
    public void DefaultPreset_DisablesSharedResultCache() => Assert.That(SmtAnalysisOptions.Default.UseSharedResultCache, Is.False);
    [Test]
    public void WithOverrides_PreservesModeAndAppliesExplicitBudgets() {
        var options = SmtAnalysisOptions.ForMode(SmtAnalysisMode.Deep).WithOverrides(TimeSpan.FromMilliseconds(123),
            TimeSpan.FromMilliseconds(456), 7, 89);
    ...
    public void Classify_WrappedNativeFailures_PreserveStableFallbackCodes() {
        AssertPermanentFailureCode(
            new TypeInitializationException("Microsoft.Z3.Native", new DllNotFoundException("missing test solver")),
            "smt_native_library_missing");
        AssertPermanentFailureCode(
            new TypeInitializationException("Microsoft.Z3.Native", new BadImageFormatException("incompatible test solver")),
            "smt_native_library_incompatible");
        AssertPermanentFailureCode(
            new TypeInitializationException("Microsoft.Z3.Native", new PlatformNotSupportedException("unsupported test platform")),
    ...
    public void Dispose_DefaultLifecycle_DisposesCurrentThreadContext() {
        ...
        var service = new SmtAnalysisService(
            SmtAnalysisOptions.Default,
            () => new StubProofSearchSession((_, _) => CreateImpureResult(), () => Interlocked.Increment(ref disposedSessions)));
    ...
    public void Dispose_ConfiguredLifecycle_DisposesContextsCreatedOnAllThreads() {
        ...
        var threads = Enumerable.Range(0, threadCount)
            .Select(index => new Thread(() => service.Classify(CreateSolverQuery("thread_owned_context_" + index))))
            .ToArray();
    ...
    public void Classify_PathConditionBudgetExceeded_ReturnsConservativeUnknownWithoutSolver() {
        ...
        var service = new SmtAnalysisService(new SmtAnalysisOptions(SmtAnalysisMode.Bounded, TimeSpan.FromMilliseconds(50),
            TimeSpan.FromMilliseconds(500), 1, 32));

        var result = service.Classify(CreateQuery(new SmtFormula[] { GreaterThanOrEqual(x, Integer(0)), LessThan(x, Integer(10)) },
            Boolean(true)));
    ...
    public void Classify_ZeroTimeout_ReturnsConservativeTimeoutWithoutSolver() {
        ...
        var service = new SmtAnalysisService(new SmtAnalysisOptions(SmtAnalysisMode.Bounded, TimeSpan.Zero, TimeSpan.FromMilliseconds(500),
            4, 32));
    ...
    [TestCase(true, 2, TestName = "Classify_DuplicateAndTruePathConditions_AreNormalizedBeforeBudgetAndCache")]
    [TestCase(false, 4, TestName = "Classify_EquivalentPathConditionOrder_UsesSameCacheEntry")]
    public void Classify_NormalizedPathConditionsUseSameCacheEntry(bool includeDuplicateAndTrueConditions, int maxPathConditions) {
        ...
        var fact = new SmtBinaryFormula(SmtBinaryOperator.GreaterThanOrEqual, new SmtIntegerBinaryTerm(SmtIntegerBinaryOperator.Add, x, y),
            Integer(0));
        var service = new SmtAnalysisService(new SmtAnalysisOptions(SmtAnalysisMode.Bounded, TimeSpan.FromSeconds(2),
            TimeSpan.FromMilliseconds(1000), maxPathConditions, 64));

        var firstPath = includeDuplicateAndTrueConditions
            ? new SmtFormula[] { xAtLeastZero, yAtLeastZero, Boolean(true), xAtLeastZero }
            : [xAtLeastZero, yAtLeastZero];
    ...
    public void SyntacticImplicationMatrix(string kind) {
        ...
        if (kind == "direct") {
            ...
            conclusion = kind switch {
                "length" => new SmtBinaryFormula(SmtBinaryOperator.GreaterThanOrEqual, new SmtStringLengthTerm(text), Integer(3)),
    ...
    public void DivideByZeroContradictionMatrix(bool positiveInterval) {
        ...
        var result = service.Classify(new AnalysisProofQuery(new[] { guard }, new AnalysisHazard(AnalysisHazardKind.DivideByZero,
            divisorIsZero)));
    ...
    public void Classify_ExpressionNodeBudgetExceeded_ReturnsConservativeUnknownWithoutSolver() {
        ...
        var service = new SmtAnalysisService(new SmtAnalysisOptions(SmtAnalysisMode.Bounded, TimeSpan.FromMilliseconds(50),
            TimeSpan.FromMilliseconds(500), 4, 3));
    ...
    public void Classify_DeepPathFormulaOverBudget_ReturnsBeforeNormalization() {
        var service = new SmtAnalysisService(new SmtAnalysisOptions(SmtAnalysisMode.Bounded, TimeSpan.FromMilliseconds(50),
            TimeSpan.FromMilliseconds(500), 4, 128));
    ...
    public async Task Classify_MethodBudgetDoesNotExpireBeforeFirstSolverQueryByWallClock() {
        ...
        var service = new SmtAnalysisService(new SmtAnalysisOptions(SmtAnalysisMode.Bounded, TimeSpan.FromSeconds(2),
            TimeSpan.FromMilliseconds(1), 4, 32));
    ...
    public void Classify_CacheHitsBypassExhaustedMethodBudget() {
        ...
        var options = new SmtAnalysisOptions(SmtAnalysisMode.Bounded, TimeSpan.FromSeconds(2), TimeSpan.FromTicks(1), 4, 32, true);
    ...
    public void Classify_SharedResultCacheEnabled_ReusesResultAcrossServices() {
        ...
        var x = Int(variableName);
        var xIsZero = Equal(x, Integer(0));
        var options = new SmtAnalysisOptions(SmtAnalysisMode.Bounded, TimeSpan.FromSeconds(2), TimeSpan.FromMilliseconds(1000), 4,
            32, true);
        var query = CreateQuery(new[] { xIsZero }, xIsZero);
        using var firstService = new SmtAnalysisService(options);
        using var secondService = new SmtAnalysisService(options);
    ...
    public void SymbolicBudgetDiagnostics_LargeBudgetsClampMilliseconds() {
        ...
        var budgetFact = SymbolicFact.Exact(
            ...
            "test.budget");
        var proof = new SymbolicProofService(smtAnalysis).ClassifyReachability(new SymbolicState(pathConditions:
            new[] { new SymbolicFactCondition(budgetFact) }));
    ...
    private static IEnumerable<TestCaseData> ServiceRegexCases() {
        yield return CreateServiceRegexCase("ClassifyImplication_ProvesStrictRegexLiteralLengthFact", @"\A[A-Z][0-9]\z", null, true, 2,
            AnalysisProofOutcome.Proven, Feasibility.Unsatisfiable);
        ...
        yield return CreateServiceRegexCase("ClassifyPathFeasibility_CombinesStrictRegexAndStringEquality", @"\AAB\z", "AB", false, null,
            AnalysisProofOutcome.Proven, Feasibility.Unsatisfiable);
        yield return CreateServiceRegexCase("ClassifyPathFeasibility_CombinesNonCapturingRegexGroupAndStringEquality", @"\A(?:AB|CD)\z",
            "EF", true, null, AnalysisProofOutcome.Proven, Feasibility.Unsatisfiable);
        yield return CreateServiceRegexCase("ClassifyPathFeasibility_CombinesNegatedRegexClassAndStringEquality", @"\A[^A]\z", "A", true,
            null, AnalysisProofOutcome.Proven, Feasibility.Unsatisfiable);
        yield return CreateServiceRegexCase("ClassifyPathFeasibility_CombinesRegexHexEscapesAndStringEquality", "\\A\\u0041\\x42\\z", "AB",
            false, null, AnalysisProofOutcome.Proven, Feasibility.Unsatisfiable);
        yield return CreateServiceRegexCase("ClassifyImplication_ProvesShorthandRegexLengthFact", @"\A\d\s\w\z", null, true, 3,
            AnalysisProofOutcome.Proven, Feasibility.Unsatisfiable);
        yield return CreateServiceRegexCase("ClassifyPathFeasibility_NegatedShorthandRegexClassConcreteMatchIsSelfVerified", @"\A[^\d]\z",
            "A", true, null, null, Feasibility.Satisfiable);
        yield return CreateServiceRegexCase("ClassifyPathFeasibility_ShorthandRegexConcreteMismatchIsRejectedByDotNetValidation",
            @"\A\d\z", "A", true, null, AnalysisProofOutcome.Proven, Feasibility.Unsatisfiable, "path_unsatisfiable");
        yield return CreateServiceRegexCase("ClassifyImplication_ProvesCategoryRegexLengthFact", @"\A\p{Lu}\P{Ll}\z", null, true, 2,
            AnalysisProofOutcome.Proven, Feasibility.Unsatisfiable);
        yield return CreateServiceRegexCase("ClassifyImplication_WordBoundaryRegexLengthFactRemainsConservative", @"\A\bAB\B?\z", null,
            true, 2, AnalysisProofOutcome.Unknown, Feasibility.Unknown);
        ...
        yield return CreateServiceRegexCase("ClassifyPathFeasibility_InvalidConcreteRegexRemainsUnknown", "(", "A", true, null,
            AnalysisProofOutcome.Unknown, Feasibility.Unknown);
    }
    ...
    public void ClassifyPathFeasibility_RuntimeTypeTestPredicateContradictsItsNegationThroughReferenceEquality() {
        ...
        var xEqualsY = Equal(x, y);
        var xIsString = new SmtRuntimeTypeTestFormula(x, "System.String");
        var yIsNotString = new SmtUnaryFormula(SmtUnaryOperator.Not, new SmtRuntimeTypeTestFormula(y, "System.String"));
        var service = new SmtAnalysisService(SmtAnalysisOptions.Default);
    ...
    private static AnalysisProofQuery CreateQuery(IReadOnlyList<SmtFormula> pathConditions, SmtFormula triggerCondition)
        => new(pathConditions, new AnalysisHazard(AnalysisHazardKind.EffectViolationReachability, triggerCondition));
    ...
    private static AnalysisProofResult CreateTransientFailure() => new(
            AnalysisProofOutcome.Unknown,
            new ProofCheckInfo(
                true,
                Feasibility.Unknown,
                new SmtSatisfyingWitness(SmtWitnessStatus.Unsupported, "z3_transient_failure", Array.Empty<SmtModelAssignment>())),
            new ProofCheckInfo(false, Feasibility.Unknown),
            "path_feasibility_unknown");
    private static AnalysisProofResult CreateImpureResult() => new(
            AnalysisProofOutcome.Disproven,
            new ProofCheckInfo(true, Feasibility.Satisfiable),
            new ProofCheckInfo(true, Feasibility.Satisfiable),
            "impure_call_reachable");
    private sealed class StubProofSearchSession(Func<AnalysisProofQuery, TimeSpan, AnalysisProofResult> classify,
        Action? dispose = null) : IAnalysisProofSearchSession {
        private readonly Func<AnalysisProofQuery, TimeSpan, AnalysisProofResult> _classify = classify;
        private readonly Action? _dispose = dispose;

        public long ConsumedResourceCount => 0;

        public AnalysisProofResult Classify(AnalysisProofQuery query, TimeSpan timeout) => _classify(query, timeout);
    ...
    private sealed class ProjectConfigOptions : AnalyzerConfigOptions {
        ...
        internal ProjectConfigOptions(ImmutableDictionary<string, string> values) => _values = values;
