...
internal static class CSharpConditionToFormula
{
        ...
        [NotNullWhen(true)] out SmtFormula? formula)
    {
        ...
        if (expression is ConditionalExpressionSyntax conditionalExpression &&
            ...
            TryTranslateValue(conditionalExpression.WhenTrue, semanticModel, cancellationToken,
        ...
        if (TryTranslateIntegralTerm(expression, semanticModel, cancellationToken, out formula)) return true;

        var symbol = semanticModel.GetSymbolInfo(expression, cancellationToken).Symbol;
        if (symbol is not ILocalSymbol && symbol is not IParameterSymbol) return false;
