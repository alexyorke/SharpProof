...
internal sealed class AcyclicBlockPredicateExecutor {
    ...
    private sealed class Run(
        ...
        internal SymbolicBodyExecution Execute() {
            ...
            if (order.IsDefault) return Failed();
            foreach (var blockId in order) {
                var state = Merge(blockId);
                if (state == null) {
                    if (_reason != WorkerClaimReason.None) return Failed();
                    continue;
                }
                if (!ExecuteBlock(program.GetBlock(blockId), state.Value)) return Failed();
            }
            ...
            for (var index = 0; index < block.Instructions.Length; index++) {
                ...
                switch (instruction) {
                    ...
                    case IrReturnInstruction returned:
                        if (index != block.Instructions.Length - 1 || returned.Value == null) return false;
                        var returnTerm = Substitute(returned.Value, environment);
                        if (returnTerm == null) return false;
        ...
        private bool TransferBranch(
            ...
            if (condition == null || condition.Type != factory.BooleanType) return false;
            ...
            if (!Supported(whenTrue) || !Supported(whenFalse)) return false;
        ...
        private FlowState? Merge(IrBlockId block) {
            ...
            foreach (var variable in values[0].Environment.Keys.OrderBy(static value => value.Value)) {
                if (!Spend(values.Count)) return null;
                if (values.Any(value => !value.Environment.ContainsKey(variable))) continue;
                ...
                if (!Supported(merged)) return null;
        ...
        private SpecApplication? ApplySpec(
            ...
            if (instantiated.Status != SpecInstantiationStatus.Succeeded) return null;
            ...
            var predicates = instantiated.Postconditions
                ...
                .ToArray();
            if (predicates.Length == 0 || predicates.Any(predicate => !Supported(predicate))) return null;
            if (projection != default) _projections[call.Target.Value] = projection;
        ...
        private ImmutableDictionary<IrVarId, IrTerm>? CreateCurrentStates(
            ImmutableDictionary<IrVarId, IrTerm> environment) {
            if (!Spend(variables.Length + parameterBindings.Count)) return null;
        ...
        private ImmutableArray<IrBlockId> CreateOrder() {
            ...
            while (pending.Count != 0) {
                ...
                if (frame.Exit) {
                    ...
                    if (complete.Add(frame.Block)) result.Add(frame.Block);
                    continue;
                }
                if (complete.Contains(frame.Block)) continue;
                if (!active.Add(frame.Block)) return default;
        ...
        private void AddIncoming(
            ...
            ImmutableDictionary<IrVarId, IrTerm> environment) {
            if (predicate is IrBooleanTerm { Value: false }) return;
        ...
        private IrTerm? Substitute(
            IrTerm term,
            IReadOnlyDictionary<IrVarId, IrTerm> environment) {
            if (!IrTraversal.CollectVariables(term).All(environment.ContainsKey)) return null;
            try {
        ...
        private bool Supported(IrTerm term) =>
            SymbolicTermOperations.GetDepth(term) <= maximumExpressionDepth;
        ...
        private SymbolicBodyExecution Failed() => SymbolicBodyExecution.Failed(
...
internal sealed record SymbolicBodyExecution(
    ...
    internal static SymbolicBodyExecution Failed(WorkerClaimReason reason) =>
        new(reason, [], ImmutableDictionary<IrVarId, SpecResultProjection>.Empty, []);
}
