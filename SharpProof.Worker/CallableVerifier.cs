...
internal sealed class CallableVerifier(ISmtBackend backend, int maximumExpressionDepth) {
    ...
    private readonly int _maximumExpressionDepth = maximumExpressionDepth > 0 ? maximumExpressionDepth : throw new ArgumentOutOfRangeException(nameof(maximumExpressionDepth));
    internal async Task<ImmutableArray<WorkerClaimResult>> VerifyAsync(CompilerCallablePreparation target, MethodResourceBudget resourceBudget, CancellationToken cancellationToken) {
        ArgumentNullException.ThrowIfNull(resourceBudget);
        cancellationToken.ThrowIfCancellationRequested();
        if (!target.IsSuccess) return CallableClaimResultAssembler.PostconditionUnknowns(target, target.FailureReason);
        ...
        if (!body.IsSuccess) return CallableClaimResultAssembler.PostconditionUnknowns(target, body.Reason);
        ...
        foreach (var clause in target.Clauses) {
            ...
            if (clause.Kind == CompilerContractKind.Requires) preconditions.Add(assumption);
            if (clause.Kind == CompilerContractKind.Assume) userAssumptionIds.Add(justification, clause.AssumptionId!);
        ...
        if (!TryAddSourceDomainAssumptions(
            ...
            return CallableClaimResultAssembler.PostconditionUnknowns(target, WorkerClaimReason.UnsupportedExpression);
        var normalCompletion = AddNormalCompletionAssumption(factory, body.Returns, body.SpecResultProjections, assumptions, assumptionLabels);
        ...
        if (!contradictoryPreconditions && preconditions.Any(static assumption =>
            ...
            var preconditionOutcome = await ProbeSatisfiabilityAsync(factory, preconditions, replayVariables, resourceBudget, cancellationToken).ConfigureAwait(false);
            if (preconditionOutcome == null) return CallableClaimResultAssembler.PostconditionUnknowns(target, WorkerClaimReason.ResourceLimit);
            contradictoryPreconditions = preconditionOutcome is ProvenOutcome;
            if (preconditionOutcome is not (ProvenOutcome or RefutedOutcome)) vacuityUnknown = preconditionOutcome;
        }
        ...
        if (!contradictoryPreconditions && vacuityUnknown == null && normalCompletion is not IrBooleanTerm) {
            var completionOutcome = await ProbeSatisfiabilityAsync(factory, assumptions.Where(static assumption => assumption.Justification is not UserAssumedJustification), replayVariables, resourceBudget, cancellationToken).ConfigureAwait(false);
            if (completionOutcome == null) return CallableClaimResultAssembler.PostconditionUnknowns(target, WorkerClaimReason.ResourceLimit);
            noModeledNormalReturn = completionOutcome is ProvenOutcome;
            if (completionOutcome is not (ProvenOutcome or RefutedOutcome)) vacuityUnknown = completionOutcome;
        }
        ...
        for (var index = 0; index < ensures.Length; index++) {
            ...
            if (outcome is ProvenOutcome && vacuityUnknown != null) outcome = vacuityUnknown;
    ...
    private async Task<ProofOutcome?> ProbeSatisfiabilityAsync(IrFactory factory, IEnumerable<Assumption> evidence, ImmutableArray<IrVarId> replayVariables, MethodResourceBudget resourceBudget, CancellationToken cancellationToken) {
        if (!resourceBudget.TryStartQuery()) return null;
        var outcome = await _kernel.VerifyAsync(new VerificationQuery(factory, evidence, new Goal(factory, factory.Boolean(false), ProofDiagnosticKind.InternalConsistency, new SourceLocationId(0)), replayVariables), cancellationToken).ConfigureAwait(false);
        return resourceBudget.IsExceeded ? null : outcome;
    }
    private static SymbolicBodyExecution TrivialBody(IrFactory factory) => new(
        WorkerClaimReason.None,
        [new SymbolicReturn(factory.Boolean(true), null, ImmutableDictionary<IrVarId, IrTerm>.Empty)],
        ImmutableDictionary<IrVarId, SpecResultProjection>.Empty, []);
    private static string ClauseLabel(CompilerContractKind kind) => kind switch {
