...
internal static class PostconditionObligationBuilder {
    ...
    internal static IrTerm AddNormalCompletionAssumption(
        ...
        foreach (var path in returns) {
            var completion = path.ReturnTerm is null or IrBooleanTerm or IrIntegerTerm or IrStringTerm or IrNullTerm or IrVariableTerm ? path.Predicate : factory.Binary(
    ...
    private static (int Order, string Label) Domain(
        CompilerCanonicalVariable variable) => variable.Role switch {
