...
try {
    $actualHistoricalTree = (& git rev-parse "HEAD:$($contract.historicalBaseline.path)").Trim()
    if ($LASTEXITCODE -ne 0) {
        throw 'Could not resolve the historical acceptance tree.'
    }
    Assert-Equal `
        $actualHistoricalTree `
        $contract.historicalBaseline.gitTree `
        'historicalBaseline.gitTree'
    ...
    & (Join-Path $repositoryRoot 'scripts\Measure-ProductionCSharp.ps1')
    if ($LASTEXITCODE -ne 0) {
    ...
    $testProjects = @(
        'SharpProof.Analyzer.V2.Test\SharpProof.Analyzer.V2.Test.csproj',
        ...
        'SharpProof.Migration.Test\SharpProof.Migration.Test.csproj',
        ...
        'SharpProof.V2Gates.Test\SharpProof.V2Gates.Test.csproj',
        'SharpProof.V2Fuzz.Test\SharpProof.V2Fuzz.Test.csproj',
    ...
    foreach ($testProject in $testProjects) {
        ...
        if (-not (Test-Path -LiteralPath $resolvedTestProject -PathType Leaf)) {
            throw "Required v2 test project is missing: $testProject"
        }
    ...
    Invoke-SharpProofDotnet -Arguments @(
        'run',
        '--project',
        'Tools\SharpProof.V2Fuzz\SharpProof.V2Fuzz.csproj',
        '-c',
        $Configuration,
        '--no-build',
        '--',
    ...
    Invoke-SharpProofDotnet -Arguments @(
        'run',
        '--project',
        'SharpProof.V2Gates\SharpProof.V2Gates.csproj',
        '-c',
        $Configuration,
        '--no-build',
        '--',
...
Write-Host 'SharpProof v2 acceptance checks passed.'
