...
- **SMT/Z3 analysis** is enabled by default with bounded-heavy guardrails. Configure it with `purelysharp_smt_mode` (`off`, `bounded`, `deep`; default `bounded`), `purelysharp_smt_timeout_ms` (bounded default `750`, deep default `2000`), `purelysharp_smt_method_budget_ms` (bounded default `5000`, deep default `15000`), `purelysharp_smt_max_path_conditions` (bounded default `192`, deep default `512`), and `purelysharp_smt_max_expression_nodes` (bounded default `2048`, deep default `8192`). Unsupported formulas, budget exhaustion, timeouts, or Z3 load failures fall back to conservative `unknown` results.
