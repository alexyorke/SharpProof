...
| R231 | `<SharpProofSpecificationPacks Condition="'$(SharpProofSpecificationPacks)' == ''"></SharpProofSpecificationPacks>` assigns empty to empty. It appears twice. | `SharpProof.AnalyzerConsumer.props:5`; `SharpProof.Package/buildTransitive/SharpProof.props:12` |
| R232 | The `SharpProofProductionProject` exclusion regex still names `PortableAnalyzer`, a project with no tracked files (it was removed; see `docs/soundness-notes/2026-08-08-coverage-authority-and-tcb-ownership.md`). An architecture test pins the dead arm in place, so the classification condition cannot shrink without editing the test that guards it. Distinct from the deliberate `PortableAnalyzer` regression guards in `SharpProof.ConsumerContract.props` and `Test-SharpProofPackageConsumers.ps1`, which assert absence and should stay. | `Directory.Build.props:36`; `SharpProof.ArchitectureTest/BoundaryEnforcementTests.cs:47` |
...
| ID | Finding | Evidence |
|---|---|---|
| R238 | `dev` and `loop` each re-declare, in full, the four environment variables and the two cache volumes already carried by `x-sharpproof-common`, because a YAML merge key replaces a whole mapping rather than merging into it. A second anchor merged into each service's `environment` and `volumes` removes about 14 duplicated lines. Distinct from applied R064/R158, which removed unused service aliases. | `compose.yaml:36-72` |
| R239 | Four `switch` arms in `build.ps1` (`quick`, `pr`, `nightly`, `security`) exist only to force a configuration before delegating. A profile-to-configuration table plus the existing `default` arm covers them. | `build.ps1:64-78,113-120` |
...
| R241 | `entrypoint.sh` runs `install -d -o sharpproof -g sharpproof` over five paths and then `chown sharpproof:sharpproof` over the same five. GNU `install -d` applies the requested ownership to directories that already exist, so the second command is expected to be redundant. Needs a container-local check before removal, because behaviour on the bind-mounted repository root is the whole point of the block. | `eng/container/entrypoint.sh:10-22` |
| R242 | The entire task-workspace block, from `task_root="$(mktemp -d ...)"` to the final `exec pwsh`, is indented four spaces at top level: leftover shape from the single-arm `case` removed by applied R130. | `eng/container/entrypoint.sh:110-176` |
...
| ID | Finding | Evidence |
|---|---|---|
| R249 | C# string-literal escaping is implemented eight times under three names: `Quote-CSharpString` twice, `ConvertTo-CSharpString` five times, `ConvertTo-CSharpStringLiteral` once. Five bodies are behaviourally identical (escape backslash, then quote); the `ApiSpecCatalog` and `DiagnosticDescriptors` variants additionally escape CR, LF, and tab, so the superset is the safe unification. All eight files already dot-source `GeneratedFileHelpers.ps1`, which is the natural home. | `Generate-AnalyzerDiagnosticCatalog.ps1:32`, `Generate-ApiSpecCatalog.ps1:290`, `Generate-CompilerArtifactModel.ps1:110`, `Generate-ContractApiCatalog.ps1:131`, `Generate-DiagnosticDescriptors.ps1:55`, `Generate-EffectContractMappings.ps1:29`, `Generate-LauncherArguments.ps1:69`, `Generate-ProtocolModel.ps1:59` |
| R250 | The same validators are redefined across generators with identical regexes: `Assert-Identifier`/`Identifier` in 11 files, `Assert-TypeName`/`TypeName` in 6, `Get-RequiredProperty`/`Required` in 4, `NamespaceName` in 2. About 23 duplicate function bodies, all in files that already share `GeneratedFileHelpers.ps1`. | `scripts/Generate-*.ps1`, function definitions as listed |
| R251 | `$repositoryRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path` appears in 57 scripts and modules, and the 14 generators then repeat the "if `IsNullOrWhiteSpace` then `Join-Path` then `GetFullPath`" default-path idiom 43 times, once per output path. Both are candidates for helpers in `GeneratedFileHelpers.ps1`. Related to applied R026, which shared the equivalent helper on the C# test side only. | `scripts/*.ps1`, `scripts/*.psm1` |
| R252 | Three spellings of the generated-file header coexist: `// <auto-generated>` with a closing `// </auto-generated>`, `// <auto-generated />`, and `// <auto-generated/>`. Consistency is nearly free once R249-R251 give the generators a shared header emitter, and is the safe part of deferred R107. | `Generate-CSharpScalarSemantics.ps1:726,1033`; `Generate-ContractApiCatalog.ps1:279`; `Generate-IrModel.ps1:586`; and the rest |
| R253 | `scripts/Generate-Readme.ps1` is 1,069 lines that never write a file. It is a documentation validator misfiled in the `Generate-*` family, and its `-Verify` switch changes nothing except which of two success messages is printed. Renaming it into the `Test-SharpProof*` family and dropping the switch removes a false generator from the generator inventory and about nine lines. | `scripts/Generate-Readme.ps1:1059-1069`; callers at `scripts/Invoke-SharpProofContainer.ps1:547,636` |
...
R229 and R231 through R262 are `pending`, and the active follow-up queue above is
...
| ID | Finding | Evidence |
|---|---|---|
| R266 | `SHARPPROOF_PORTABLE_ARGUMENT_GUARD` is defined by no project, `.props`, or script, yet it survives as a term in two `#if` conditions. It is left over with the removed `SharpProof.PortableAnalyzer` project, the same removal that left the dead classification arm in R232. A sweep of every `#if` symbol in tracked C# found `NEVER`, `SHARPPROOF_DATAFLOW_ARGUMENT_GUARD`, `SHARPPROOF_NEGATIVE_PROBE`, `SHARPPROOF_PROBE_GENERATED`, and `SHARPPROOF_SMT_ARGUMENT_GUARD` all live; this is the only dead one. | `SharpProof.Ir/ArgumentNullGuard.cs:1,98`; no `DefineConstants` anywhere declares it |
| R267 | `ArgumentNullGuard.RequireNonnegative(int)` and `RequireNonnegative(long)` have byte-identical bodies, as do `RequirePositive(int)` and `RequirePositive(long)`. The `int` overloads can forward to the `long` ones. Note that a generic collapse over `INumber<T>` is not available: the file is compiled into netstandard2.0 assemblies. | `SharpProof.Ir/ArgumentNullGuard.cs:21-79` |
...
R263 through R276 are `pending` and extend the same follow-up queue. R263, R265,
