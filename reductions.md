...
| R345 | **`Analyzer.Core` carries two single-call private static forwarders to `DefiniteOperationFacts.IsDefinitelyString`.** `EffectCallPreconditionPolicy.cs:299-304` and `RequiresCallSiteAnalyzer.cs:692-695` both define `private static bool IsDefinitelyString(IOperation operation) => DefiniteOperationFacts.IsDefinitelyString(operation);` with exactly one caller each (lines 114 and 407 respectively). `DefiniteOperationFacts.IsDefinitelyString` is already `internal static` in `SharpProof.Effects` and is directly accessible to all files in `SharpProof.Analyzer.Core` via `GlobalUsings.cs:16`. Inlining the call directly eliminates two redundant wrappers. | `SharpProof.Analyzer.Core/EffectCallPreconditionPolicy.cs:114,299-304`; `SharpProof.Analyzer.Core/RequiresCallSiteAnalyzer.cs:407,692-695`; `SharpProof.Effects/ManagedAbstractFlow.cs:2847-2855`; `SharpProof.Analyzer.Core/GlobalUsings.cs:16` |
| R346 | **`ProtocolJson.ComputeSha256` re-implements `HashEncoding.ComputeSha256Hex`.** `SharpProof.Worker.Protocol/ProtocolJson.cs:1062-1067` instantiates `SHA256.Create()` and string-formats byte results with `"x2"` using invariant culture. `SharpProof.Ir/HashEncoding.cs:13-17` already declares the identical `ComputeSha256Hex(byte[] bytes)` and `ToLowerHex(IEnumerable<byte> bytes)`. Linking `HashEncoding.cs` into `SharpProof.Worker.Protocol` (as `SharpProof.CompilerProbe.TestAsset` already does) or sharing the encoding helper unifies SHA-256 formatting between wire protocols and IR hashing. | `SharpProof.Worker.Protocol/ProtocolJson.cs:1062-1067`; `SharpProof.Ir/HashEncoding.cs:1-18`; `SharpProof.CompilerProbe.TestAsset/SharpProof.CompilerProbe.TestAsset.csproj:14` |
| R347 | **`build.ps1` defines two redundant switch branches with identical validation and container dispatch.** Lines 101-107 handle `'package-consumers'` by verifying `-PackageSource` and invoking `Invoke-Container 'package-consumers' $Configuration @('-PackageSource', $PackageSource)`. Lines 108-114 handle `{ $_ -in @('pilots', 'release-plan') }` with the exact same validation and dispatch. Merging `'package-consumers'` into the multi-target arm eliminates 7 redundant lines. | `build.ps1:101-114` |
| R348 | **`Invoke-SharpProofContainer.ps1` duplicates the `SharpProof.Gates` restore/run invocation in `'performance-smoke'`.** The switch block at lines 418-426 already defines a multi-target arm `{ $_ -in @('corpus', 'corpus-update', 'gates') }` that restores `SharpProof.Gates.csproj` and runs `dotnet run --project ... -- $gateMode`. Lines 438-445 define a separate `'performance-smoke'` arm executing the exact same restore and run with argument `'performance-smoke'`. Folding `'performance-smoke'` into the multi-target arm removes 8 lines. | `scripts/Invoke-SharpProofContainer.ps1:418-426,438-445` |
