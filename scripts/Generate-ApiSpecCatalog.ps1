...
function Format-Term {
    ...
    switch -CaseSensitive ($kind) {
        ...
        'boolean' {
            ...
            if ($type -ne 'SpecValueType.Boolean') {
        ...
        'integer' {
            $value = [long](
                Get-RequiredProperty $Term 'value' $Context)
            if ($type -ne 'SpecValueType.Integer') {
        ...
        'string' {
            $value = Assert-Text `
                ...
                -AllowEmpty
            if ($type -ne 'SpecValueType.String') {
