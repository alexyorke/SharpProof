...
foreach ($set in @($catalog.assemblySets)) {
    ...
    $identitySet = [Collections.Generic.HashSet[string]]::new(
        [StringComparer]::Ordinal)
    foreach ($key in $keys) {
        if (-not $identitySet.Add([string]$key)) {
...
function Assert-UniqueOrdinal {
    ...
    foreach ($value in $Values) {
        if (-not $seen.Add([string]$value)) {
            throw "$Context must be unique under ordinal comparison."
        }
    }
}
...
$declarations.Sort([Comparison[object]]{
    param($left, $right)
    [StringComparer]::Ordinal.Compare(
        [string]$left.target.witnessIdentifier,
        [string]$right.target.witnessIdentifier)
})
...
Assert-UniqueOrdinal $witnesses 'API-spec witness identifiers'
$factoryNames = @($witnesses | ForEach-Object {
    ConvertTo-RuntimeWitnessFactoryName $_
})
Assert-UniqueOrdinal $factoryNames 'API-spec witness factory names'

$source = [Collections.Generic.List[string]]::new()
$source.Add('// <auto-generated>')
$source.Add('// Generated by scripts/Generate-ApiSpecCatalog.ps1 from')
$source.Add('// SharpProof.Specs/DefaultApiSpecCatalog.json.')
$source.Add('// Do not edit this file directly.')
$source.Add('// </auto-generated>')
$source.Add('#nullable enable')
