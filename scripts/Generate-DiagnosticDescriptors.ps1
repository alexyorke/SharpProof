...
foreach ($output in $outputs) {
    ...
    $lines.Add('// Generated by scripts/Generate-DiagnosticDescriptors.ps1 from')
    $lines.Add('// eng/diagnostics/diagnostic-descriptors.v1.json.')
    ...
    if ($null -ne $supportedMember) {
        ...
        $lines.Add(
            ...
            "$supportedMember = [")
        for ($index = 0; $index -lt $diagnostics.Count; $index++) {
            $symbol = [string]$diagnostics[$index].symbol
