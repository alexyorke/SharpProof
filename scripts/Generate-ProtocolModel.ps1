...
$lines.Add('// Generated by scripts/Generate-ProtocolModel.ps1 from')
$lines.Add('// SharpProof.Worker.Protocol/ProtocolModel.schema.json.')
...
$lines.Add(
    '    private static string CreateManifestPayload(' +
    'WorkerClaimManifest manifest) {')
$lines.Add(
    '        var writer = new ManifestWriter().Add(' +
    (ConvertTo-CSharpString $manifestDomain) + ')')
$lines.Add("            .Add($manifestVersionMember);")
foreach ($field in @(
        Get-RequiredMember `
            $manifestIdentity 'rootFields' 'manifest identity')) {
    $label = [string](
        Get-RequiredMember $field 'label' 'manifest identity root field')
    $propertyName = [string](
        Get-RequiredMember $field 'property' 'manifest identity root field')
    $kind = [string](
        Get-RequiredMember $field 'kind' 'manifest identity root field')
    $resolved = Resolve-ValidationProperty `
        'WorkerClaimManifest' $propertyName 'manifest'
    if ($kind -ne 'int' -or $resolved.Type -ne 'int') {
        throw "Manifest root field '$propertyName' has an invalid kind."
    }
    $lines.Add(
        '        writer.Add(' + (ConvertTo-CSharpString $label) +
        ").Add($($resolved.Source));")
}
foreach ($collection in @(
    ...
    $propertyName = [string](
        Get-RequiredMember `
            $collection 'property' 'manifest identity collection')
    $lengthLabel = [string](
        Get-RequiredMember `
            $collection 'lengthLabel' 'manifest identity collection')
    $entryLabel = [string](
        Get-RequiredMember `
            $collection 'entryLabel' 'manifest identity collection')
    $resolvedCollection = Resolve-ValidationProperty `
        'WorkerClaimManifest' $propertyName 'manifest'
    if ($resolvedCollection.Type -notmatch '^(?<element>[A-Za-z_][A-Za-z0-9_]*)\[\]$') {
        throw "Manifest identity collection '$propertyName' is not an array."
    }
    $elementType = $Matches['element']
    $lines.Add(
        '        writer.Add(' + (ConvertTo-CSharpString $lengthLabel) +
        ").Add($($resolvedCollection.Source)?.Length ?? -1);")
    $order = @(
        Get-RequiredMember `
            $collection 'order' "manifest collection '$propertyName'")
    $lines.Add(
        "        foreach (var entry in ($($resolvedCollection.Source) ?? [])")
    for ($index = 0; $index -lt $order.Count; $index++) {
        $orderProperty = [string](
            Get-RequiredMember `
                $order[$index] 'property' "manifest collection '$propertyName' order")
        $orderKind = [string](
            Get-RequiredMember `
                $order[$index] 'kind' "manifest collection '$propertyName' order")
        $resolvedOrder = Resolve-ValidationProperty `
            $elementType $orderProperty 'value?'
        $operator = if ($index -eq 0) { 'OrderBy' } else { 'ThenBy' }
        switch ($orderKind) {
            'ordinalString' {
                if ($resolvedOrder.Type -ne 'string') {
                    throw "Manifest order '$orderProperty' must be a string."
                }
                $lines.Add(
                    "            .$operator(static value => " +
                    "$($resolvedOrder.Source), StringComparer.Ordinal)")
            }
            'integer' {
                if ($resolvedOrder.Type -ne 'int') {
                    throw "Manifest order '$orderProperty' must be an integer."
                }
                $lines.Add(
                    "            .$operator(static value => " +
                    "$($resolvedOrder.Source) ?? int.MinValue)")
            }
            default {
                throw "Unsupported manifest order kind '$orderKind'."
            }
        }
    }
    $lines[$lines.Count - 1] += ') {'
    $lines.Add(
        '            writer.Add(' + (ConvertTo-CSharpString $entryLabel) + ');')
    foreach ($field in @(
            Get-RequiredMember `
                $collection 'fields' "manifest collection '$propertyName'")) {
        $label = [string](
            Get-RequiredMember `
                $field 'label' "manifest collection '$propertyName' field")
        $fieldProperty = [string](
            Get-RequiredMember `
                $field 'property' "manifest collection '$propertyName' field")
        $fieldKind = [string](
            Get-RequiredMember `
                $field 'kind' "manifest collection '$propertyName' field")
        $resolvedField = Resolve-ValidationProperty `
            $elementType $fieldProperty 'entry?'
        $quotedLabel = ConvertTo-CSharpString $label
        switch ($fieldKind) {
            'string' {
                $lines.Add(
                    "            writer.Add($quotedLabel)" +
                    ".Add($($resolvedField.Source));")
            }
            'int' {
                $lines.Add(
                    "            writer.Add($quotedLabel)" +
                    ".Add($($resolvedField.Source) ?? -1);")
            }
            'enum' {
                $defaultMember = [string](
                    Get-RequiredMember `
                        $field 'defaultMember' "manifest field '$fieldProperty'")
                Assert-ValidationMember $defaultMember "Manifest identity"
                $lines.Add(
                    "            writer.Add($quotedLabel).Add(" +
                    "ManifestName($($resolvedField.Source) ?? $defaultMember));")
            }
            'location' {
                $lines.Add(
                    "            writer.AddLocation($quotedLabel, " +
                    "$($resolvedField.Source));")
            }
            'enumArray' {
                $lines.Add(
                    "            writer.AddItems($quotedLabel, " +
                    "$($resolvedField.Source),")
                $lines.Add(
                    "                SortManifestEnums($($resolvedField.Source)),")
                $lines.Add(
                    '                static (target, value) => ' +
                    'target.Add(ManifestName(value)));')
            }
            'ordinalStringArray' {
                $lines.Add(
                    "            writer.AddItems($quotedLabel, " +
            ...
            'assumptionArray' {
                $lines.Add(
                    ...
                    "$($resolvedField.Source),")
                $lines.Add(
                    "                CanonicalizeAssumptions($($resolvedField.Source)),")
                $lines.Add(
                    '                static (target, value) => target.Add(value.Id)' +
                    '.Add(ManifestName(value.Kind)));')
            }
            default {
                throw "Unsupported manifest field kind '$fieldKind'."
            }
        }
    }
    $lines.Add('        }')
...
$producerLines.Add('// <auto-generated>')
$producerLines.Add('// Generated by scripts/Generate-ProtocolModel.ps1 from')
$producerLines.Add('// SharpProof.Worker.Protocol/ProtocolModel.schema.json.')
$producerLines.Add('// Do not edit this file directly.')
$producerLines.Add('// </auto-generated>')
$producerLines.Add('#nullable enable')
$producerLines.Add('')
