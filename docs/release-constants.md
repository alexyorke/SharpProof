...
| TCB paths and mutation catalog digest | `eng/acceptance/contract.json` | exact digest and deterministic mutation gates |
...
Source complexity, TCB inventory hashes, generated-output digests, package
hashes, SBOM hashes, and release evidence are computed. They are never copied
into production behavior. Checked-in ratchets are reviewed upper bounds or
expected digests, and their scripts recompute the current value before a gate
can pass.
