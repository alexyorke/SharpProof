# SharpProof v2 hardening audit - 2026-07-25
...
- Worker body verification uses a bounded, acyclic CFG executor for locals,
  reassignment, branches, multiple returns, entry-state `Old`, and pure resolved
  API specs. Loops, stateful operations, unknown calls, and exceeded bounds
  abstain.
...
- The 162-family legacy API inventory has one typed disposition per family.
  Only sound, resolved, executable ports are proof-producing; all other rows
  fail closed.
...
- The Phase 6 numerical checkpoint requiring executable runtime-tested coverage
  ...
  proof specs; inventing unvalidated summaries would be worse than abstaining.
