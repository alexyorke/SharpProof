...
| `ContractFor` validation | Analyzer final-compilation reconciliation, with the incremental generator loaded for package composition | Validates companion type and member identity, including receiver, overload, generic constraints, ref/scoped kinds, nullability, defaults, and return shape | The generator emits no source or diagnostics by itself; reconciliation validates and binds existing source and does not make an unsupported contract provable |
...
|---|---|---|
| Callable kinds | Non-generic ordinary methods, instance and static constructors, property getters/setters, event add/remove accessors, explicit interface implementations, and members of primary-constructor types (captured parameters are modeled as receiver-backed state) | Async methods, generic source methods, ref returns, ref parameters, declarations without an operation root, the synthesized primary-constructor body, and unsupported method kinds |
...
explicit first receiver parameter. The analyzer's final-compilation
