...
`SharpProofAnalysisSession.Analyze(new SharpProofQuery(SharpProofQueryKind.Complexity, target))` returns a
...
Construct a `SharpProofQuery` with `SharpProofQueryKind.Complexity` and use a session created from:
