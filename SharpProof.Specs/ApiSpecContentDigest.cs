...
internal static class ApiSpecContentDigest {
    internal static string Compute(ImmutableArray<ApiSpecTemplate> templates) {
        ...
        foreach (var template in templates) {
            ...
            foreach (var type in target.ParameterTypes) hash.Add(type);
            ...
            foreach (var exception in facets.Throws.ExceptionMetadataNames) hash.Add(exception);
            ...
            foreach (var variable in template.Variables) hash.Add(variable.Role, variable.Ordinal, variable.Type);
    ...
    private static void Add(
        ...
        if (parts.Payload != null) hash.Add(parts.Payload);
        foreach (var child in parts.Children) Add(hash, child, variables);
    }
}
