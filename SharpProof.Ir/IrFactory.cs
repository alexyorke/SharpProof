...
public sealed class IrFactory {
    ...
    public IrIdentityId CreateIdentity() { lock (_gate) return CreateIdentityCore(); }
    ...
    public IrIdentityId InternExternalIdentity<T>(T identity, IEqualityComparer<T> comparer) where T : notnull {
        ...
        if (typeof(T) == typeof(string)) throw new ArgumentException(
            "Semantic identities cannot be interned from strings.", nameof(identity));
        lock (_gate) {
            var key = new ExternalIdentityKey<T>(identity, comparer);
            if (_externalIdentityIds.TryGetValue(key, out var existing)) return existing;
    ...
    public string GetString(IrStringId id) { lock (_gate) return GetScoped(id.Scope, id.Value, _strings, nameof(id)); }
    ...
    public IrTypeInfo GetTypeInfo(IrTypeId id) { lock (_gate) return GetTypeInfoCore(id, nameof(id)); }
    ...
    public IrVariableInfo GetVariableInfo(IrVarId id) { lock (_gate) return GetVariableInfoCore(id, nameof(id)); }
    ...
    public IrMemberId GetOrCreateMember(IrIdentityId identity, IrTypeId declaringType, string name, IrTypeId returnType, bool isStatic,
        ...
        lock (_gate) {
            ...
            foreach (var parameterType in parameterTypes) GetTypeInfoCore(parameterType, nameof(parameterTypes));
    ...
    public IrMemberInfo GetMemberInfo(IrMemberId id) { lock (_gate) return GetMemberInfoCore(id, nameof(id)); }
    ...
    public IrOperationInfo GetOperationInfo(OperationId id) { lock (_gate) return GetOperationInfoCore(id, nameof(id)); }

    public IrTerm GetTerm(IrId id) { lock (_gate) return GetScoped(id.Scope, id.Value, _terms, nameof(id)); }

    public IrValue CreateBooleanValue(bool value) => new(BooleanType, IrValueKind.Boolean, value);
    public IrValue CreateIntegerValue(long value) => new(IntegerType, IrValueKind.Integer, value);
    ...
    public IrTerm Conditional(IrTerm condition, IrTerm whenTrue, IrTerm whenFalse) {
        ...
        lock (_gate) {
            ...
            if (condition.Type != BooleanType) throw new ArgumentException("The conditional guard must be boolean.", nameof(condition));
            if (whenTrue.Type != whenFalse.Type) throw new ArgumentException(
    ...
    public IrTerm Length(IrTerm value) {
        ...
        lock (_gate) {
            ...
            if (info.Kind is not (IrTypeKind.String or IrTypeKind.Sequence)) throw new ArgumentException(
    ...
    internal void EnsureTerm(IrTerm term, string parameterName) { lock (_gate) EnsureTermCore(term, parameterName); }
    ...
    internal IrTypeId ValidateSequenceTerms(
        ...
        lock (_gate) return ValidateSequenceTermsCore(
    ...
    internal void ValidateCallShape(
        ...
        lock (_gate) ValidateCallShapeCore(member, receiver, arguments, parameterName, opaque: false);
    }
    ...
    private IrOpaqueTerm Opaque(IrMemberId member, IrTerm? receiver, IrTerm[] arguments, IrOpaquePurity purity, OperationId operation) {
        ...
        lock (_gate) {
            ...
            if (purity == IrOpaquePurity.Pure) {
                if (!operation.IsDefault) throw new ArgumentException(
                    "Pure opaque terms cannot carry an operation identity.", nameof(operation));
            }
            else GetOperationInfoCore(operation, nameof(operation));
    ...
    private IrTypeId ValidateSequenceTermsCore(
        IrTerm sequence, IrTerm index, string sequenceMessage, string indexMessage,
        string sequenceParameter, string indexParameter) {
        if (sequence == null) throw new ArgumentNullException(sequenceParameter);
        if (index == null) throw new ArgumentNullException(indexParameter);
    ...
    private IrTerm? FoldBinary(IrBinaryOperator @operator, IrTerm left, IrTerm right) {
        if (@operator == IrBinaryOperator.AndAlso && left is IrBooleanTerm andLeft) return andLeft.Value ? right : left;
        if (@operator == IrBinaryOperator.OrElse && left is IrBooleanTerm orLeft) return orLeft.Value ? left : right;
        if (@operator is IrBinaryOperator.Equal or IrBinaryOperator.NotEqual) {
            var equal = TryCompareConstants(left, right);
            if (equal.HasValue) return Boolean(@operator == IrBinaryOperator.Equal ? equal.Value : !equal.Value);
        }
    ...
    private static bool? TryCompareConstants(IrTerm left, IrTerm right) {
        ...
        if (left is IrNullTerm && IsNonNullLiteral(right) || right is IrNullTerm && IsNonNullLiteral(left)) return false;
        return null;
    }
    ...
    private static IrTypeId RequireTypes(
        ...
        IrBinaryOperator @operator) {
        if (left.Type != expected || right.Type != expected) throw new ArgumentException(
            "Operands are not valid for binary operator " + @operator + ".", nameof(right));
        return result;
    }

    private static bool IsNonNullLiteral(IrTerm term) => term is IrBooleanTerm or IrIntegerTerm or IrStringTerm;
    private static bool IsNullable(IrTypeKind kind) => kind is IrTypeKind.String or IrTypeKind.Reference or IrTypeKind.Sequence;

    private IrTypeId GetBuiltInType(IrTypeKind kind) => kind switch {
    ...
    private static int PurityKey(IrOpaquePurity purity) => purity switch {
    ...
    private static void ValidateName(string? value, string parameterName) {
        if (string.IsNullOrWhiteSpace(value)) throw new ArgumentException("A non-empty name is required.", parameterName);
    }
    ...
    private string GetStringCore(IrStringId id) => GetScoped(id.Scope, id.Value, _strings, nameof(id));
    private IrIdentityId CreateIdentityCore() => new(_scope, _identityCount++);
    private IrTypeId CreateBuiltInType(string name, IrTypeKind kind) => GetOrCreateTypeCore(CreateIdentityCore(), name, kind, null);
    ...
    private IrTypeInfo GetTypeInfoCore(IrTypeId id, string parameterName) => GetScoped(id.Scope, id.Value, _types, parameterName);
    private IrVariableInfo GetVariableInfoCore(IrVarId id, string name) => GetScoped(id.Scope, id.Value, _variables, name);
    private IrMemberInfo GetMemberInfoCore(IrMemberId id, string parameterName) => GetScoped(id.Scope, id.Value, _members, parameterName);
    ...
    private void EnsureScope(long scope, string parameterName) {
        if (scope != _scope) throw new ArgumentException("The identifier belongs to a different IR factory.", parameterName);
    }
    ...
    private T GetScoped<T>(long scope, int value, IReadOnlyList<T> items, string parameterName) {
        ...
        if (value < 0 || value >= items.Count) throw new ArgumentOutOfRangeException(parameterName);
        return items[value];
    }
    ...
    private sealed class ExternalIdentityKey<T>(T value, IEqualityComparer<T> comparer) :
        ...
        public bool Equals(ExternalIdentityKey<T>? other) =>
            other != null && ReferenceEquals(_comparer, other._comparer) && _comparer.Equals(_value, other._value);
        public override bool Equals(object? obj) => Equals(obj as ExternalIdentityKey<T>);
    ...
    private readonly struct StructuralKey(
        ...
        public bool Equals(StructuralKey other) => _value.Equals(other._value);
    ...
    private readonly struct IntSequenceKey(ImmutableArray<int> values) : IEquatable<IntSequenceKey> {
        ...
        public override bool Equals(object? obj) => obj is IntSequenceKey other && Equals(other);
