...
public sealed class IrProgramInterpreter(IrFactory factory) {
    ...
    private IrEvaluationResult? EvaluateLocationOperands(IrLocation location, IrTerm? storedValue,
        ...
        switch (location) {
            ...
            case IrSequenceLocation sequence:
                ...
                if (sequenceResult.Status != IrEvaluationStatus.Value) return sequenceResult;
                var indexResult = _terms.Evaluate(sequence.Index, values, cancellationToken);
                if (indexResult.Status != IrEvaluationStatus.Value) return indexResult;
                if (storedValue != null) {
                    var storedValueResult = _terms.Evaluate(storedValue, values, cancellationToken);
                    if (storedValueResult.Status != IrEvaluationStatus.Value) return storedValueResult;
                }
    ...
    private IrEvaluationResult? EvaluateCallOperands(IrTerm? receiver, IReadOnlyList<IrTerm> arguments,
        ...
        if (receiver != null) {
            ...
            if (receiverResult.Status != IrEvaluationStatus.Value) return receiverResult;
